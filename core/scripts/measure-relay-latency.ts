...
const OFFER = {
  ...
  relay: { endpoint: "relay.rocky.sh:443" },
};

const DIRECT_ENDPOINT = "localhost:7767";
const PING_COUNT = 20;
...
async function main() {
  ...
  await measurePings("Direct (localhost:7767)", directClient, PING_COUNT, WARMUP_COUNT);
  ...
  await measurePings("Relay (relay.rocky.sh:443)", relayClient, PING_COUNT, WARMUP_COUNT);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const ws = new WebSocket(
      `wss://relay.rocky.sh/ws?serverId=latency_probe_${Date.now()}&role=client&clientId=probe_${i}`,
    );
    await new Promise<void>((resolve, reject) => {
      ws.on("open", () => {
