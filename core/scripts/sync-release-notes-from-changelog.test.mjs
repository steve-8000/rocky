...
function withTempChangelog(fn, changelogText = "## 0.1.60-beta.1 - 2026-04-20\n\n- Beta notes.\n") {
  ...
  const tempDir = mkdtempSync(path.join(tmpdir(), "rocky-release-notes-test-"));
...
test("updates an existing release body through the release id API", () => {
  withTempChangelog(() => {
    ...
    const execFileSync = (command, args, options) => {
      ...
      if (args[0] === "api" && args[1] === "repos/getrocky/rocky/releases/tags/v0.1.60-beta.1") {
    ...
    syncReleaseNotes(["--repo", "getrocky/rocky", "--tag", "v0.1.60-beta.1"], {
      execFileSync,
    });
    ...
    assert.equal(
      calls.some(
        (call) =>
          ...
          call.args[1] === "-X" &&
          call.args[2] === "PATCH" &&
          call.args[3] === "repos/getrocky/rocky/releases/311163621",
      ),
      true,
...
test("converts contributor profile links to mentions in synced release notes", () => {
  const changelogText = [
    ...
    "",
    "- Beta notes. ([#526](https://github.com/getrocky/rocky/pull/526) by [@therainisme](https://github.com/therainisme))",
    "",
  ].join("\n");
