...
let
  cfg = config.services.rocky;
in
{
  imports = [
    (lib.mkRenamedOptionModule [ "services" "rocky" "allowedHosts" ] [ "services" "rocky" "hostnames" ])
  ];

  options.services.rocky = {
    enable = lib.mkEnableOption "Rocky, a self-hosted daemon for AI coding agents";

    package = lib.mkPackageOption pkgs "rocky" { };

    user = lib.mkOption {
      type = lib.types.str;
      default = "rocky";
      description = "User account under which Rocky runs.";
    };

    group = lib.mkOption {
      type = lib.types.str;
      default = "rocky";
      description = "Group under which Rocky runs.";
    };
    ...
    dataDir = lib.mkOption {
      ...
      default =
        if cfg.user == "rocky"
        then "/var/lib/rocky"
        else "/home/${cfg.user}/.rocky";
      defaultText = lib.literalExpression ''
        if cfg.user == "rocky"
        then "/var/lib/rocky"
        else "/home/''${cfg.user}/.rocky"
      '';
      description = "Directory for Rocky state (ROCKY_HOME). Stores agent data, config, and logs.";
    };

    port = lib.mkOption {
      type = lib.types.port;
      default = 6767;
      description = "Port for the Rocky daemon to listen on.";
    };

    listenAddress = lib.mkOption {
      type = lib.types.str;
      default = "127.0.0.1";
      description = "Address for the Rocky daemon to bind to.";
    };

    openFirewall = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Whether to open the firewall for the Rocky daemon port.";
    };
    ...
    hostnames = lib.mkOption {
      ...
      description = ''
        Hostnames the Rocky daemon accepts in the Host header (DNS rebinding protection).
    ...
    relay = {
      ...
      mode = lib.mkOption {
        ...
        description = ''
          ...
          - `"hosted"` (default): use the upstream `app.rocky.sh` relay.
            Preserves the current behavior; no extra options needed.
          - `"remote"`: connect to a self-hosted relay at
            `relay.host:relay.port`. Sets `ROCKY_RELAY_ENDPOINT` and
            `ROCKY_RELAY_USE_TLS` for the daemon.
    ...
    inheritUserEnvironment = lib.mkOption {
      type = lib.types.bool;
      default = cfg.user != "rocky";
      defaultText = lib.literalExpression ''cfg.user != "rocky"'';
      description = ''
        Whether to include the user's profile PATH in the service environment.

        When Rocky runs as a real user (not the default system user), AI agents
    ...
    environment = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          ROCKY_RELAY_ENDPOINT = "relay.rocky.sh:443";
        }
      '';
      description = "Extra environment variables for the Rocky daemon.";
    };
      ...
      example = lib.literalExpression ''
        {
          ...
          log.file = { level = "info"; path = "/var/lib/rocky/daemon.log"; };
        }
      '';
      description = ''
        Declarative content for `$ROCKY_HOME/config.json`. Rendered to JSON
        and installed on every service start.

        Runtime mutations to `config.json` (e.g. via `rocky daemon set-password`
  ...
  config = lib.mkIf cfg.enable (
    let
      settingsFile = (pkgs.formats.json { }).generate "rocky-config.json" cfg.settings;
    in
    {
    assertions = [
      {
        ...
        message = ''
          services.rocky.relay.host must be set when relay.mode = "remote".
        '';
      }
    ];

    users.users.${cfg.user} = lib.mkIf (cfg.user == "rocky") {
      isSystemUser = true;
      group = cfg.group;
      home = cfg.dataDir;
    };

    users.groups.${cfg.group} = lib.mkIf (cfg.group == "rocky") { };

    systemd.tmpfiles.rules = [
      "d ${cfg.dataDir} 0700 ${cfg.user} ${cfg.group} - -"
    ];

    systemd.services.rocky = {
      ...
      environment = {
        NODE_ENV = "production";
        ROCKY_HOME = cfg.dataDir;
        ROCKY_LISTEN = "${cfg.listenAddress}:${toString cfg.port}";
        ...
        ROCKY_HOSTNAMES = "true";
      } // lib.optionalAttrs (lib.isList cfg.hostnames && cfg.hostnames != [ ]) {
        ROCKY_HOSTNAMES = lib.concatStringsSep "," cfg.hostnames;
      } // lib.optionalAttrs (cfg.relay.enable && cfg.relay.mode == "remote") {
        ROCKY_RELAY_ENDPOINT = "${cfg.relay.host}:${toString cfg.relay.port}";
        ROCKY_RELAY_USE_TLS = if cfg.relay.useTls then "true" else "false";
      } // lib.optionalAttrs (cfg.relay.enable && cfg.relay.mode == "remote" && cfg.relay.publicUseTls != null) {
        ROCKY_RELAY_PUBLIC_USE_TLS = if cfg.relay.publicUseTls then "true" else "false";
      } // cfg.environment;
