...
buildNpmPackage rec {
  ...
  installPhase = ''
    ...
    mkdir -p $out/lib/rocky
    node scripts/trace-daemon.mjs > daemon-files.txt

    while IFS= read -r path; do
      [ -z "$path" ] && continue
      mkdir -p "$out/lib/rocky/$(dirname "$path")"
      cp -a "$path" "$out/lib/rocky/$path"
    done < daemon-files.txt

    # Root package.json lets node resolve the workspace layout when the
    # CLI/server bin starts from $out.
    cp package.json $out/lib/rocky/

    # Create wrapper for the server entry point (for systemd / direct use)
    mkdir -p $out/bin
    makeWrapper ${nodejs}/bin/node $out/bin/rocky-server \
      --add-flags "$out/lib/rocky/packages/server/dist/scripts/supervisor-entrypoint.js" \
      --set NODE_ENV production

    # Create wrapper for the CLI
    makeWrapper ${nodejs}/bin/node $out/bin/rocky \
      --add-flags "$out/lib/rocky/packages/cli/dist/index.js" \
      --set NODE_PATH "$out/lib/rocky/node_modules"
