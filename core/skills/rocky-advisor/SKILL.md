...
Read the **rocky** skill. Before choosing a provider, read `~/.rocky/orchestration-preferences.json` unless the user explicitly named a provider in this request. Do not create the advisor until you have read it.
...
- `/rocky-advisor /unslop` → advisor runs `/unslop` on the current diff.
- `/rocky-advisor /unslop-risk` → advisor does an unslop-risk review.
- `/rocky-advisor $diagnose this build failure` → advisor invokes `/diagnose`.
...
Create the advisor agent via Rocky with a `[Advisor] <topic>` title and the briefing as the initial prompt. Wait for it to finish. Read its response. Synthesize for the user — the advisor's verdict + your recommendation.
