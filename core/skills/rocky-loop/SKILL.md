...
Read the **rocky** skill. Before choosing worker or verifier providers, read `~/.rocky/orchestration-preferences.json` unless the user explicitly named providers in this request. Do not start the loop until you have read it.

Loops are a CLI primitive: `rocky loop run`. Manage with `rocky loop ls`, `rocky loop inspect <id>`, `rocky loop logs <id>`, `rocky loop stop <id>`.
