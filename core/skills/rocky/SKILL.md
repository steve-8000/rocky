---
name: rocky
description: Rocky reference for managing agents and worktrees. Load whenever you need to create agents, send them prompts, or manage worktrees.
---

Rocky is a daemon that supervises AI coding agents on your machine. Control it through tools or a CLI.
...
The `rocky` CLI is a thin wrapper over the same daemon. Same surface:

```bash
rocky run --provider codex/gpt-5.4 --mode full-access --worktree feat/x "<prompt>"
rocky send <agent-id> "<follow-up>"
rocky ls
rocky worktree ls
rocky schedule create --cron "*/15 * * * *" "ping main build"
```

Discover with `rocky --help` and `rocky <cmd> --help`.

**If `rocky` isn't on PATH but the desktop app is installed**, the bundled CLI is at:

- macOS: `/Applications/Rocky.app/Contents/Resources/bin/rocky`
- Linux: `<install-dir>/resources/bin/rocky`
- Windows: `C:\Program Files\Rocky\resources\bin\rocky.cmd`

The desktop app's first-run hook (`installCli`) symlinks this to `~/.local/bin/rocky` (macOS/Linux) or drops a `.cmd` trampoline (Windows) and adds `~/.local/bin` to PATH via shell rc files. If that didn't take, offer to symlink it — don't do it silently.
...
|                | Default                                                         |
| -------------- | --------------------------------------------------------------- |
| Listen address | `127.0.0.1:6767` (override `ROCKY_LISTEN`)                      |
| Home           | `~/.rocky` (override `ROCKY_HOME`)                              |
| Daemon log     | `$ROCKY_HOME/daemon.log`                                        |
| Agent state    | `$ROCKY_HOME/agents/<id>.json`                                  |
| Worktrees      | `$ROCKY_HOME/worktrees/` (or `worktrees.root` in `config.json`) |
| PID file       | `$ROCKY_HOME/rocky.pid`                                         |
| Health         | `GET http://127.0.0.1:6767/api/health`                          |

Debug order:

1. `tail -n 200 ~/.rocky/daemon.log`.
2. `rocky daemon status` for liveness.
