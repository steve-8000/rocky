...
Read the **rocky** skill. Before choosing committee members, read `~/.rocky/orchestration-preferences.json` unless the user explicitly named providers in this request. Do not create committee agents until you have read it.
...
Create both agents in parallel via Rocky with `[Committee] <task>` titles and the same prompt. Wait for both — not just whichever finishes first.
