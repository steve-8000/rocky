---
title: Voice
description: Rocky voice architecture, local-first model execution, and provider configuration.
nav: Voice
order: 9
---
...
Rocky has first-class voice support for dictation and realtime conversations with your coding environment.

## Philosophy

Voice is local-first. You can run speech fully on-device, or choose OpenAI for speech features. For voice reasoning/orchestration, Rocky reuses agent providers already installed and authenticated on your machine.
...
Missing models are downloaded at daemon startup into `$ROCKY_HOME/models/local-speech`. Downloads happen only for missing files.
...
```json
{
  ...
  "providers": {
    "local": {
      "modelsDir": "~/.rocky/models/local-speech"
    }
  }
}
```
...
The `language` field applies only to the OpenAI STT provider: set `features.dictation.stt.language` for dictation and `features.voiceMode.stt.language` for realtime voice. If voice language is omitted, Rocky uses the dictation language before falling back to `en`. It has no effect on the local Parakeet models.
...
- `ROCKY_VOICE_LLM_PROVIDER`, voice agent provider override
- `ROCKY_LOCAL_MODELS_DIR`, local model storage directory
- `ROCKY_DICTATION_LOCAL_STT_MODEL`, local dictation STT model ID
- `ROCKY_VOICE_LOCAL_STT_MODEL`, `ROCKY_VOICE_LOCAL_TTS_MODEL`, local voice STT/TTS model IDs
- `ROCKY_DICTATION_LANGUAGE`, dictation STT language (OpenAI STT only; ignored by local Parakeet)
- `ROCKY_VOICE_LANGUAGE`, realtime voice STT language; falls back to `ROCKY_DICTATION_LANGUAGE` when unset (OpenAI STT only; ignored by local Parakeet)
