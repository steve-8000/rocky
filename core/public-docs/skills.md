...
description: "Rocky orchestration skills: teach coding agents to spawn, coordinate, and manage other agents using slash commands."
nav: Skills
order: 8
---
...
- **Manual:** `npx skills add getrocky/rocky`, this installs to `~/.agents/skills/` and sets up symlinks for each agent.

When the desktop app finds installed Rocky skills, it keeps the bundled skills up to date on startup. If automatic update fails, use Settings → Integrations → Update or the manual command above.

## `/rocky`, Rocky Reference

The foundational skill. Rocky reference for managing agents and worktrees. Load it when an agent needs to create agents, send them prompts, or manage worktrees.

Not typically invoked directly by users, it's a reference that other skills depend on.

```
/rocky show me the Rocky CLI surface for creating an agent in a worktree
```

## `/rocky-handoff`, Task Handoff
...
```
/rocky-handoff hand off the auth fix to codex in a worktree
/rocky-handoff hand this to claude opus for review
```

## `/rocky-loop`, Iterative Loops
...
```
/rocky-loop keep trying until the changed test file passes, max 5 iterations
/rocky-loop babysit PR 123 until checks are green, check every 2m, max-time 1h
```

## `/rocky-committee`, Committee Planning
...
```
/rocky-committee why are the websocket connections dropping under load?
/rocky-committee plan the auth system migration
```

## `/rocky-advisor`, Advisor
...
```
/rocky-advisor did I miss anything in this migration plan?
/rocky-advisor --provider claude/opus what is the UX risk in this flow?
```

## `/rocky-epic`, Epic Orchestration
...
The plan file at `~/.rocky/plans/<slug>.md` is the source of truth. Default mode is conversational, with clarification and gates between phases. `--autopilot` runs through delivery without grills or gates. `--worktree` isolates the work in a new Rocky worktree.

```
/rocky-epic build the settings import/export flow end to end
/rocky-epic --autopilot --worktree migrate the relay config UI overnight
```
