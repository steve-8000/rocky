...
import { getStructuredAgentResponse, StructuredAgentResponseError } from "@getrocky/server";
import type { AgentSnapshotPayload } from "@getrocky/protocol/messages";
...
function validateRunOptions(prompt: string, options: AgentRunOptions, outputSchema: unknown): void {
  if (!prompt || prompt.trim().length === 0) {
    throw {
      ...
      details: "Usage: rocky agent run [options] <prompt>",
    } satisfies CommandError;
  }
  ...
  if (options.base && !options.worktree) {
    throw {
      ...
      details: "Usage: rocky agent run --worktree <name> --base <branch> <prompt>",
    } satisfies CommandError;
  }
...
async function connectToDaemonOrThrow(
  ...
  try {
    ...
    throw {
      ...
      details: "Start the daemon with: rocky daemon start",
    } satisfies CommandError;
  }
}
  ...
  try {
    ...
    if (options.thinking !== undefined && !thinkingOptionId) {
      const error: CommandError = {
        ...
        details:
          'Provide a thinking option ID. Use "rocky provider models <provider> --thinking" to list valid IDs.',
      };
      throw error;
    }
