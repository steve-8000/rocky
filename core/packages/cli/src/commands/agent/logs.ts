...
import type { AgentStreamMessage } from "@getrocky/protocol/messages";
import { curateAgentActivity } from "@getrocky/server";
...
export async function runLogsCommand(
  ...
  if (!id) {
    ...
    console.error("Usage: rocky agent logs <id>");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Start the daemon with: rocky daemon start");
    process.exit(1);
  }
  ...
  try {
    ...
    if (!fetchResult) {
      ...
      console.error("Use `rocky ls` to list available agents");
      await client.close();
      process.exit(1);
    }
