...
export async function runArchiveCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: rocky agent archive <id-or-name>",
    };
    throw error;
  }
  ...
  try {
    ...
    if (!agentId) {
      const error: CommandError = {
        ...
        details: 'Use "rocky ls" to list available agents',
      };
      throw error;
    }
    ...
    if (agent.status === "running" && !options.force) {
      const error: CommandError = {
        ...
        details:
          "Use --force to archive a running agent (it will interrupt the active run), or stop it first with: rocky agent stop. Use rocky agent delete to hard-delete it.",
      };
      throw error;
    }
