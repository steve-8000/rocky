 ...
 * - `rocky agent ls`    → active non-archived agents
 * - `rocky agent ls -a` → include archived agents
 */
export async function runLsCommand(
  ...
  try {
    ...
    const error: CommandError = {
      ...
      details:
        "Start the daemon with: rocky daemon start\nFor a remote daemon, pass --host <host:port> or set ROCKY_HOST.",
    };
    throw error;
  }
