...
import { resolveLocalRockyHome } from "./local-daemon.js";
...
export async function setDaemonPasswordInConfig(
  ...
  const configPath = path.join(rockyHome, CONFIG_FILENAME);
  ...
  savePersistedConfig(rockyHome, nextConfig);

  return {
    action: "password_set",
    configPath,
    restartCommand: "rocky daemon restart",
    message: `Password written to ${configPath}\nRestart the daemon for the change to take effect.\nRun: rocky daemon restart`,
  };
}
