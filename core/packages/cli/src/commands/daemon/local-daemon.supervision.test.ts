...
class FakeDaemonRuntime implements DaemonLaunchRuntime {
  ...
  resolveHome(env: NodeJS.ProcessEnv): string {
    return env.ROCKY_HOME ?? "/tmp/rocky";
  }
...
async function createRockyHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "rocky-local-daemon-"));
  tempRoots.push(root);
...
describe("local daemon launch supervision", () => {
  ...
  test("foreground start spawns supervisor-entrypoint instead of server/index", async () => {
    ...
    const status = startLocalDaemonForeground({ home: "/tmp/rocky-test", relay: false }, runtime);
  ...
  test("detached start spawns supervisor-entrypoint instead of server/index", async () => {
    ...
    const resultPromise = startLocalDaemonDetached(
      { home: "/tmp/rocky-test", mcp: false },
      runtime,
    );
    await vi.advanceTimersByTimeAsync(1200);
    const result = await resultPromise;

    expect(result).toEqual({ pid: 4242, logPath: "/tmp/rocky-test/daemon.log" });
  ...
  test("relay TLS flag is passed to the supervised daemon", async () => {
    ...
    expect(launch?.options?.env?.ROCKY_RELAY_USE_TLS).toBe("true");
  });
  ...
  test("local daemon state keeps public relay TLS separate from daemon relay TLS", async () => {
    const home = await createRockyHome({
      ...
      daemon: {
        relay: {
          ...
          publicEndpoint: "rocky.example.com",
          useTls: false,
          publicUseTls: true,
        },
      },
    ...
    expect(state.relayEndpoint).toBe("rocky.example.com");
