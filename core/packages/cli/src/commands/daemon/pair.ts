...
import { generateLocalPairingOffer, loadConfig, resolveRockyHome } from "@getrocky/server";
...
export async function runPairCommand(options: PairOptions): Promise<void> {
  if (options.home) {
    process.env.ROCKY_HOME = options.home;
  }

  const rockyHome = resolveRockyHome();
  const state = resolveLocalDaemonState({ home: rockyHome });
