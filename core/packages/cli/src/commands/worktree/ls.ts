...
export function resolveRockyHomePath(): string {
  return process.env.ROCKY_HOME ?? join(homedir(), ".rocky");
}

export function resolveRockyWorktreesDir(): string {
  return join(resolveRockyHomePath(), "worktrees");
}

function isAgentInManagedWorktree(agentCwd: string): boolean {
  const worktreesDir = resolveRockyWorktreesDir();
...
export async function runLsCommand(
  ...
  try {
    ...
    const response = await client.getRockyWorktreeList({});
