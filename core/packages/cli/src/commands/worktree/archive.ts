...
export async function runArchiveCommand(
  ...
  if (!nameArg || nameArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: rocky worktree archive <name>",
    };
    throw error;
  }
  ...
  try {
    ...
    const listResponse = await client.getRockyWorktreeList({});
    ...
    if (!worktree) {
      const error: CommandError = {
        ...
        details: 'Use "rocky worktree ls" to list available worktrees',
      };
      throw error;
    }

    // Archive the worktree
    const response = await client.archiveRockyWorktree({
