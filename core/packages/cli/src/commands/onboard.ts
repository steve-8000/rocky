...
} from "@getrocky/server";
import {
  resolveLocalRockyHome,
...
function savePersistedConfig(rockyHome: string, config: OnboardPersistedConfig): void {
  const configPath = path.join(rockyHome, "config.json");
...
function printNextSteps(pairingUrl: string | null, rockyHome: string, richUi: boolean): void {
  const daemonLogPath = path.join(rockyHome, "daemon.log");
  const nextStepsLines = [
    pairingUrl
      ? "1. Open Rocky and scan the QR code above, or paste the pairing link."
      : "1. Open Rocky and connect to your daemon.",
    "2. Web app: https://rocky.clab.one",
    "3. Desktop app: https://github.com/getrocky/rocky/releases/latest",
    "4. Docs: https://rocky.clab.one/docs",
    '5. Example: rocky run --output-schema schema.json "extract fields"',
  ];
  const quickReferenceLines = [
    "1. rocky --help",
    "2. rocky ls",
    '3. rocky run "your prompt"',
    "4. rocky status",
...
export function onboardCommand(): Command {
  return new Command("onboard")
    ...
    .option("--port <port>", "Port to listen on (default: 7767)")
    .option("--home <path>", "Rocky home directory (default: ~/.rocky)")
...
async function resolveAndPersistVoice(
  ...
  let persisted = loadPersistedConfig(rockyHome) as OnboardPersistedConfig;
  ...
  savePersistedConfig(rockyHome, persisted);
  return voiceEnabled;
}
...
export async function runOnboard(options: OnboardOptions): Promise<void> {
  ...
  const rockyHome = resolveLocalRockyHome(options.home);
  if (richUi) {
    renderNote(rockyHome, "Rocky home");
  }

  const voiceEnabled = await resolveAndPersistVoice(rockyHome, options);
  const config = loadConfig(rockyHome, { cli: toCliOverrides(options) });
  ...
  if (config.relayEnabled === false) {
    ...
    printNextSteps(null, rockyHome, richUi);
    if (richUi) {
      outro("Rocky daemon is running.");
    }
    return;
  }
  ...
  printNextSteps(pairing.url, rockyHome, richUi);
  if (richUi) {
    outro("Rocky is ready!");
  }
}
