...
import { spawnProcess } from "@getrocky/server";

function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Rocky.app",
      path.join(homedir(), "Applications", "Rocky.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Rocky",
      "/opt/Rocky/Rocky",
      path.join(homedir(), "Applications", "Rocky.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Rocky", "Rocky.exe");
...
export async function openDesktopWithProject(projectPath: string): Promise<void> {
  try {
    if (process.env.ROCKY_DESKTOP_CLI === "1") {
      throw new Error(
    ...
    if (!desktopApp) {
      throw new Error(
        "Rocky desktop app not found. Install it from https://github.com/getrocky/rocky/releases",
      );
    }
