...
describe("classifyInvocation", () => {
  ...
  it("classifies '.' as an open-project invocation", () => {
    const projectDir = mkdtempSync(path.join(tmpdir(), "rocky-classify-dot-"));
  ...
  it("classifies '..' as an open-project invocation", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "rocky-classify-parent-"));
  ...
  it("classifies './myproject' as an open-project invocation", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "rocky-classify-relative-"));
  ...
  it("classifies an absolute path as an open-project invocation", () => {
    const projectDir = mkdtempSync(path.join(tmpdir(), "rocky-classify-absolute-"));
  ...
  it("classifies a home-relative path as an open-project invocation", () => {
    const projectDir = mkdtempSync(path.join(homedir(), "rocky-classify-home-"));
  ...
  it("classifies an existing directory name as an open-project invocation", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "rocky-classify-existing-"));
    const projectDir = path.join(parentDir, "myproject");
    mkdirSync(projectDir);
  ...
  it("keeps known commands in CLI mode even when a matching directory exists", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "rocky-classify-command-"));
...
describe("path helpers", () => {
  ...
  it("detects existing directories relative to cwd", () => {
    const parentDir = mkdtempSync(path.join(tmpdir(), "rocky-classify-helper-"));
