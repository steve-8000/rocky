...
import { loadConfig, resolveRockyHome } from "@getrocky/server";
import {
...
import { DaemonClient, type WebSocketLike } from "@getrocky/client/internal/daemon-client";
...
const DEFAULT_HOST = "localhost:7767";
const DEFAULT_TIMEOUT = 15000;
const PID_FILENAME = "rocky.pid";
...
function readPidSocketTarget(rockyHome: string): string | null {
  const pidPath = path.join(rockyHome, PID_FILENAME);
...
function resolveConfiguredIpcDaemonHost(env: NodeJS.ProcessEnv, rockyHome: string): string | null {
  const directEnvHost = normalizeDaemonHost(env.ROCKY_LISTEN ?? "");
  if (isIpcDaemonHost(directEnvHost)) {
    return directEnvHost;
  }

  const pidHost = normalizeDaemonHost(readPidSocketTarget(rockyHome) ?? "");
...
function resolveConfiguredTcpDaemonHost(env: NodeJS.ProcessEnv, rockyHome: string): string | null {
  const configuredHost = normalizeDaemonHost(loadConfig(rockyHome, { env }).listen);
  if (!isTcpDaemonHost(configuredHost)) {
    return null;
  }
  return configuredHost === "127.0.0.1:7767" ? null : configuredHost;
}

export function resolveDefaultDaemonHosts(env: NodeJS.ProcessEnv = process.env): string[] {
  const rockyHome = resolveRockyHome(env);
  const candidates: string[] = [];
  const configuredIpcHost = resolveConfiguredIpcDaemonHost(env, rockyHome);
  if (configuredIpcHost) {
    candidates.push(configuredIpcHost);
  }
  const configuredTcpHost = resolveConfiguredTcpDaemonHost(env, rockyHome);
...
function resolveDaemonHostCandidates(options?: ConnectOptions): string[] {
  const explicitHost = options?.host ?? process.env.ROCKY_HOST;
  if (explicitHost) {
    return [explicitHost];
  }
...
export function resolveDaemonPassword(host: string): string | undefined {
  ...
  const fromEnv = process.env.ROCKY_PASSWORD;
...
export async function connectToDaemon(options?: ConnectOptions): Promise<DaemonClient> {
  ...
  async function tryNext(index: number, lastError: unknown): Promise<DaemonClient> {
    if (index >= hosts.length) {
      ...
      throw new Error(`Unable to connect to Rocky daemon via ${hosts.join(", ")}`);
    }
