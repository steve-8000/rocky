...
/**
 * Test runner for Rocky CLI E2E tests
 *
 ...
 * its own daemon (ephemeral port + tmp ROCKY_HOME), so parallelism is safe.
 */
...
// packages live in the root. Prepend it so `npx rocky` resolves locally.
...
const concurrencyEnv = process.env.ROCKY_CLI_TEST_CONCURRENCY;
...
const shardTotal = parsePositiveInt(process.env.ROCKY_CLI_TEST_SHARD_TOTAL, 1);
const shardIndexRaw = parsePositiveInt(process.env.ROCKY_CLI_TEST_SHARD, 1);
if (shardIndexRaw < 1 || shardIndexRaw > shardTotal) {
  throw new Error(
    `ROCKY_CLI_TEST_SHARD=${shardIndexRaw} out of range for SHARD_TOTAL=${shardTotal}`,
  );
}
...
async function writeJsonSummary({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        suite: "cli-local",
        command: "npm run test:local --workspace=@getrocky/cli",
        counts: {
          passed,
          failed,
          skipped: 0,
...
console.log("🧪 Rocky CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
async function runSingleTest(testFile: string): Promise<TestOutcome> {
  ...
  const npmCache = await mkdtemp(join(tmpdir(), "rocky-cli-test-npm-cache-"));
    ...
    return await new Promise<TestOutcome>((resolve) => {
      const proc = spawn("npx", ["tsx", testPath], {
        env: {
          ...
          ROCKY_LOCAL_SPEECH_AUTO_DOWNLOAD: testEnvDefaults.ROCKY_LOCAL_SPEECH_AUTO_DOWNLOAD,
          ROCKY_DICTATION_ENABLED: testEnvDefaults.ROCKY_DICTATION_ENABLED,
          ROCKY_VOICE_MODE_ENABLED: testEnvDefaults.ROCKY_VOICE_MODE_ENABLED,
        },
