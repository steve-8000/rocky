...
try {
  // Test 1: stop --help shows options
  {
    ...
    const result = await $`npx rocky stop --help`.nothrow();
  ...
  // Test 2: stop requires ID, --all, or --cwd
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky stop`.nothrow();
  ...
  // Test 3: stop handles daemon not running
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky stop abc123`.nothrow();
  ...
  // Test 4: stop --all flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky stop --all`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: stop --cwd flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky stop --cwd /tmp`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: stop with ID and --host flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky stop abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: rocky --help shows stop command
  {
    console.log("Test 7: rocky --help shows stop command");
    ...
    console.log("✓ rocky --help shows stop command\n");
  }
  ...
  // Test 8: -q (quiet) flag is accepted with stop
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky -q stop abc123`.nothrow();
    const output = result.stdout + result.stderr;
