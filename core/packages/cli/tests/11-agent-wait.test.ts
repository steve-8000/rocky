...
try {
  // Test 1: wait --help shows options
  {
    ...
    const result = await $`npx rocky wait --help`.nothrow();
  ...
  // Test 2: wait requires id argument
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky wait`.nothrow();
  ...
  // Test 3: wait handles daemon not running
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky wait abc123`.nothrow();
  ...
  // Test 4: wait --timeout flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOME=${rockyHome} npx rocky wait --timeout 30 --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: wait --host flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOME=${rockyHome} npx rocky wait --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: -q (quiet) flag is accepted with wait
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky -q wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --json flag is accepted with wait
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky wait abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: --format yaml flag is accepted with wait
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky --format yaml wait abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 9: rocky --help shows wait command
  {
    console.log("Test 9: rocky --help shows wait command");
    ...
    console.log("rocky --help shows wait command\n");
  }
