...

{
  ...
  assert.strictEqual(resolveTcpHostFromListen("7767"), "127.0.0.1:7767");
...

{
  ...
  assert.strictEqual(resolveTcpHostFromListen("localhost:7767"), "localhost:7767");
...

{
  ...
  assert.strictEqual(resolveTcpHostFromListen("/tmp/rocky.sock"), null);
  assert.strictEqual(resolveTcpHostFromListen("unix:///tmp/rocky.sock"), null);
  assert.strictEqual(resolveTcpHostFromListen("pipe://\\\\.\\pipe\\rocky-managed-test"), null);
  assert.strictEqual(resolveTcpHostFromListen("\\\\.\\pipe\\rocky-managed-test"), null);
...

{
  ...
  assert.strictEqual(resolveTcpHostFromListen("C:\\Users\\foo\\.rocky\\rocky.sock"), null);
  assert.strictEqual(resolveTcpHostFromListen("D:\\project\\socket"), null);
  assert.strictEqual(resolveTcpHostFromListen("C:\\rocky.sock"), null);
