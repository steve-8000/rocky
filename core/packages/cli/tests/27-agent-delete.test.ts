...
const rockyHome = await mkdtemp(join(tmpdir(), "rocky-delete-test-home-"));
...
async function runDelete(args: string[]) {
  return $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npm --prefix ${repoRoot} run cli -- delete ${args}`.nothrow();
}
...
try {
  ...
  {
    console.log("Test 7: rocky --help shows delete command");
    ...
    console.log("✓ rocky --help shows delete command\n");
  }

  {
    console.log("Test 8: -q (quiet) flag is accepted with delete");
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npm --prefix ${repoRoot} run cli -- -q delete abc123`.nothrow();
    const output = result.stdout + result.stderr;
