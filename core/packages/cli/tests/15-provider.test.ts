...
async function runProviderModelsJson(provider: string): Promise<ProviderModel[]> {
  ...
  async function attemptRun(attempt: number): Promise<ProviderModel[]> {
    const result = await ctx.rocky(["provider", "models", provider, "--json"]);
...
try {
  // Test 1: provider --help shows subcommands
  {
    ...
    const result = await ctx.rocky(["provider", "--help"]);
  ...
  // Test 2: provider ls lists all providers
  {
    ...
    const result = await ctx.rocky(["provider", "ls"]);
  ...
  // Test 3: provider ls --json outputs valid JSON
  {
    ...
    const result = await ctx.rocky(["provider", "ls", "--json"]);
  ...
  // Test 4: provider ls includes disabled providers
  {
    ...
    const disabledCtx = await startTestDaemon({ rockyHome, workDir, timeout: 120000 });
    try {
      const result = await runRockyCli(disabledCtx, ["provider", "ls", "--json"]);
      assert.strictEqual(result.exitCode, 0, "provider ls should exit 0");
      ...
      const modelsResult = await runRockyCli(disabledCtx, ["provider", "models", "claude"]);
  ...
  // Test 5: provider ls --quiet outputs provider names only
  {
    ...
    const result = await ctx.rocky(["provider", "ls", "--quiet"]);
    assert.strictEqual(result.exitCode, 0, "should exit 0");
  ...
  // Test 9: provider models unknown fails with error
  {
    ...
    const result = await ctx.rocky(["provider", "models", "unknown"]);
  ...
  // Test 11: provider models --quiet outputs model IDs only
  {
    ...
    const result = await ctx.rocky(["provider", "models", "claude", "--quiet"]);
    assert.strictEqual(result.exitCode, 0, "should exit 0");
    const lines = result.stdout.trim().split("\n").filter(Boolean);
    assert.strictEqual(
      lines.length,
