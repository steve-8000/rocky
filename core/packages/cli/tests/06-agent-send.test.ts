...
const promptFilePath = join(rockyHome, "send-prompt.txt");
...
try {
  // Test 1: send --help shows options
  {
    ...
    const result = await $`npx rocky send --help`.nothrow();
  ...
  // Test 2: send requires id argument
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky send`.nothrow();
  ...
  // Test 3: send requires prompt argument
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky send abc123`.nothrow();
  ...
  // Test 4: send handles daemon not running
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky send abc123 "test prompt"`.nothrow();
  ...
  // Test 5: send --no-wait flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky send --no-wait abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5b: send --prompt flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky send --prompt "test prompt" abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5c: send --prompt-file flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky send --prompt-file ${promptFilePath} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: send --host flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOME=${rockyHome} npx rocky send --host localhost:${port} abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: -q (quiet) flag is accepted with send
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky -q send --no-wait abc123 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: Combined flags work together
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky -q send --no-wait abc123 "Run the linter"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8b: conflicting prompt sources are rejected
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky send abc123 "positional prompt" --prompt "flag prompt"`.nothrow();
  ...
  // Test 9: rocky --help shows send command
  {
    console.log("Test 9: rocky --help shows send command");
    ...
    console.log("✓ rocky --help shows send command\n");
  }
