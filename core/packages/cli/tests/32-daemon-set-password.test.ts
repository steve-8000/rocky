...
import { isBearerTokenValid } from "@getrocky/server";
import {
...
const root = await mkdtemp(join(tmpdir(), "rocky-set-password-"));
const rockyHome = join(root, ".rocky");
...
try {
  {
      ...
      `${JSON.stringify(
        {
          ...
          app: { baseUrl: "https://rocky.clab.one" },
        },
        null,
        2,
      )}\n`,
    ...
    const result = await setDaemonPasswordInConfig("shared-secret", { home: rockyHome });
    const config = JSON.parse(await readFile(join(rockyHome, "config.json"), "utf-8"));

    assert.strictEqual(result.configPath, join(rockyHome, "config.json"));
    assert.strictEqual(result.restartCommand, "rocky daemon restart");
