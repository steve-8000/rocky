...
try {
...

  {
    ...
    const posted = await ctx.rocky(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { ROCKY_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.rocky(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.rocky(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    // always wakes it up.
    const waitPromise = ctx.rocky(["chat", "wait", "coord-room", "--timeout", "30s"]);
    const waitSentinel = waitPromise.then(() => "settled" as const);
    let postedSecond = false;
    for (let attempt = 0; attempt < 60; attempt++) {
      const post = await ctx.rocky(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.rocky(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.rocky(["chat", "delete", "coord-room"]);
  ...
  await ctx.stop();
  await rm(ctx.rockyHome, { recursive: true, force: true });
