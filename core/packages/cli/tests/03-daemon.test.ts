 ...
 *
 * Tests daemon commands with an isolated ROCKY_HOME.
 *
 * Tests:
...
// Keep restart off default 7767 to avoid collisions with any existing daemon.
const port = 10000 + Math.floor(Math.random() * 50000);
const rockyHome = await mkdtemp(join(tmpdir(), "rocky-test-home-"));

function daemonCommand(args: string[]) {
  return runLocalRocky(["daemon", ...args], { ROCKY_HOME: rockyHome });
}
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalRocky(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, rockyHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
