...
import { buildRelayWebSocketUrl } from "@getrocky/protocol/daemon-endpoints";
import { parseConnectionOfferFromUrl } from "@getrocky/protocol/connection-offer";
import { generateLocalPairingOffer } from "@getrocky/server";
import { DaemonClient } from "@getrocky/client/internal/daemon-client";
...
(shouldRunRelayE2e ? describe : describe.skip)("CLI --host offer URL via relay", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createE2ETestContext({
      timeout: 60_000,
      env: {
        ROCKY_RELAY_ENABLED: "true",
        ROCKY_RELAY_ENDPOINT: relayEndpoint,
        ROCKY_RELAY_PUBLIC_ENDPOINT: relayEndpoint,
      },
    });

    const offer = await generateLocalPairingOffer({
      rockyHome: ctx.rockyHome,
      relayEnabled: true,
      relayEndpoint,
  ...
  it("runs `rocky --host <offer-url> ls` over the relay and matches direct ls output", async () => {
    if (!ctx) throw new Error("test context not initialized");

    const direct = await ctx.rocky(["ls", "--json"]);
    ...
    const relay = await ctx.rocky(["ls", "--json", "--host", offerUrl], {
      timeout: 30_000,
      env: { ROCKY_HOST: offerUrl },
    });
