 ...
 * 2. Create agent with `rocky run "say hello" --provider claude`
 * 3. List agents with `rocky ls`
 * 4. Wait for agent with `rocky wait <id>`
 * 5. Inspect agent with `rocky inspect <id>`
 * 6. Stop agent with `rocky stop <id>` and verify it remains inspectable
 * 7. Delete agent with `rocky delete <id>`
 * 8. Cleanup: stop daemon, remove temp dirs
 *
 * CRITICAL RULES:
 * - NEVER use port 7767 (user's running daemon)
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`ROCKY_HOME: ${ctx.rockyHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.rocky(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.rocky(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.rocky(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.rocky(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.rocky(["stop", agentId]);
  ...
  const inspectResult = await ctx.rocky(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.rocky(["delete", agentId]);
