...
const schemaPath = join(rockyHome, "run-output-schema.json");
await writeFile(
  schemaPath,
  JSON.stringify({
    type: "object",
...
try {
  // Test 1: run --help shows options
  {
    ...
    const result = await $`npx rocky run --help`.nothrow();
  ...
  // Test 2: run requires prompt argument
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run`.nothrow();
  ...
  // Test 3: run handles daemon not running
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run --provider claude "test prompt"`.nothrow();
  ...
  // Test 4: run -d flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run -d "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: run --name flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run --name "test-agent" "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: run --provider flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run --provider codex "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6b: run --provider provider/model syntax is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run --provider codex/gpt-5.4 "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: run --mode flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run --mode bypass "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: run --cwd flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run --cwd /tmp "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 9: run --output-schema flag is accepted
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run --output-schema ${schemaPath} "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 10: run --output-schema cannot be used with --detach
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run -d --output-schema ${schemaPath} "test prompt"`.nothrow();
  ...
  // Test 11: -q (quiet) flag is accepted with run
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky -q run -d "test prompt"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 12: Combined flags work together
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky -q run -d --name "test-fixer" --provider claude --mode bypass --cwd /tmp "Fix the tests"`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 12b: conflicting provider/model syntax is rejected before connect
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run --provider codex/gpt-5.4 --model gpt-5.5 "test prompt"`.nothrow();
  ...
  // Test 13: rocky --help shows run command
  {
    console.log("Test 13: rocky --help shows run command");
    const result = await $`npx rocky --help`.nothrow();
    ...
    console.log("✓ rocky --help shows run command\n");
  }
  ...
  // Test 14: run --ui is rejected (flag removed)
  {
    ...
    const result =
      await $`ROCKY_HOST=localhost:${port} ROCKY_HOME=${rockyHome} npx rocky run --ui "test prompt"`.nothrow();
