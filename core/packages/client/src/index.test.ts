...
import { createRockyClient } from "./index.js";
import type { RockyAgent, RockyClient, RockyProviderConfig, RockyWorkspace } from "./index.js";
...
async function connectClient(): Promise<{ client: RockyClient; ws: FakeWebSocket }> {
  vi.stubGlobal("WebSocket", FakeWebSocket);
  const client = createRockyClient({
    url: "ws://daemon.test",
  ...
  expect(hello.clientId).toEqual(expect.stringMatching(/^rocky-sdk-/));
  ws.message(
    sessionMessage({
      type: "status",
      payload: {
...
function createWorkspace(input: Partial<RockyWorkspace> = {}): RockyWorkspace {
  return {
    id: "workspace_sdk",
...
function createAgent(input: Partial<RockyAgent> = {}): RockyAgent {
  return {
    id: "agent_sdk",
    provider: "codex",
    cwd: "/repo/sdk",
...
test("createRockyClient exposes workspace list through the daemon client", async () => {
