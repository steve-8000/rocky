...
import { CLIENT_CAPS } from "@getrocky/protocol/client-capabilities";
import {
...
import type {
  ...
  RockyWorktreeListResponse,
  RockyWorktreeArchiveResponse,
  ProjectIconResponse,
...
import type {
  ...
  AgentProvider,
  AgentSessionConfig,
} from "@getrocky/protocol/agent-types";
import type { MutableDaemonConfig, MutableDaemonConfigPatch } from "@getrocky/protocol/messages";
import { isRelayClientWebSocketUrl } from "@getrocky/protocol/daemon-endpoints";
import {
  asUint8Array,
...
export interface CreateRockyWorktreeInput extends Pick<
  CreateRockyWorktreeRequest,
  | "cwd"
  | "projectId"
  | "worktreeSlug"
...
type RockyWorktreeListPayload = RockyWorktreeListResponse["payload"];
type RockyWorktreeArchivePayload = RockyWorktreeArchiveResponse["payload"];
type CreateRockyWorktreePayload = Extract<
  SessionOutboundMessage,
...
export class DaemonClient {
  ...
  private attemptConnect(): void {
    ...
    const protocols = password ? [`rocky.bearer.${password}`] : undefined;
  ...
  async getRockyWorktreeList(
    input: { cwd?: string; repoRoot?: string },
    requestId?: string,
  ): Promise<RockyWorktreeListPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "rocky_worktree_list_response",
      timeout: 60000,
    });
  }

  async archiveRockyWorktree(
    input: { worktreePath?: string; repoRoot?: string; branchName?: string },
    requestId?: string,
  ): Promise<RockyWorktreeArchivePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "rocky_worktree_archive_response",
      timeout: 60000,
    });
  }

  async createRockyWorktree(
    input: CreateRockyWorktreeInput,
    requestId?: string,
  ): Promise<CreateRockyWorktreePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "create_rocky_worktree_response",
      timeout: 60000,
    });
  }
