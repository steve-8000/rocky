...
} from "@getrocky/protocol/terminal-stream-protocol";
...
test("passes password as HTTP bearer header and WebSocket subprotocol", async () => {
  ...
  expect(transportFactory).toHaveBeenCalledWith({
    ...
    protocols: ["rocky.bearer.shared-secret"],
  });
});
  ...
  mock.triggerMessage(
    wrapSessionMessage({
      ...
      payload: {
        ...
        detail: {
          type: "worktree_setup",
          worktreePath: "/tmp/project/.rocky/worktrees/feature-a",
          ...
          commands: [
            {
              index: 1,
              command: "npm install",
              cwd: "/tmp/project/.rocky/worktrees/feature-a",
              log: "phase-one\n",
              status: "running",
              exitCode: null,
            },
...
test("sends structured first-agent context attachments with create_rocky_worktree_request", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const createPromise = client.createRockyWorktree({
    cwd: "/tmp/project",
    worktreeSlug: "review-pr-123",
    firstAgentContext: {
      attachments: [
...
test("sends worktree base-ref fields in create_rocky_worktree_request", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
...
test("omitting create_rocky_worktree_request worktree base-ref fields preserves legacy wire shape", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  mock.triggerMessage(
    JSON.stringify({
      ...
      message: {
        ...
        payload: {
          directories: ["/Users/test/projects/rocky"],
