...
const originalDesktopCli = process.env.ROCKY_DESKTOP_CLI;
...
describe("passthrough CLI", () => {
  afterEach(() => {
    ...
    if (originalDesktopCli === undefined) {
      delete process.env.ROCKY_DESKTOP_CLI;
    } else {
      process.env.ROCKY_DESKTOP_CLI = originalDesktopCli;
    }
  });
  ...
  it("returns null when no CLI args are provided", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Rocky.app/Contents/MacOS/Rocky"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores macOS GUI launch arguments", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Rocky.app/Contents/MacOS/Rocky", "-psn_0_12345"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores --no-sandbox injected by Linux wrapper", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Rocky", "--no-sandbox", "status"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["status"]);
  ...
  it("passes --open-project through as a normal CLI arg", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Rocky.app/Contents/MacOS/Rocky", "--open-project", "/tmp/project"],
        isDefaultApp: false,
        forceCli: false,
      }),
