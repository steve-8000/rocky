...
import { resolveRockyHome, spawnProcess } from "@getrocky/server";
import {
...
function getRockyHome(): string {
  return resolveRockyHome(process.env);
}

function logFilePath(): string {
  return path.join(getRockyHome(), DAEMON_LOG_FILENAME);
}

export function isDesktopManagedDaemonRunningSync(): boolean {
  try {
    const raw = readFileSync(path.join(getRockyHome(), "rocky.pid"), "utf-8");
...
async function startDaemon(): Promise<DesktopDaemonStatus> {
  ...
  const child: ChildProcess = spawnProcess(invocation.command, invocation.args, {
    ...
    envMode: "internal",
    env: invocation.env,
    envOverlay: { ROCKY_DESKTOP_MANAGED: "1" },
