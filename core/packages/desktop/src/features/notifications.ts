...
export function registerNotificationHandlers(): void {
  ipcMain.handle("rocky:notification:isSupported", () => {
    return Notification.isSupported();
  });

  ipcMain.handle("rocky:notification:send", async (event, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        win.webContents.send("rocky:event:notification-click", payload);
      }
