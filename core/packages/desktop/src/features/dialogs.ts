...
export function registerDialogHandlers(): void {
  ipcMain.handle("rocky:dialog:ask", async (event, message: string, options?: AskOptions) => {
  ...
  ipcMain.handle("rocky:dialog:open", async (event, options?: OpenOptions) => {
    const win = BrowserWindow.fromWebContents(event.sender);
