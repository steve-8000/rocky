...
export function registerWindowManager(): void {
  ...
  ipcMain.handle("rocky:window:toggleMaximize", (event) => {
  ...
  ipcMain.handle("rocky:window:isFullscreen", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    return win?.isFullScreen() ?? false;
  });

  ipcMain.handle("rocky:window:setBadgeCount", (_event, count?: unknown) => {
  ...
  ipcMain.handle("rocky:window:updateWindowControls", (event, update?: unknown) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) {
      return;
    }
...
export function setupWindowResizeEvents(win: BrowserWindow): void {
  win.on("resize", () => {
    win.webContents.send("rocky:window:resized", {});
  });
