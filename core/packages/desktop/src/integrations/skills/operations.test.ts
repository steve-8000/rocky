...
vi.mock("electron", () => ({
  app: {
    getPath: vi.fn(() => "/tmp/rocky-user-data"),
    isPackaged: false,
  },
}));
...
async function makeSandbox(): Promise<Sandbox> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "rocky-skills-"));
...
async function writeCurrentBundle(sourceDir: string): Promise<void> {
  await writeBundleSkill(sourceDir, "rocky", { "SKILL.md": "rocky-v1" });
  await writeBundleSkill(sourceDir, "rocky-loop", { "SKILL.md": "loop-v1" });
}
...
describe("getSkillsStatus", () => {
  ...
  it("returns up-to-date when every bundled skill matches on disk", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await writeOnDiskSkillToAllTargets(sandbox.targets, "rocky", { "SKILL.md": "rocky-v1" });
    await writeOnDiskSkillToAllTargets(sandbox.targets, "rocky-loop", { "SKILL.md": "loop-v1" });
  ...
  it("ignores user-added files inside current managed skill dirs in every target", async () => {
    ...
    await writeOnDiskSkillToAllTargets(sandbox.targets, "rocky", { "SKILL.md": "rocky-v1" });
    await writeOnDiskSkillToAllTargets(sandbox.targets, "rocky-loop", { "SKILL.md": "loop-v1" });
    await writeOnDiskSkill(sandbox.targets.agentsDir, "rocky", {
      "SKILL.md": "rocky-v1",
      "my-context.md": "user context",
    });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "rocky", {
      "SKILL.md": "rocky-v1",
      "commands/local.md": "user command",
    });
    await writeOnDiskSkill(sandbox.targets.codexDir, "rocky", {
      "SKILL.md": "rocky-v1",
  ...
  it("returns drift with a single update op when one bundled file diverges", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await writeOnDiskSkill(sandbox.targets.agentsDir, "rocky", { "SKILL.md": "stale" });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "rocky", { "SKILL.md": "rocky-v1" });
    await writeOnDiskSkill(sandbox.targets.codexDir, "rocky", { "SKILL.md": "rocky-v1" });
    await writeOnDiskSkillToAllTargets(sandbox.targets, "rocky-loop", { "SKILL.md": "loop-v1" });

    const status = await getSkillsStatus(sandbox.targets);

    expect(status.state).toBe("drift");
    expect(status.ops).toEqual([{ kind: "update", name: "rocky" }]);
  });

  it("returns drift when a secondary agent target is stale", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await writeOnDiskSkill(sandbox.targets.agentsDir, "rocky", { "SKILL.md": "rocky-v1" });
    await writeOnDiskSkill(sandbox.targets.agentsDir, "rocky-loop", { "SKILL.md": "loop-v1" });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "rocky", { "SKILL.md": "stale" });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "rocky-loop", { "SKILL.md": "loop-v1" });
    await writeOnDiskSkill(sandbox.targets.codexDir, "rocky", { "SKILL.md": "rocky-v1" });
    await writeOnDiskSkill(sandbox.targets.codexDir, "rocky-loop", { "SKILL.md": "loop-v1" });
  ...
  it("returns drift with add ops for the bundled skills missing from disk", async () => {
    ...
    expect(status.state).toBe("drift");
    expect(status.ops).toEqual([{ kind: "add", name: "rocky-loop" }]);
  });
  ...
  it("returns drift with a delete op for a legacy skill name still on disk", async () => {
    ...
    await writeOnDiskSkillToAllTargets(sandbox.targets, "rocky", { "SKILL.md": "rocky-v1" });
    await writeOnDiskSkillToAllTargets(sandbox.targets, "rocky-loop", { "SKILL.md": "loop-v1" });
    await writeOnDiskSkill(sandbox.targets.agentsDir, "rocky-chat", { "SKILL.md": "chat-old" });

    const status = await getSkillsStatus(sandbox.targets);

    expect(status.state).toBe("drift");
    expect(status.ops).toEqual([{ kind: "delete", name: "rocky-chat" }]);
  });
  ...
  it("emits add + update + delete ops sorted by name when state is mixed", async () => {
    ...
    expect(status.state).toBe("drift");
    expect(status.ops).toEqual([
      { kind: "update", name: "rocky" },
      { kind: "delete", name: "rocky-chat" },
      { kind: "add", name: "rocky-loop" },
    ]);
  });
});
...
describe("installSkills / updateSkills", () => {
  ...
  it("converges to up-to-date when state has missing + edited + legacy skills", async () => {
    ...
    await writeOnDiskSkill(sandbox.targets.agentsDir, "rocky", { "SKILL.md": "stale" });
    await writeOnDiskSkill(sandbox.targets.agentsDir, "rocky-chat", { "SKILL.md": "chat-old" });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "rocky-chat", { "SKILL.md": "chat-old" });
    await writeOnDiskSkill(sandbox.targets.codexDir, "rocky-chat", { "SKILL.md": "chat-old" });

    const status = await updateSkills(sandbox.targets);

    expect(status).toEqual({ state: "up-to-date", ops: [] });
    expect(
      await fs.readFile(path.join(sandbox.targets.agentsDir, "rocky", "SKILL.md"), "utf-8"),
    ).toBe("rocky-v1");
    expect(
      await fs.readFile(path.join(sandbox.targets.agentsDir, "rocky-loop", "SKILL.md"), "utf-8"),
    ).toBe("loop-v1");
    for (const dir of [
      ...
      expect(await pathExists(path.join(dir, "rocky-chat"))).toBe(false);
    }
  });
  ...
  it("defines updated as the state reached after preserving user files", async () => {
    ...
    expect(
      await fs.readFile(
        path.join(sandbox.targets.agentsDir, "rocky", "hooks", "guard.sh"),
        "utf-8",
      ),
    ).toBe("user guard");
    expect(
      await fs.readFile(
        path.join(sandbox.targets.claudeDir, "rocky", "notes", "local.md"),
        "utf-8",
      ),
    ).toBe("claude notes");
    expect(
      await fs.readFile(
        path.join(sandbox.targets.codexDir, "rocky", "prompts", "local.md"),
        "utf-8",
      ),
    ).toBe("codex prompt");
  });
  ...
  it("repairs secondary agent targets even when agents skills are current", async () => {
    ...
    expect(status).toEqual({ state: "up-to-date", ops: [] });
    expect(
      await fs.readFile(path.join(sandbox.targets.claudeDir, "rocky-loop", "SKILL.md"), "utf-8"),
    ).toBe("loop-v1");
  });
  ...
  it("does not auto-install skills on a clean machine", async () => {
    ...
    expect(await pathExists(path.join(sandbox.targets.agentsDir, "rocky"))).toBe(false);
    expect(await pathExists(path.join(sandbox.targets.claudeDir, "rocky"))).toBe(false);
    expect(await pathExists(path.join(sandbox.targets.codexDir, "rocky"))).toBe(false);
  });
...
describe("uninstallSkills", () => {
  ...
  it("removes every Rocky skill from all three targets and preserves user dirs", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    ...
    expect(status.state).toBe("not-installed");
    for (const name of ROCKY_SKILL_NAMES) {
  ...
  it("cleans up legacy skill names that linger in agents, claude, and codex", async () => {
    ...
    for (const dir of [
      ...
      await writeOnDiskSkill(dir, "rocky-chat", { "SKILL.md": "chat-old" });
    }
