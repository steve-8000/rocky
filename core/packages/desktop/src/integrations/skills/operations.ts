...
export const ROCKY_SKILL_NAMES = [
  "rocky",
  "rocky-advisor",
  "rocky-chat",
  "rocky-committee",
...
function hasInstalledRockySkill(disks: readonly TargetSkills[]): boolean {
...
export async function getSkillsStatus(targets?: SkillTargets): Promise<SkillsStatus> {
  ...
  if (!hasInstalledRockySkill(disks)) return { state: "not-installed", ops };
