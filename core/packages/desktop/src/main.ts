...
import {
  getRockyBrowserIdForWebContents,
  getRockyBrowserWebContents,
  listRegisteredRockyBrowserIds,
  registerRockyBrowserWebContents,
  setWorkspaceActiveRockyBrowserId,
...
const APP_SCHEME = "rocky";
const ROCKY_DEBUG = process.env.ROCKY_DEBUG === "1";
const DISABLE_SINGLE_INSTANCE_LOCK = process.env.ROCKY_DISABLE_SINGLE_INSTANCE_LOCK === "1";
const APP_NAME = process.env.ROCKY_TEST_APP_NAME?.trim() || "Rocky";

// Rocky: default daemon home to ~/.rocky so a stock Rocky install on the same
// machine (~/.rocky, port 7767) is never touched. Explicit ROCKY_HOME wins.
if (!process.env.ROCKY_HOME) {
  process.env.ROCKY_HOME = path.join(app.getPath("home"), ".rocky");
}
...
const OPEN_PROJECT_EVENT = "rocky:event:open-project";
const BROWSER_SHORTCUT_EVENT = "rocky:event:browser-shortcut";
const BROWSER_FORWARDED_KEY_EVENT = "rocky:event:browser-forwarded-key";

const FORWARDED_ROCKY_SHORTCUT_KEYS = new Set([
  "b",
  "e",
  "w",
  "t",
...
const DESKTOP_SMOKE_ENV = "ROCKY_DESKTOP_SMOKE";
const DESKTOP_SMOKE_STOP_REQUEST = "rocky-smoke-stop";
app.setName(APP_NAME);

function getBrowserIdFromWebviewPartition(partition: string | undefined): string | null {
  const prefix = "persist:rocky-browser-";
...
function isForwardableRockyShortcutInput(input: Electron.Input): boolean {
  ...
  return FORWARDED_ROCKY_SHORTCUT_KEYS.has(input.key.toLowerCase());
}
          ...
          { type: "separator" as const },
          {
            ...
            click: () => {
              log.info("[browser-devtools] inspect-element.request", {
                ...
                browserId: getRockyBrowserIdForWebContents(contents),
                x: params.x,
                y: params.y,
...
const forcedUserDataDir = process.env.ROCKY_ELECTRON_USER_DATA_DIR?.trim();
if (forcedUserDataDir) {
  ...
  try {
    ...
    // Main checkout (e.g. "rocky") gets default userData — only worktrees diverge.
    ...
    if (isWorktree) {
      app.setPath("userData", path.join(app.getPath("appData"), `Rocky-${devWorktreeName}`));
...
// Allow users to pass Chromium flags via ROCKY_ELECTRON_FLAGS for debugging
// rendering issues (e.g. "--disable-gpu --ozone-platform=x11").
// Must run before app.whenReady().
const electronFlags = process.env.ROCKY_ELECTRON_FLAGS?.trim();
if (electronFlags) {
...
ipcMain.handle("rocky:get-pending-open-project", () => {
...
ipcMain.handle("rocky:browser:set-workspace-active-browser", (_event, browserId: unknown) => {
  setWorkspaceActiveRockyBrowserId(typeof browserId === "string" ? browserId : null);
});

ipcMain.handle("rocky:browser:open-devtools", (_event, browserId: unknown) => {
  if (typeof browserId !== "string" || browserId.trim().length === 0) {
    const result = {
      ok: false,
      reason: "invalid-browser-id",
      browserId,
      registeredBrowserIds: listRegisteredRockyBrowserIds(),
    };
    log.warn("[browser-devtools] open-devtools.invalid", result);
    return result;
  }
  const contents = getRockyBrowserWebContents(browserId);
  if (!contents) {
    const result = {
      ok: false,
...
ipcMain.handle("rocky:browser:clear-partition", async (_event, browserId: unknown) => {
  if (typeof browserId !== "string" || browserId.trim().length === 0) {
    return;
  }
  const partition = `persist:rocky-browser-${browserId}`;
...
async function createMainWindow(): Promise<void> {
  ...
  mainWindow.webContents.on("did-attach-webview", (_event, contents) => {
    ...
    if (browserId) {
      registerRockyBrowserWebContents(contents, browserId);
    ...
    contents.on("before-input-event", (event, input) => {
      ...
      if (isBrowserLocationInput(input)) {
        event.preventDefault();
        const focusedBrowserId = getRockyBrowserIdForWebContents(contents);
      ...
      if (isForwardableRockyShortcutInput(input)) {
        event.preventDefault();
...
function setupSingleInstanceLock(): boolean {
  if (DISABLE_SINGLE_INSTANCE_LOCK) {
    log.info("[single-instance] disabled by ROCKY_DISABLE_SINGLE_INSTANCE_LOCK");
    return true;
  }
...
function showDaemonShutdownDialog(): void {
  for (const win of BrowserWindow.getAllWindows()) {
    win.webContents.send("rocky:event:quitting", {});
  }
}
