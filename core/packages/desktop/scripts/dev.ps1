...
$RemoteDebuggingPort = if ($env:ROCKY_ELECTRON_REMOTE_DEBUGGING_PORT) {
    $env:ROCKY_ELECTRON_REMOTE_DEBUGGING_PORT
} else {
    "9223"
}
$ExistingElectronFlags = if ($env:ROCKY_ELECTRON_FLAGS) {
    "$($env:ROCKY_ELECTRON_FLAGS) "
} else {
    ""
}
$env:ROCKY_ELECTRON_FLAGS = "$($ExistingElectronFlags)--remote-debugging-port=$RemoteDebuggingPort"
...
& "$RootDir\node_modules\.bin\concurrently" `
    ...
    "cd `"$AppDir`" && `$env:ROCKY_WEB_PLATFORM = `"electron`"; npx expo start --port $($env:EXPO_PORT)" `
