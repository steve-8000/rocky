...
REMOTE_DEBUGGING_PORT="${ROCKY_ELECTRON_REMOTE_DEBUGGING_PORT:-9223}"
export ROCKY_ELECTRON_FLAGS="${ROCKY_ELECTRON_FLAGS:+$ROCKY_ELECTRON_FLAGS }--remote-debugging-port=$REMOTE_DEBUGGING_PORT"
...
exec "$ROOT_DIR/node_modules/.bin/concurrently" \
  ...
  "cd '$APP_DIR' && ROCKY_WEB_PLATFORM=electron npx expo start --port $EXPO_PORT" \
