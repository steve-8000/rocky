...
function getCliShimPath(appPath) {
  if (process.platform === "darwin") {
    return path.join(appPath, "Contents", "Resources", "bin", "rocky");
  }

  if (process.platform === "win32") {
    return path.join(appPath, "resources", "bin", "rocky.cmd");
  }

  return path.join(appPath, "resources", "bin", "rocky");
}
...
function createDefaultDaemonEnv(extraEnv) {
  ...
  delete env.ROCKY_HOME;
  delete env.ROCKY_LISTEN;
  return env;
}

function parseSmokeLine(line) {
  const prefix = "[rocky-smoke] ";
...
function formatLogs({ stdout, stderr, userData }) {
  ...
  const daemonLog = readIfExists(path.join(os.homedir(), ".rocky", "daemon.log"));
  return [
...
async function smokeCliTerminal({ appPath, env }) {
  const cwd = createTempDir("rocky-smoke-terminal-cwd-");
  const marker = `rocky-packaged-terminal-smoke-${Date.now()}`;
...
async function smokePackagedDesktopApp({ appPath }) {
  ...
  const userData = createTempDir("rocky-smoke-user-data-");
  const env = createDefaultDaemonEnv({
    ROCKY_DESKTOP_SMOKE: "1",
    ROCKY_ELECTRON_USER_DATA_DIR: userData,
  });
...
if (require.main === module) {
  ...
  if (!appPath) {
    process.stderr.write("Usage: node smoke-packaged-desktop-app.js --app <Rocky.app>\n");
    process.exit(2);
  }
