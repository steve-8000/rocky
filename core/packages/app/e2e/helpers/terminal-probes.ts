...
export async function installTerminalRenderProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    interface ProbeTerm {
      ...
      __rockyRenderProbeWriteWrapped?: boolean;
    }
    ...
    const win = window as unknown as Record<string, unknown> & {
      ...
      __rockyTerminal?: ProbeTerm;
    };
    const existingDescriptor = Object.getOwnPropertyDescriptor(win, "__rockyTerminal");
    const getExisting = () =>
    ...
    Object.defineProperty(win, "__rockyTerminal", {
      configurable: true,
      get() {
        return probe.term;
      },
      set(next: ProbeTerm | undefined) {
        ...
        if (next?.write && !next.__rockyRenderProbeWriteWrapped) {
          ...
          next.__rockyRenderProbeWriteWrapped = true;
        }
      },
    });
  });
...
export async function installTerminalKeystrokeStressProbe(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    const existingDescriptor = Object.getOwnPropertyDescriptor(window, "__rockyTerminal");
    const getExisting = () =>
      existingDescriptor?.get ? existingDescriptor.get.call(window) : existingDescriptor?.value;

    let terminal = getExisting();
    Object.defineProperty(window, "__rockyTerminal", {
      configurable: true,
      get() {
        return terminal;
      },
      set(next: {
        write?: (data: string | Uint8Array, callback?: () => void) => void;
        __rockyKeystrokeProbeWriteWrapped?: boolean;
      }) {
        terminal = next;
        if (next?.write && !next.__rockyKeystrokeProbeWriteWrapped) {
          const originalWrite = next.write.bind(next);
          next.write = (data: string | Uint8Array, callback?: () => void) => {
            const text = typeof data === "string" ? data : new TextDecoder().decode(data);
          ...
          next.__rockyKeystrokeProbeWriteWrapped = true;
        }
      },
    });
  });
