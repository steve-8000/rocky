...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@rocky.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "Rocky E2E"', { cwd: repoPath, stdio: "ignore" });
  execSync("git config commit.gpgsign false", { cwd: repoPath, stdio: "ignore" });
  await writeFile(path.join(repoPath, "README.md"), "# Temp Repo\n");
  if (options?.rockyConfig) {
    await writeFile(
      path.join(repoPath, "rocky.json"),
      JSON.stringify(options.rockyConfig, null, 2),
    );
  }
  ...
  if (options?.rockyConfig) {
    execSync("git add rocky.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.rocky-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
 ...
 */
export async function createTempDirectory(prefix = "rocky-e2e-dir-"): Promise<TempDirectory> {
