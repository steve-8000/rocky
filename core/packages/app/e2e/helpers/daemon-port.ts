 ...
 *
 * The port-7767 guard is a hard guardrail: 7767 is the developer's default
 * daemon, which manages real agents. The e2e port is never legitimately 7767,
...
export function getE2EDaemonPort(): string {
  ...
  if (port === "7767") {
    throw new Error("E2E_DAEMON_PORT must not point at the developer daemon (7767).");
  }
  return port;
}
