 ...
 * status endpoint, PID from the rocky.pid lock file, log path from the
 * E2E_ROCKY_HOME directory. Call this in Node test code (not in the browser).
 */
export async function loadRealDaemonState(): Promise<RealDaemonState> {
  const port = getE2EDaemonPort();
  const rockyHome = process.env.E2E_ROCKY_HOME;
  if (!rockyHome) throw new Error("E2E_ROCKY_HOME not set — globalSetup must run first");
  ...
  try {
    const raw = readFileSync(`${rockyHome}/rocky.pid`, "utf8");
    ...
    console.warn("[desktop-updates] rocky.pid not found:", err);
  }

  return { version: data.version, pid, logPath: `${rockyHome}/daemon.log` };
}
...
/**
 * Injects window.rockyDesktop before app load so all Electron-gated code
...
export async function injectDesktopBridge(page: Page, config: DesktopBridgeConfig): Promise<void> {
  ...
  await page.addInitScript((cfg) => {
    ...
    (window as unknown as { rockyDesktop: unknown }).rockyDesktop = desktopBridge;
  }, config);
}
