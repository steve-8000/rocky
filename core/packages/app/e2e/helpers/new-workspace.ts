...
import type { DaemonClient as InternalDaemonClient } from "@getrocky/client/internal/daemon-client";
...
export async function archiveWorkspaceFromDaemon(
  ...
  const payload = await client.archiveRockyWorktree({ worktreePath: workspaceId });
  if (payload.error) {
...
export async function createWorktreeViaDaemon(
  ...
  const payload = await client.createRockyWorktree({
    cwd: input.cwd,
