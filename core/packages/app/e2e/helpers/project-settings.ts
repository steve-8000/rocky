...
export async function corruptRockyConfig(repoPath: string): Promise<void> {
  await writeFile(path.join(repoPath, "rocky.json"), "{not valid json}");
}

export async function bumpRockyConfigOnDisk(repoPath: string): Promise<void> {
  const configPath = path.join(repoPath, "rocky.json");
...
export async function restoreRockyConfig(
  repoPath: string,
  config: Record<string, unknown>,
): Promise<void> {
  await writeFile(path.join(repoPath, "rocky.json"), JSON.stringify(config, null, 2) + "\n");
}
...
export async function blockRockyConfigWrites(repoPath: string): Promise<void> {
  await chmod(repoPath, 0o555);
}

export async function unblockRockyConfigWrites(repoPath: string): Promise<void> {
