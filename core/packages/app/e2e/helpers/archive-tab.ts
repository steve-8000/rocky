...
export async function primeAdditionalPage(page: Page): Promise<void> {
  ...
  await page.addInitScript(
    ({ daemon: seededDaemon, preferences: seededPreferences, seedNonce: nonce }) => {
      ...
      localStorage.setItem("@rocky:daemon-registry", JSON.stringify([seededDaemon]));
      localStorage.removeItem("@rocky:settings");
      localStorage.setItem("@rocky:create-agent-preferences", JSON.stringify(seededPreferences));
    },
