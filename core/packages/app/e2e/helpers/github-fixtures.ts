...
async function seedPr(args: {
  ...
  git(["config", "user.email", "e2e@rocky.test"], localPath);
  git(["config", "user.name", "Rocky E2E"], localPath);
...
const TEMP_GITHUB_REPO_PREFIX = "rockytmp-";
...
export async function createTempGithubRepo(options: {
  ...
  git(["config", "user.email", "e2e@rocky.test"], basePath);
  git(["config", "user.name", "Rocky E2E"], basePath);
