...
export interface RockyHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveRockyHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkRockyHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<RockyHomeMetadataForkResult> {
  const sourceHome = resolveRockyHomePath(input.sourceHome);
  const targetHome = resolveRockyHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Rocky metadata onto the same ROCKY_HOME.");
  }
