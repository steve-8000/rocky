...
const test = base.extend<{ rockyE2ESetup: void; withWorkspace: WithWorkspace }>({
  ...
  rockyE2ESetup: [
    async ({ page }, provide, testInfo) => {
      ...
      // This blocks both HTTP and WS attempts to :7767 (before any navigation).
      await page.route(/:(7767)\b/, (route) => route.abort());
      await page.routeWebSocket(/:(7767)\b/, async (ws) => {
        await ws.close({ code: 1008, reason: "Blocked connection to localhost:7767 during e2e." });
      });
      ...
      await page.addInitScript(
        ({ daemon, preferences, seedNonce: nonce }) => {
          ...
          const disableOnceKey = "@rocky:e2e-disable-default-seed-once";
          ...
          localStorage.setItem("@rocky:e2e", "1");
          localStorage.setItem("@rocky:e2e-seed-nonce", nonce);

          // Hard-reset anything that could point to a developer's real daemon.
          localStorage.setItem("@rocky:daemon-registry", JSON.stringify([daemon]));
          localStorage.removeItem("@rocky:settings");
          localStorage.setItem("@rocky:create-agent-preferences", JSON.stringify(preferences));
        },
