...
import { forkRockyHomeMetadata, resolveRockyHomePath } from "./helpers/rocky-home-fork";
...
let rockyHome: string | null = null;
let fakeToolBinDir: string | null = null;
let relayProcess: ChildProcess | null = null;

function resolveOptionalRockyHomeEnv(value: string | undefined): string | null {
  const trimmed = value?.trim();
  if (!trimmed) {
    return null;
  }
  if (trimmed === "current") {
    return resolveRockyHomePath("~/.rocky");
  }
  return resolveRockyHomePath(trimmed);
}
...
async function createFakeToolBin(): Promise<string> {
  const binDir = await mkdtemp(path.join(tmpdir(), "rocky-e2e-tool-bin-"));
  ...
  console.log(JSON.stringify([
    {
      ...
      url: "https://github.com/getrocky/rocky/pull/515",
      state: "OPEN",
...
if (args[0] === "pr" && args[1] === "view" && args[2] === "--json" && args[3]) {
  const fixture = path.join(process.cwd(), ".rocky-e2e-pr.json");
...
if (args[0] === "api" && args[1] === "graphql") {
  const fixture = path.join(process.cwd(), ".rocky-e2e-timeline.json");
  if (fs.existsSync(fixture)) {
    console.log(fs.readFileSync(fixture, "utf8"));
    process.exit(0);
  }
...
const fs = require("fs");
const path = require("path");
const recordPath = process.env.ROCKY_E2E_EDITOR_RECORD_PATH;
...
function ensureRelayBuildArtifact(repoRoot: string): void {
  ...
  console.log("[e2e] Building @getrocky/relay for daemon startup");
...
function loadPairingOfferFromCli(repoRoot: string, rockyHomePath: string): OfferPayload {
...
async function waitForPairingOfferFromCli(args: {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      return loadPairingOfferFromCli(args.repoRoot, args.rockyHome);
    } catch (error) {
      lastError = error;
      await sleep(100);
    }
  ...
  throw new Error(
    `Timed out waiting for \`rocky daemon pair --json\` to produce a pairing offer: ${
...
async function applyRockyHomeFork(targetHome: string): Promise<void> {
  const forkSourceHome = resolveOptionalRockyHomeEnv(process.env.E2E_FORK_ROCKY_HOME_FROM);
  if (!forkSourceHome) {
    return;
  }
  const forkResult = await forkRockyHomeMetadata({
    sourceHome: forkSourceHome,
    targetHome,
  });
  process.env.E2E_FORK_SOURCE_ROCKY_HOME = forkResult.sourceHome;
  process.env.E2E_FORK_TARGET_ROCKY_HOME = forkResult.targetHome;
  ...
  console.log(
    `[e2e] Forked Rocky metadata from ${forkResult.sourceHome} to ${forkResult.targetHome} ` +
  ...
  if (forkResult.skippedMissing.length > 0) {
    console.warn(
      `[e2e] Rocky metadata fork skipped missing paths: ${forkResult.skippedMissing.join(", ")}`,
    );
  }
}
...
async function resolveDictationConfig(): Promise<DictationConfig> {
  ...
  // them gate on ROCKY_DICTATION_ENABLED.
  if (!openAiUsable && !hasDefaultLocalModelsDir) {
    console.warn(
      "[e2e] Neither OPENAI_API_KEY nor local speech models found — running with dictation/voice disabled. " +
        "Tests that require dictation should gate on ROCKY_DICTATION_ENABLED.",
    );
...
function startDaemon(args: DaemonSpawnArgs): ChildProcess {
  ...
  const child = spawn(tsxBin, ["scripts/supervisor-entrypoint.ts", "--dev"], {
    ...
    env: {
      ...
      ROCKY_HOME: args.rockyHome,
      ROCKY_E2E_EDITOR_RECORD_PATH: args.editorRecordPath,
      ROCKY_SERVER_ID: "srv_e2e_test_daemon",
      ROCKY_LISTEN: `0.0.0.0:${args.port}`,
      ROCKY_RELAY_ENDPOINT: `127.0.0.1:${args.relayPort}`,
      ROCKY_CORS_ORIGINS: `http://localhost:${args.metroPort}`,
      ROCKY_DICTATION_ENABLED: openAiUsable ? "1" : "0",
      ROCKY_VOICE_MODE_ENABLED: openAiUsable ? "1" : "0",
      ROCKY_NODE_ENV: "development",
      ...(openAiUsable
        ? {
            ROCKY_DICTATION_STT_PROVIDER: "openai",
            ROCKY_VOICE_STT_PROVIDER: "openai",
            ROCKY_VOICE_TTS_PROVIDER: "openai",
          }
        : {}),
      ...(localModelsDir ? { ROCKY_LOCAL_MODELS_DIR: localModelsDir } : {}),
...
async function performCleanup(shouldRemoveRockyHome: boolean): Promise<void> {
  await Promise.all([
  ...
  metroProcess = null;
  relayProcess = null;
  if (rockyHome && shouldRemoveRockyHome) {
    await rm(rockyHome, { recursive: true, force: true });
    rockyHome = null;
  } else if (rockyHome) {
    console.log(`[e2e] Preserving ROCKY_HOME: ${rockyHome}`);
  }
  if (fakeToolBinDir) {
...
export default async function globalSetup() {
  ...
  const requestedRockyHome = resolveOptionalRockyHomeEnv(process.env.E2E_ROCKY_HOME);
  const shouldRemoveRockyHome = !requestedRockyHome && process.env.E2E_KEEP_ROCKY_HOME !== "1";
  rockyHome = requestedRockyHome ?? (await mkdtemp(path.join(tmpdir(), "rocky-e2e-home-")));
  const editorRecordPath = path.join(rockyHome, "editor-open-records.jsonl");
  fakeToolBinDir = await createFakeToolBin();
  const metroLineBuffer = createLineBuffer();
  const daemonLineBuffer = createLineBuffer();

  await applyRockyHomeFork(rockyHome);

  const cleanup = () => performCleanup(shouldRemoveRockyHome);
  ...
  try {
    ...
    process.env.E2E_ROCKY_HOME = rockyHome;
    process.env.E2E_EDITOR_RECORD_PATH = editorRecordPath;
    console.log(
      `[e2e] Test daemon started on port ${port}, Metro on port ${metroPort}, home: ${rockyHome}`,
    );
