...
export function getDaemonManagementErrorPresentation(
  ...
  if (presentationError instanceof DaemonConnectionRegistrationError) {
    return {
      message:
        "Built-in daemon started, but Rocky could not save the localhost connection. Toggle daemon management off and on again, or add localhost manually.",
      refreshStatus: true,
    };
  }
  if (wasManagingDaemon) {
    return {
      message: "Built-in daemon management was paused, but Rocky could not stop the daemon.",
      refreshStatus: false,
    };
  }
  return {
