...
describe("desktop-updates helpers", () => {
  ...
  it("parses local daemon version error result", async () => {
    ...
    expect(
      parseLocalDaemonVersionResult({ version: null, error: "rocky command not found in PATH" }),
    ).toEqual({
      version: null,
      error: "rocky command not found in PATH",
    });
  });
  ...
  it("builds the direct Apple Silicon DMG URL from a version", async () => {
    ...
    expect(buildMacAppleSiliconDownloadUrl("v0.1.64")).toBe(
      "https://github.com/getrocky/rocky/releases/download/v0.1.64/Rocky-0.1.64-arm64.dmg",
    );
