...
function destroyWebviewSelector(webview: ElectronWebview): void {
  void executeWebviewJavaScript(
    webview,
    "if(window.__rockySelector) window.__rockySelector.destroy();",
...
function clearWebviewSelector(webview: ElectronWebview): void {
  void executeWebviewJavaScript(
    webview,
    "if(window.__rockySelector) window.__rockySelector.destroy(); window.__rockySelectorResult = null;",
  ).catch(ignoreWebviewJavaScriptError);
}
    ...
    void (async () => {
      try {
        const raw = await executeWebviewJavaScript(
          webview,
          "JSON.stringify(window.__rockySelectorResult || null)",
        );
        ...
        onDone();
        await executeWebviewJavaScript(webview, "window.__rockySelectorResult = null;");
...
export function BrowserPane({
  ...
  useEffect(() => {
    ...
    webview.setAttribute("partition", `persist:rocky-browser-${browserId}`);
  ...
  const startElementSelector = useCallback(() => {
    ...
    const js = `
      (function() {
        ...
        style.textContent = [
          '.__rocky-hover { outline: 2px solid #3b82f6 !important; outline-offset: 2px !important; cursor: crosshair !important; }',
          '.__rocky-select-mode, .__rocky-select-mode * { cursor: crosshair !important; pointer-events: auto !important; user-select: none !important; }',
          '.__rocky-select-mode *, .__rocky-select-mode *::before, .__rocky-select-mode *::after { animation: none !important; transition: none !important; }',
          '.__rocky-select-mode a, .__rocky-select-mode button, .__rocky-select-mode input, .__rocky-select-mode select, .__rocky-select-mode textarea, .__rocky-select-mode [role="button"], .__rocky-select-mode [onclick] { pointer-events: none !important; }',
          '.__rocky-select-mode iframe, .__rocky-select-mode video, .__rocky-select-mode audio { pointer-events: none !important; }',
        ].join('\\n');
        document.head.appendChild(style);
        document.documentElement.classList.add('__rocky-select-mode');
        var last = null;
        function onMove(e) {
          e.preventDefault();
          e.stopPropagation();
          if (last) last.classList.remove('__rocky-hover');
          e.target.classList.add('__rocky-hover');
          last = e.target;
        }
        ...
        function onClick(e) {
          ...
          destroy();
          window.__rockySelectorResult = result;
        }
        function onKey(e) {
          if (e.key === 'Escape') { destroy(); window.__rockySelectorResult = { __cancelled: true }; }
        }
        ...
        function destroy() {
          ...
          document.documentElement.classList.remove('__rocky-select-mode');
          if (last) last.classList.remove('__rocky-hover');
          style.remove();
          window.__rockySelector = null;
        }
        ...
        window.__rockySelector = { destroy: destroy };
      })()
    `;
