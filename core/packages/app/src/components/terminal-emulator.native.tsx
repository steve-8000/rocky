...
export default function TerminalEmulator({
  ...
  const flushPendingMessages = useCallback(() => {
    ...
    for (const message of pending) {
      ...
      webViewRef.current.injectJavaScript(
        `window.__ROCKY_TERMINAL_WEBVIEW_RECEIVE__ && window.__ROCKY_TERMINAL_WEBVIEW_RECEIVE__(${payload}); true;`,
      );
    }
  }, []);
  ...
  useImperativeHandle(
    ...
    (): TerminalEmulatorHandle => ({
      ...
      blur: () => {
        webViewRef.current?.injectJavaScript(
          "window.__ROCKY_TERMINAL_WEBVIEW_BLUR__ && window.__ROCKY_TERMINAL_WEBVIEW_BLUR__(); true;",
        );
        Keyboard.dismiss();
      },
    }),
