...
import type { CheckoutPrMergeMethod } from "@getrocky/protocol/messages";
...
function invalidateWorktreeList() {
  void appQueryClient.invalidateQueries({
    predicate: (query) =>
      Array.isArray(query.queryKey) && query.queryKey[0] === "rockyWorktreeList",
  });
  void appQueryClient.invalidateQueries({
    predicate: (query) =>
      Array.isArray(query.queryKey) && query.queryKey[0] === "sidebarRockyWorktreeList",
  });
}
  ...
  appQueryClient.setQueriesData(
    {
      predicate: (query) =>
        ...
        query.queryKey[0] === "rockyWorktreeList" &&
  ...
  appQueryClient.setQueriesData(
    {
      predicate: (query) =>
        Array.isArray(query.queryKey) &&
        query.queryKey[0] === "sidebarRockyWorktreeList" &&
        query.queryKey[1] === serverId,
    },
    removeFromList,
  );
...
function isWorktreeListQuery(input: { queryKey: QueryKey; serverId: string }): boolean {
  return (
    ...
    (input.queryKey[0] === "rockyWorktreeList" ||
      input.queryKey[0] === "sidebarRockyWorktreeList") &&
  ...
  archiveWorktree: async ({ serverId, cwd, worktreePath }) => {
    await runCheckoutAction({
      ...
      run: async () => {
        ...
        try {
          const payload = await client.archiveRockyWorktree({ worktreePath });
          if (payload.error) {
            throw new Error(payload.error.message);
          }
        } catch (error) {
