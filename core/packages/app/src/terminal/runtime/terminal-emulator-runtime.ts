...
} from "@getrocky/protocol/terminal-input-mode";
import {
...
export class TerminalEmulatorRuntime {
  ...
  mount(input: TerminalEmulatorRuntimeMountInput): void {
    ...
    window.__rockyTerminal = terminal;
  ...
  unmount(): void {
    ...
    this.cleanup?.();
    this.cleanup = null;
    if (window.__rockyTerminal === this.terminal) {
      window.__rockyTerminal = undefined;
    }
    this.terminal = null;
    this.fitAddon = null;
