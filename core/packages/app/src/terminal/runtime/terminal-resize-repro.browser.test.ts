...
// Regression: "streaming pino log, resized the Rocky terminal, old logs stayed
...
function mount(width: number, height: number): Mounted {
  ...
  const terminal = window.__rockyTerminal as InspectableTerminal | undefined;
  if (!terminal) {
...
describe("terminal resize reflow repro (Rocky terminal)", () => {
