...
function getBrowserTerminal(): BrowserTerminal {
  const terminal = window.__rockyTerminal as BrowserTerminal | undefined;
  if (!terminal) {
...
describe("terminal emulator runtime in a real browser", () => {
  it("passes configured scrollback to xterm", async () => {
    ...
    await waitFor({
      predicate: () => window.__rockyTerminal !== undefined,
    });

    expect(window.__rockyTerminal?.options.scrollback).toBe(42_000);
  });
  ...
  it("updates scrollback on the mounted xterm", async () => {
    ...
    const terminal = window.__rockyTerminal;

    mounted.runtime.setScrollback({ lines: 42_000 });

    expect(window.__rockyTerminal).toBe(terminal);
    expect(window.__rockyTerminal?.options.scrollback).toBe(42_000);
  });
