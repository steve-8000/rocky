...
import type {
  ...
  FetchAgentsEntry,
  FetchAgentsOptions,
} from "@getrocky/client/internal/daemon-client";
import type { ConnectionOffer } from "@getrocky/protocol/connection-offer";
...
function makeOffer(input?: Partial<ConnectionOffer>): ConnectionOffer {
  return {
    ...
    relay: {
      endpoint: input?.relay?.endpoint ?? "relay.invalid:443",
...
function encodeOfferUrl(payload: unknown): string {
  const encoded = Buffer.from(JSON.stringify(payload), "utf8")
    ...
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
  return `https://rocky.clab.one/#offer=${encoded}`;
}
...
describe("HostRuntimeController", () => {
  it("replaces the active relay client when re-pairing changes the daemon public key", async () => {
    const oldRelay: HostConnection = {
      id: "relay:wss:relay.invalid:443",
      type: "relay",
      relayEndpoint: "relay.invalid:443",
      useTls: true,
  ...
  it("keeps known hosts in connecting when a created client reports idle during connect", async () => {
    ...
    ).switchToConnection({ connectionId: "direct:lan:7767" });

    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:7767");
  ...
  it("adopts the first successful probe on startup", async () => {
    const host = makeHost({ preferredConnectionId: "direct:lan:7767" });
    const clients: FakeDaemonClient[] = [];
    const latencies: Record<string, number | Error> = {
      "direct:lan:7767": 82,
      "relay:relay.invalid:443": 18,
    };
    const controller = new HostRuntimeController({
      host,
    ...
    expect(snapshot.activeConnectionId).toBe("direct:lan:7767");
  ...
  it("activates the first successful probe without waiting for slower probes", async () => {
    ...
    const controller = new HostRuntimeController({
      ...
      deps: {
        ...
        connectToDaemon: async ({ host: hostProfile, connection }) => {
          const client = makeConnectedProbeClient(connection.id === "direct:lan:7767" ? 12 : 30);
          if (connection.id === "relay:relay.invalid:443") {
    ...
    while (Date.now() < timeoutAt) {
      const snapshot = controller.getSnapshot();
      if (
        snapshot.activeConnectionId === "direct:lan:7767" &&
  ...
  it("probes the active online connection through the existing client", async () => {
    ...
    expect(probeAttempts).toEqual(["relay:relay.invalid:443"]);
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:7767");
    expect(controller.getSnapshot().connectionStatus).toBe("online");
    expect(controller.getSnapshot().probeByConnectionId.get("direct:lan:7767")).toEqual({
      status: "available",
      latencyMs: 9,
    });
  });
  ...
  it("rejects probes that resolve to a different server id", async () => {
    const host = makeHost({
      ...
      connections: [
        {
          id: "direct:localhost:7767",
          type: "directTcp",
          endpoint: "localhost:7767",
        },
      ],
    });
    ...
    expect(controller.getSnapshot().probeByConnectionId.get("direct:localhost:7767")).toEqual({
      status: "unavailable",
      latencyMs: null,
    });
  ...
  it("fails over when the active client ping fails", async () => {
    ...
    latencies["direct:lan:7767"] = new Error("direct unavailable");
    latencies["relay:relay.invalid:443"] = 42;
    clearProbeBackoff(controller);
    await controller.runProbeCycleNow();

    const snapshot = controller.getSnapshot();
    expect(snapshot.activeConnectionId).toBe("relay:relay.invalid:443");
    expect(snapshot.connectionStatus).toBe("online");
  ...
  it("backs off inactive connection probes while a host is online", async () => {
    ...
    const initialRelayProbe = controller
      .getSnapshot()
      .probeByConnectionId.get("relay:relay.invalid:443");

    latencies["direct:lan:7767"] = 12;
    latencies["relay:relay.invalid:443"] = 25;
    const lastProbedAt = (
      controller as unknown as {
        connectionLastProbedAt: Map<string, number>;
      }
    ...
    lastProbedAt.set("direct:lan:7767", now - 60_000);
    lastProbedAt.set("relay:relay.invalid:443", now - 60_000);

    await controller.runProbeCycleNow();

    const snapshot = controller.getSnapshot();
    expect(clients.length).toBe(initialClientCount);
    expect(snapshot.probeByConnectionId.get("direct:lan:7767")).toEqual({
      status: "available",
      latencyMs: 12,
    });
    expect(snapshot.probeByConnectionId.get("relay:relay.invalid:443")).toEqual(initialRelayProbe);
  });
  ...
  it("switches only after the faster alternative wins consecutive probes", async () => {
    ...
    latencies["direct:lan:7767"] = 95;
    latencies["relay:relay.invalid:443"] = 30;
    clearProbeBackoff(controller);
    await controller.runProbeCycleNow();
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:7767");
    ...
    let switched = controller.getSnapshot().activeConnectionId === "relay:relay.invalid:443";
    for (let index = 0; index < 6 && !switched; index += 1) {
      clearProbeBackoff(controller);
      await controller.runProbeCycleNow();
      switched = controller.getSnapshot().activeConnectionId === "relay:relay.invalid:443";
    }
  ...
  it("does not switch on a transient latency spike", async () => {
    ...
    latencies["direct:lan:7767"] = 100;
    latencies["relay:relay.invalid:443"] = 20;
    clearProbeBackoff(controller);
    await controller.runProbeCycleNow();
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:7767");

    latencies["direct:lan:7767"] = 20;
    latencies["relay:relay.invalid:443"] = 90;
    clearProbeBackoff(controller);
    await controller.runProbeCycleNow();
    expect(controller.getSnapshot().activeConnectionId).toBe("direct:lan:7767");
  ...
  it("exposes one snapshot with active connection and status from same source", async () => {
    ...
    expect(latest?.activeConnectionId).toBe("direct:lan:7767");
  ...
  it("ignores stale switch failures after a newer connection is already online", async () => {
    ...
    const deps: HostRuntimeControllerDeps = {
      createClient: ({ connection }) => {
        const client = new FakeDaemonClient();
        if (connection.id === "direct:lan:7767") {
    ...
    ).switchToConnection({ connectionId: "relay:relay.invalid:443" });
    await waitUntil(() => {
      const snapshot = controller.getSnapshot();
      return (
        snapshot.activeConnectionId === "relay:relay.invalid:443" &&
        snapshot.connectionStatus === "online"
      );
    });
  ...
  it("coalesces overlapping probe cycles instead of invalidating the in-flight result", async () => {
    ...
    const probeAfterCycle = controller.getSnapshot().probeByConnectionId.get("direct:lan:7767");
  ...
  it("does not notify or replace the snapshot for equal probe maps", () => {
    ...
    const firstProbeMap = makeProbeMap([
      ["direct:lan:7767", { status: "available", latencyMs: 12 }],
    ]);
  ...
  it("notifies once and replaces the probe map when probe contents change", () => {
    ...
    const changedProbeMap = makeProbeMap([
      ["direct:lan:7767", { status: "available", latencyMs: 12 }],
      ["relay:relay.invalid:443", { status: "unavailable", latencyMs: null }],
    ]);
...
describe("HostRuntimeStore", () => {
      ...
      makeFetchAgentsPayload({
        entries: [
          makeFetchAgentsEntry({
            ...
            cwd: "/Users/moboudra/dev/rocky-pr67-review",
  ...
  it("records unavailable startup probes when no connection can be established", async () => {
    ...
    while (
      snapshot?.probeByConnectionId.get("direct:lan:7767")?.status !== "unavailable" &&
      Date.now() < timeoutAt
    ) {
      await new Promise((resolve) => setTimeout(resolve, 0));
  ...
  it("upsertDirectConnection stores SSL and password settings", async () => {
    ...
    await store.upsertDirectConnection({
      ...
      endpoint: "example.rocky.test:7443",
      useTls: true,
    ...
    expect(host?.connections).toEqual([
      {
        id: "direct:example.rocky.test:7443",
        type: "directTcp",
        endpoint: "example.rocky.test:7443",
        useTls: true,
        password: "shared-secret",
  ...
  it("probeAndUpsertConnection replaces a matching placeholder host with the real server id", async () => {
    ...
    (
      ...
      makeHost({
        serverId: "local:lan:7767",
        label: "local:lan:7767",
        connections: [connection],
  ...
  it("uses TLS for old pairing URLs that omit relay TLS on port 443", async () => {
    ...
    const oldPairingUrl = encodeOfferUrl({
      ...
      serverId: "srv_offer",
      daemonPublicKeyB64: "pk_test_offer",
      relay: { endpoint: "relay.invalid:443" },
    });
