...
import type { AgentAttachment, GitHubSearchItem } from "@getrocky/protocol/messages";
import type { CreateRockyWorktreeInput } from "@getrocky/client/internal/daemon-client";
import type { AgentProvider } from "@getrocky/protocol/agent-types";
...
async function createAndMergeWorkspace(input: {
  ...
  createInput: Parameters<
    NonNullable<ReturnType<typeof useHostRuntimeClient>>["createRockyWorktree"]
  >[0];
  mergeWorkspaces: (
    serverId: string,
  ...
  const payload = await input.client.createRockyWorktree(input.createInput);
