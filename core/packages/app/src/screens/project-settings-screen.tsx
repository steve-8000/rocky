...
const WORKTREE_DOCS_URL = "https://rocky.clab.one/docs/worktrees";
...
const METADATA_GROUP_INFO =
  "Project-specific instructions injected into the AI prompts Rocky uses to generate metadata — use them to enforce your team's conventions like branch naming, commit style, or PR format";
...
function ProjectSettingsBody({
  ...
  const loadedConfig: RockyConfigRaw | null = data?.ok ? (data.config ?? {}) : null;
  const loadedRevision: RockyConfigRevision | null = data?.ok ? data.revision : null;
...
interface RenderContentInput {
  ...
  loadedConfig: RockyConfigRaw | null;
  loadedRevision: RockyConfigRevision | null;
...
function revisionToKey(revision: RockyConfigRevision | null): string {
...
function resolveReadFailureCopy(input: {
  ...
  if (input.kind === "invalid_project_config") {
    return {
      ...
      title: "rocky.json couldn't be parsed",
  ...
  if (input.kind === "transport") {
    ...
    return {
      ...
      title: "Couldn't load rocky.json",
...
interface ProjectConfigFormProps {
  baseConfig: RockyConfigRaw;
  revision: RockyConfigRevision | null;
  repoRoot: string;
  queryKey: readonly [string, string, string];
  client: DaemonClient;
  onReload: () => void;
      ...
      {isStale ? (
        <View style={styles.calloutWrap}>
          <Alert
            ...
            description="Reload to fetch the latest rocky.json before saving."
          >
            <Button
      ...
      {isWriteFailed ? (
        <View style={styles.calloutWrap}>
          <Alert
            ...
            variant="error"
            title="Couldn't save rocky.json"
        ...
        <View style={styles.serviceToggleRow}>
          <View style={styles.serviceToggleText}>
            ...
            <Text style={styles.modalHint}>
              Rocky supervises the process and assigns a port via $ROCKY_PORT
            </Text>
          </View>
          <Switch
            value={isService}
