...
function RestartDaemonCard({ host }: { host: HostProfile }) {
  ...
  const handleRestart = useCallback(() => {
    ...
    if (!isHostConnected()) {
      Alert.alert(
        "Host offline",
        "This host is offline. Rocky reconnects automatically—wait until it's back online before restarting.",
      );
      return;
    }
      ...
      .then((confirmed) => {
        ...
        void daemonClient
          ...
          .catch((error) => {
            ...
            Alert.alert(
              "Error",
              "Failed to send the restart request. Rocky reconnects automatically—try again once the host shows as online.",
            );
          });
    ...
    <View style={settingsStyles.card} testID="host-page-auto-archive-merged-workspaces-card">
      <View style={settingsStyles.row}>
        <View style={settingsStyles.rowContent}>
          ...
          <Text style={settingsStyles.rowHint}>
            Automatically archive clean Rocky workspaces after their pull request is merged
          </Text>
        </View>
        <Switch
