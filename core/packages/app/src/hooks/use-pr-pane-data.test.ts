...
describe("shouldFetchTimelineFrom", () => {
  const baseGate = {
    ...
    githubFeaturesEnabled: true,
    cwd: "/repo",
    identity: { prNumber: 42, repoOwner: "getrocky", repoName: "rocky" },
...
describe("fetchPrPaneTimelinePage", () => {
  it("forwards the cwd, PR number, owner, and name to the daemon client", async () => {
    ...
    expect(client.calls).toEqual([
      { cwd: "/repo", prNumber: 42, repoOwner: "getrocky", repoName: "rocky" },
    ]);
  });
      ...
      items: [
        {
          ...
          body: "Looks good",
          createdAt: Date.now(),
          url: "https://github.com/getrocky/rocky/pull/42#c1",
        },
      ],
    });
        ...
        items: [
          {
            ...
            createdAt: Date.now(),
            url: "https://github.com/getrocky/rocky/pull/41#c1",
          },
        ],
      }),
    });
