...
function inferMainRepoRootFromRockyWorktreePath(cwd: string): string | null {
  const normalizedPath = cwd.replace(/\\/g, "/");
  const marker = "/.rocky/worktrees";
...
export function resolveNewAgentWorkingDir(
  ...
  const explicitMainRepoRoot = checkout?.isRockyOwnedWorktree
  ...
  return inferMainRepoRootFromRockyWorktreePath(cwd) ?? cwd;
}
