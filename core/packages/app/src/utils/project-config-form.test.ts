...
import { RockyConfigRawSchema } from "@getrocky/protocol/rocky-config-schema";
import type { RockyConfigRaw } from "@getrocky/protocol/messages";
...
describe("applyDraftToConfig", () => {
  it("preserves the original string kind when editing an existing setup field", () => {
    const base: RockyConfigRaw = { worktree: { setup: "npm install" } };
  ...
  it("preserves unknown top-level, worktree, and script entry fields on round-trip", () => {
    const base = RockyConfigRawSchema.parse({
      worktree: {
        setup: "npm install",
