...
import type { DaemonClientConfig } from "@getrocky/client/internal/daemon-client";
...
class FakeDaemonProbe {
  ...
  readonly deps: DaemonConnectionDependencies<FakeDaemonClient> = {
    ...
    buildLocalTransportUrl: ({ transportType, transportPath }) =>
      `rocky+local://${transportType}?path=${encodeURIComponent(transportPath)}`,
    createClient: (config) => {
...
describe("test-daemon-connection connectToDaemon", () => {
  ...
  it("encodes the local socket target into the client config", async () => {
    ...
    expect(probe.createdConfigs()[0]?.url).toBe("rocky+local://socket?path=%2Ftmp%2Frocky.sock");
  });
  ...
  it("uses relay TLS from the stored connection", async () => {
    ...
    expect(probe.createdConfigs()[1]?.url).toMatch(/^ws:\/\/relay\.rocky\.sh:443\/ws\?/);
  });
