...
describe("resolveNewAgentWorkingDir", () => {
  ...
  it("falls back to repo root when checkout metadata is unavailable", () => {
    expect(resolveNewAgentWorkingDir("/repo/.rocky/worktrees/feature", null)).toBe("/repo");
  });

  it("supports windows-style rocky worktree paths without checkout metadata", () => {
    expect(resolveNewAgentWorkingDir("C:\\Users\\me\\repo\\.rocky\\worktrees\\feature", null)).toBe(
      "C:\\Users\\me\\repo",
    );
  });

  it("returns the main repo root for rocky-owned worktrees", () => {
    const checkout = {
      isRockyOwnedWorktree: true,
      worktreeRoot: "/repo/.rocky/worktrees/feature",
      mainRepoRoot: "/repo/main",
    } as unknown as CheckoutStatusPayload;

    expect(resolveNewAgentWorkingDir("/repo/.rocky/worktrees/feature", checkout)).toBe(
      "/repo/main",
    );
  });
});
...
describe("parseAgentKey", () => {
  ...
  it("uses the last separator to preserve server ids with colons", () => {
    expect(parseAgentKey("localhost:7767:agent-9")).toEqual({
      serverId: "localhost:7767",
      agentId: "agent-9",
    });
  });
