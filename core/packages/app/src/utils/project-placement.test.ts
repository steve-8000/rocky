...
describe("project-placement", () => {
  ...
  it("normalizes rocky worktree paths into the parent repo key", () => {
    const placement = deriveProjectPlacementFromCwd("/Users/test/repo/.rocky/worktrees/feature-x");

    expect(placement.projectKey).toBe("/Users/test/repo");
    expect(placement.projectName).toBe("repo");
    expect(placement.checkout.cwd).toBe("/Users/test/repo/.rocky/worktrees/feature-x");
  });
  ...
  it("prefers an existing placement when present", () => {
    const existing = {
      ...
      checkout: {
        ...
        isRockyOwnedWorktree: false as const,
        mainRepoRoot: null,
      },
    };
