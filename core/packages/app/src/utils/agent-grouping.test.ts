  ...
  it("shows owner/repo for GitHub remote keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "remote:github.com/getrocky/rocky",
        projectName: "rocky",
      }),
    ).toBe("getrocky/rocky");
  });
...
describe("groupAgents", () => {
  it("groups active agents by remote URL when available", () => {
    const agents = [
      makeAgent({ id: "a1", cwd: "/Users/me/dev/rocky" }),
      makeAgent({ id: "a2", cwd: "/Users/me/dev/rocky-fix/worktree" }),
    ];

    const { activeGroups } = groupAgents(agents, {
      getRemoteUrl: () => "git@github.com:getrocky/rocky.git",
    });
