...
const ROCKY_WORKTREE_PATH_PATTERN = /(^|\/)\.rocky\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isRockyOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isRockyOwnedWorktreePath(cwd: string): boolean {
  return ROCKY_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
