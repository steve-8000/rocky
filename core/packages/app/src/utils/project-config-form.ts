import type {
  RockyConfigRaw,
  RockyMetadataGeneration,
  RockyMetadataGenerationEntry,
  RockyScriptEntryRaw,
...
export interface ProjectConfigDraft {
  ...
  metadataGenerationBase: RockyMetadataGeneration | undefined;
}
...
export function configToDraft(config: RockyConfigRaw | null | undefined): ProjectConfigDraft {
...
interface ApplyDraftInput {
  ...
  base: RockyConfigRaw | null | undefined;
}

export function applyDraftToConfig(input: ApplyDraftInput): RockyConfigRaw {
  ...
  const nextScripts: Record<string, RockyScriptEntryRaw> = {};
  for (const row of input.draft.scripts) {
    ...
    nextScripts[trimmedName] = nextEntry as RockyScriptEntryRaw;
  }
  ...
  return result as RockyConfigRaw;
}
