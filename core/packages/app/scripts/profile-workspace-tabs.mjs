...
const baseUrl = process.env.ROCKY_PROFILE_APP_URL ?? "http://localhost:19010";
const serverId = requiredEnv("ROCKY_PROFILE_SERVER_ID");
const workspaceId = requiredEnv("ROCKY_PROFILE_WORKSPACE_ID");
const agentId = requiredEnv("ROCKY_PROFILE_AGENT_ID");
const switchCount = Number(process.env.ROCKY_PROFILE_SWITCH_COUNT ?? 6);
const switchWaitMs = Number(process.env.ROCKY_PROFILE_SWITCH_WAIT_MS ?? 250);
const idleWaitMs = Number(process.env.ROCKY_PROFILE_IDLE_WAIT_MS ?? 0);
const dumpCommits = process.env.ROCKY_PROFILE_DUMP_COMMITS === "1";
...
const createdTerminalId = process.env.ROCKY_PROFILE_TERMINAL_ID ? null : createTerminal();
const terminalId = process.env.ROCKY_PROFILE_TERMINAL_ID ?? createdTerminalId;
...
async function main() {
  ...
  try {
    ...
    await page.evaluate(() => globalThis.__ROCKY_RESET_RENDER_PROFILE__?.());
    ...
    const samples = await page.evaluate(() => globalThis.__ROCKY_RENDER_PROFILE__ ?? []);
    const reasons = await page.evaluate(() => globalThis.__ROCKY_RENDER_PROFILE_REASONS__ ?? {});
    console.log(
      JSON.stringify(
        {
          appUrl: baseUrl,
