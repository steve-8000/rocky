...
working with `${ROCKY_MAESTRO_*}` placeholders.
...
The scripts assume a development build with package id `sh.rocky.debug`, an
already-running local daemon on `127.0.0.1:7767`, and a connected Android device
or emulator. They call `adb reverse tcp:7767 tcp:7767`; they do not restart the
daemon.
...
```bash
ROCKY_MAESTRO_APP_ID=sh.rocky.debug
ROCKY_MAESTRO_DIRECT_ENDPOINT=127.0.0.1:7767
ROCKY_MAESTRO_DAEMON_WS_URL=ws://127.0.0.1:7767/ws
ROCKY_MAESTRO_PROJECT_PATH=/path/to/git/repo
```
