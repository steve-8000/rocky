...
describe("connection offer", () => {
  ...
  it("parses connection offers from QR-style URLs", () => {
    ...
    expect(parseConnectionOfferFromUrl(`https://rocky.clab.one/#offer=${encoded}`)).toEqual(offer);
  });
  ...
  it("round-trips relay TLS in offers without rejecting extra relay fields", () => {
    ...
    expect(parseConnectionOfferFromUrl(`https://rocky.clab.one/#offer=${encoded}`)).toEqual({
      v: 2,
      serverId: "server-123",
      daemonPublicKeyB64: "pubkey",
  ...
  it("returns null when the URL has no offer fragment", () => {
    expect(parseConnectionOfferFromUrl("https://rocky.clab.one/pair")).toBeNull();
  });
});
