...
export const DEFAULT_RELAY_ENDPOINT = "relay.invalid:443";
...
export function parseHostPort(input: string): HostPortParts {
  ...
  if (trimmed.startsWith("[")) {
    ...
    if (!match) {
      throw new Error("Invalid host:port (expected [::1]:7767)");
    }
  ...
  const match = trimmed.match(/^(.+):(\d{1,5})$/);
  if (!match) {
    throw new Error("Invalid host:port (expected localhost:7767)");
  }
  const host = match[1].trim();
  if (!host) throw new Error("Host is required");
  const port = parsePort(match[2], "Invalid host:port");
