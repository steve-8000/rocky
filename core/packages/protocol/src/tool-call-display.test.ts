...
describe("shared tool-call display mapping", () => {
  ...
  it("builds display model for worktree setup detail", () => {
    const display = buildToolCallDisplayModel({
      ...
      detail: {
        type: "worktree_setup",
        worktreePath: "/tmp/repo/.rocky/worktrees/repo/branch",
        ...
        commands: [
          {
            index: 1,
            command: "npm install",
            cwd: "/tmp/repo/.rocky/worktrees/repo/branch",
            log: "==> [1/1] Running: npm install\n",
            status: "running",
            exitCode: null,
          },
  ...
  it("humanizes Rocky MCP tool names (Claude Code format)", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__rocky__create_agent",
      status: "running",
      error: null,
  ...
  it("humanizes Rocky MCP tool names (Codex format)", () => {
    const display = buildToolCallDisplayModel({
      name: "rocky.create_agent",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
  ...
  it("humanizes list_agents Rocky tool", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__rocky__list_agents",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
