...
describe("connection URI parsing", () => {
  test("round-trips a tcp host and port", () => {
    const parsed = parseConnectionUri("tcp://localhost:7767");
    ...
    expect(serializeConnectionUri(parsed)).toBe("tcp://localhost:7767");
  });
  ...
  test("round-trips an IPv6 host", () => {
    const parsed = parseConnectionUri("tcp://[::1]:7767?ssl=true");
    ...
    expect(serializeConnectionUri(parsed)).toBe("tcp://[::1]:7767?ssl=true");
  });
  ...
  test("rejects an invalid scheme", () => {
    expect(() => parseConnectionUri("http://localhost:7767")).toThrow(
      "Connection URI protocol must be tcp:",
    );
  });

  test("parses password without including it in the public serializer", () => {
    const parsed = parseConnectionUri("tcp://localhost:7767?ssl=true&password=secret");
    ...
    expect(serializeConnectionUri(parsed)).toBe("tcp://localhost:7767?ssl=true");
    expect(serializeConnectionUriForStorage(parsed)).toBe(
      "tcp://localhost:7767?ssl=true&password=secret",
    );
  });

  test("rejects userinfo passwords", () => {
    expect(() => parseConnectionUri("tcp://:secret@localhost:7767?ssl=true")).toThrow(
...
describe("daemon websocket URLs", () => {
  ...
  test("uses wss for non-443 ports when TLS is enabled", () => {
    expect(buildDaemonWebSocketUrl("example.com:7767", { useTls: true })).toBe(
      "wss://example.com:7767/ws",
    );
  });
});
...
describe("relay websocket URLs", () => {
  ...
  test("uses wss for non-443 ports when TLS is enabled", () => {
    const url = new URL(
      buildRelayWebSocketUrl({
        endpoint: "relay.rocky.clab.one:7767",
        useTls: true,
        serverId: "srv_test",
        role: "client",
      }),
...
describe("shouldUseTlsForDefaultHostedRelay", () => {
  test("returns true for the hosted Rocky relay on port 443", () => {
    expect(shouldUseTlsForDefaultHostedRelay("relay.invalid:443")).toBe(true);
  });
