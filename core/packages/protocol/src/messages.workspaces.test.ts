...
describe("workspace message schemas", () => {
          ...
          scripts: [
            {
              ...
              port: 3000,
              proxyUrl: "http://web.rocky.localhost:7767",
              lifecycle: "running",
              health: "healthy",
            },
          ],
  ...
  test("parses workspace service payloads from old daemons without split proxy URLs", () => {
    ...
    expect(parsed.proxyUrl).toBe("http://web--repo.localhost:7767");
  });
  ...
  test("parses workspace service payloads with split local and public proxy URLs", () => {
    ...
    expect(parsed.localProxyUrl).toBe("http://web--repo.localhost:7767");
          ...
          commands: [
            {
              index: 1,
              command: "npm install",
              cwd: "/repo/.rocky/worktrees/feature-a",
              log: "done",
              status: "completed",
              exitCode: 0,
              durationMs: 100,
