...
ROCKY_HOME=~/.rocky        # Runtime state directory (agents/, etc.)
ROCKY_LISTEN=127.0.0.1:7767  # Listen address (host:port or /path/to/socket)
```

`ROCKY_HOME` defaults to `~/.rocky` and isolates runtime artifacts like `agents/`. `ROCKY_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
ROCKY_HOME=~/.rocky-blue ROCKY_LISTEN=127.0.0.1:7777 npm run dev
```
