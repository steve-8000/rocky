...
import { CheckoutPrStatusResponseSchema } from "@getrocky/protocol/messages";
    ...
    data: {
      repository: {
        pullRequest: {
          ...
          headRepository: {
            sshUrl: "git@github.com:therainisme/rocky.git",
            url: "https://github.com/therainisme/rocky",
          },
        },
      },
    },
...
describe("GitHubService", () => {
  ...
  it("loads pull request checkout target details through GraphQL", async () => {
    ...
    await expect(
      ...
      headRepositorySshUrl: "git@github.com:therainisme/rocky.git",
      headRepositoryUrl: "https://github.com/therainisme/rocky",
      isCrossRepository: true,
    });
    ...
    expect(runner.calls[1]?.args).toContain("owner=getrocky");
    expect(runner.calls[1]?.args).toContain("name=rocky");
  ...
  it("loads GitHub merge, auto-merge, permission, policy, and queue facts for PR 993 shape", async () => {
    const runner = createScriptedRunner([
      currentPullRequestJson({
        number: 993,
        url: "https://github.com/getrocky/rocky/pull/993",
        title: "Auto-merge UX",
        ...
        statusCheckRollup: [
          {
            ...
            status: "IN_PROGRESS",
            conclusion: null,
            detailsUrl: "https://github.com/getrocky/rocky/actions/runs/993",
          },
        ],
      }),
      currentPullRequestGithubFactsJson(),
      ...
      checks: [
        {
          name: "server tests",
          status: "pending",
          url: "https://github.com/getrocky/rocky/actions/runs/993",
          workflow: "CI",
        },
      ],
  ...
  it("treats a GitHub issue or PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.com/getrocky/rocky/pull/793",
      limit: 5,
    });
  ...
  it("resolves GitHub repos from the origin remote URL", async () => {
    ...
    try {
      ...
      execFileSync("git", ["remote", "add", "origin", "git@github.com:getrocky/rocky.git"], {
        cwd,
        stdio: "ignore",
      });

      await expect(resolveGitHubRepo(cwd)).resolves.toBe("getrocky/rocky");
    } finally {
