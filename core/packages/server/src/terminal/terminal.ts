...
import type { TerminalCell, TerminalState } from "@getrocky/protocol/messages";
import { TerminalInputModeTracker } from "@getrocky/protocol/terminal-input-mode";
...
function resolveZshShellIntegrationRuntimeDir(): string {
  ...
  return join(tmpdir(), `${username}-rocky-zsh`);
}
...
function prepareZshShellIntegrationRuntimeDir(sourceDir = resolveZshShellIntegrationDir()): string {
  ...
  writePrivateFileAtomicSync(
    join(runtimeDir, "rocky-integration.zsh"),
    readFileSync(join(readableSourceDir, "rocky-integration.zsh")),
  );
  return runtimeDir;
}
...
export function buildTerminalEnvironment(
  ...
  return {
    ...baseEnv,
    ROCKY_ZSH_ZDOTDIR: originalZdotdir,
