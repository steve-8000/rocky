...
import { buildSelfNodeCommand } from "../server/rocky-env.js";
...
describe("execCommand", () => {
  ...
  test("treats env as the replacement base and finalizes external command env", async () => {
    ...
    expect(parsePrintedEnv(result.stdout)).toEqual({
      ...
      ROCKY_DESKTOP_MANAGED: null,
      ROCKY_NODE_ENV: null,
      ROCKY_SUPERVISED: null,
    });
  });

  test("does not inherit process.env when env replacement is supplied", async () => {
    process.env.ROCKY_TEST_SHOULD_NOT_LEAK = "leaked";
    try {
      const result = await execCommand(
        process.execPath,
        [
          "-e",
          "console.log(JSON.stringify({ leaked: process.env.ROCKY_TEST_SHOULD_NOT_LEAK ?? null }))",
        ],
        {
          env: {
            PATH: process.env.PATH,
      ...
      delete process.env.ROCKY_TEST_SHOULD_NOT_LEAK;
    }
  });
  ...
  test("internal env mode preserves Rocky-owned launcher env", async () => {
    const result = await execCommand(process.execPath, ["-e", printEnvScript], {
      envMode: "internal",
      baseEnv: {
