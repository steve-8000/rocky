...
describe("rocky worktree manager", () => {
  ...
  it("treats a worktree as rocky-owned even when its .git admin is missing", async () => {
    ...
    const ownership = await isRockyOwnedWorktreeCwd(created.worktreePath, { rockyHome });
    expect(ownership.allowed).toBe(true);
  });

  it("rejects paths that are not under the rocky worktrees root", async () => {
    const outsidePath = join(tempDir, "outside-rocky-home");
    mkdirSync(outsidePath, { recursive: true });

    const ownership = await isRockyOwnedWorktreeCwd(outsidePath, { rockyHome });
  ...
  it("rejects the worktrees root itself and the per-repo hash dir", async () => {
    ...
    const worktreesRoot = join(rockyHome, "worktrees");
    const projectHashDir = join(worktreesRoot, projectHash);
    mkdirSync(projectHashDir, { recursive: true });

    await expect(isRockyOwnedWorktreeCwd(worktreesRoot, { rockyHome })).resolves.toMatchObject({
      allowed: false,
    });
    await expect(isRockyOwnedWorktreeCwd(projectHashDir, { rockyHome })).resolves.toMatchObject({
      allowed: false,
    });
  });
