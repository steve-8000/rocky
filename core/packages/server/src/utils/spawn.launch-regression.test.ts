...
const expected = JSON.parse(process.env.ROCKY_EXPECTED_ARGV_JSON);
const sliceFrom = process.env.ROCKY_ARGV_SLICE_FROM ? Number(process.env.ROCKY_ARGV_SLICE_FROM) : 2;
...
function makeFixture(): {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "rocky spawn regression "));
  tempDirs.push(root);

  const fakeDaemonNode = path.join(root, "Fake Rocky.exe");
...
async function runFixture(params: {
  ...
  const child = spawnProcess(params.command, params.args, {
    env: {
      ...process.env,
      ROCKY_EXPECTED_ARGV_JSON: JSON.stringify(["--config", JSON_ARG]),
    },
...
function makeLaunchFixture(ext: "exe" | "cmd" | "bat"): LaunchFixture {
  const root = mkdtempSync(path.join(tmpdir(), `rocky-launch-${ext}-`));
  tempDirs.push(root);

  // Unique base name so a globally installed binary cannot satisfy findExecutable.
  const command = `rocky-launch-fake-${path.basename(root)}`;
...
async function findAndLaunch(fixture: LaunchFixture): Promise<{
  ...
  return withPathPrepended(fixture.root, async () => {
    ...
    const child = spawnProcess(found, fixture.args, {
      env: {
        ...process.env,
        ROCKY_EXPECTED_ARGV_JSON: fixture.expectedArgvJson,
        ROCKY_ARGV_SLICE_FROM: String(fixture.sliceFrom),
      },
      stdio: ["ignore", "pipe", "pipe"],
    });
