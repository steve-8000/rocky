...
import { isRockyOwnedWorktreeCwd, resolveRockyWorktreesBaseRoot } from "./worktree.js";
import { readRockyWorktreeMetadata } from "./worktree-metadata.js";
...
export interface CheckoutStatusGitNonRocky {
  isGit: true;
  repoRoot: string;
...
export interface CheckoutStatusGitRocky {
  isGit: true;
  repoRoot: string;
  mainRepoRoot: string;
  currentBranch: string | null;
...
export type CheckoutStatusGit = CheckoutStatusGitNonRocky | CheckoutStatusGitRocky;
...
export type CheckoutSnapshotFacts =
  ...
  | {
      ...
      rockyWorktree: RockyWorktreeForCwd;
...
async function getMainRepoRootFromCommonDir(
  ...
  const nonBareNonRocky = worktrees.filter(
    (wt) =>
      !wt.isBare &&
      !isRockyWorktreePath(wt.path, {
        rockyHome: context?.rockyHome,
        worktreesRoot: context?.worktreesRoot,
      }),
  );
  const childrenOfBareRepo = nonBareNonRocky.filter((wt) => isDescendantPath(wt.path, normalized));
  const mainChild = childrenOfBareRepo.find((wt) => basename(wt.path) === "main");
  return mainChild?.path ?? childrenOfBareRepo[0]?.path ?? nonBareNonRocky[0]?.path ?? normalized;
}
...
/** Check whether a path is under Rocky's worktree root. */
export function isRockyWorktreePath(
  p: string,
  options?: { rockyHome?: string; worktreesRoot?: string },
): boolean {
  if (options?.worktreesRoot || options?.rockyHome) {
    return isDescendantPath(p, resolveRockyWorktreesBaseRoot(options));
  }
  return /[/\\]\.rocky[/\\]worktrees[/\\]/.test(p);
}
...
type RockyWorktreeForCwd =
  | { isRockyOwnedWorktree: false }
  | { isRockyOwnedWorktree: true; worktreeRoot: string };

async function getRockyWorktreeForCwd(
  cwd: string,
  context?: CheckoutContext,
  knownWorktreeRoot?: string | null,
): Promise<RockyWorktreeForCwd> {
  // Fast-path reject: non-worktree paths do not need expensive ownership checks.
  if (!/[\\/]worktrees[\\/]/.test(cwd)) {
    return { isRockyOwnedWorktree: false };
  }

  const ownership = await isRockyOwnedWorktreeCwd(cwd, {
    rockyHome: context?.rockyHome,
    worktreesRoot: context?.worktreesRoot,
  });
...
function readRockyWorktreeBaseRef(worktreeRoot: string): string | null {
  return readRockyWorktreeMetadata(worktreeRoot)?.baseRefName ?? null;
}
...
async function getStoredBaseRefForCwd(
  ...
  const rockyWorktree = await getRockyWorktreeForCwd(cwd, context);
  if (!rockyWorktree.isRockyOwnedWorktree) {
    return null;
  }

  return readRockyWorktreeBaseRef(rockyWorktree.worktreeRoot);
}
...
async function resolvePullRequestStatusLookupTarget(
  ...
  if (!remoteName?.startsWith("rocky-pr-")) {
...
async function inspectCheckoutContext(
  ...
  try {
    ...
    const [currentBranch, remoteUrl, absoluteGitDir, gitCommonDir, rockyWorktree] =
      await Promise.all([
        getCurrentBranch(cwd),
        getOriginRemoteUrl(cwd),
        resolveAbsoluteGitDir(cwd),
        resolveGitCommonDir(cwd),
        getRockyWorktreeForCwd(cwd, context, root),
      ]);
...
function buildPullRequestLookupTargetFromBranchConfig(input: {
  ...
  if (!input.branchRemoteName?.startsWith("rocky-pr-")) {
...
export async function getCheckoutSnapshotFacts(
  ...
  const storedBaseRef = inspected.rockyWorktree.isRockyOwnedWorktree
    ? readRockyWorktreeBaseRef(inspected.rockyWorktree.worktreeRoot)
    : null;
  ...
  if (inspected.remoteUrl && inspected.currentBranch) {
    ...
    if (branchRemoteName) {
      ...
      if (branchRemoteName.startsWith("rocky-pr-")) {
  ...
  return {
    ...
    rockyWorktree: inspected.rockyWorktree,
    storedBaseRef,
    resolvedBaseRef,
    mainRepoRoot,
    comparisonBaseRef,
...
export async function getCheckoutStatus(
  ...
  const rockyWorktree = facts.rockyWorktree;
  ...
  if (rockyWorktree.isRockyOwnedWorktree && baseRef) {
    return {
      isGit: true,
