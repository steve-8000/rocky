...
describe("searchHomeDirectories", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(homeDir, "projects", "rocky"), { recursive: true });
  ...
  it("returns only existing directories", async () => {
    ...
    expect(resolvedResults).toContain(realpathSync.native(path.join(homeDir, "projects", "rocky")));
  ...
  it("supports home-relative path query syntax", async () => {
    ...
    expect(results.map((result) => realpathSync.native(result))).toEqual([
      realpathSync.native(path.join(homeDir, "projects", "rocky")),
    ]);
  });
...
describe("searchWorkspaceEntries", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(path.join(workspaceDir, "README.md"), "# rocky\n");
    writeFileSync(
  ...
  it("suffix mode matches whole path segment suffixes without fuzzy matches", async () => {
    ...
    writeFileSync(path.join(workspaceDir, "src", "rocky-config-file.ts"), "");
