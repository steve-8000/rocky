...
function createGitHubServiceForStatus(
  ...
  return {
    ...
    getPullRequest: async () => ({
      number: 1,
      title: "PR",
      url: "https://github.com/getrocky/rocky/pull/1",
      state: "OPEN",
      body: null,
      baseRefName: "main",
      headRefName: "feature",
...
function createPullRequestStatus(overrides?: Partial<GitHubCurrentPullRequestStatus>) {
  return {
    url: "https://github.com/getrocky/rocky/pull/123",
    title: "Ship feature",
    state: "open",
    baseRefName: "main",
    headRefName: "feature",
...
describe("checkout git utilities", () => {
  ...
  it("reuses checkout snapshot facts across status, shortstat, and PR status reads", async () => {
    ...
    const facts = await getCheckoutSnapshotFacts(repoDir, { rockyHome });
    const status = await getCheckoutStatus(repoDir, { rockyHome, facts });
    const shortstat = await getCheckoutShortstat(repoDir, { rockyHome, facts }, { force: true });
    ...
    const statusWithFacts = await getCheckoutStatus(repoDir, { rockyHome, facts });
...
`;
  ...
  it("returns checkout root metadata for normal repos", async () => {
    ...
    expect(status.isRockyOwnedWorktree).toBe(false);
  ...
  it("reports a PR worktree as not ahead when its branch is pushed to the configured PR remote", async () => {
    ...
    commitFile(repoDir, "feature.txt", "feature\n", "feature commit");
    execFileSync("git", ["remote", "add", "rocky-pr-1285", prRemoteDir], { cwd: repoDir });
    execFileSync(
      "git",
      ["push", "rocky-pr-1285", "HEAD:refs/heads/open-button-targets-active-file"],
      { cwd: repoDir },
    );
    execFileSync(
      "git",
      ["config", "branch.aaronzhongg/open-button-targets-active-file.remote", "rocky-pr-1285"],
      {
        cwd: repoDir,
      },
    );
  ...
  it("reports a PR worktree as behind when its configured PR remote has newer commits", async () => {
    ...
    execFileSync("git", ["fetch", "rocky-pr-1285"], { cwd: repoDir });
  ...
  it("does not report full history as unpushed for fresh no-track Rocky worktrees", async () => {
    setupRemoteTrackingMain(repoDir, tempDir);
    ...
    const status = await getCheckoutStatus(worktree.worktreePath, { rockyHome });
    expect(status).toMatchObject({
      isGit: true,
  ...
  it("handles status/diff/commit in a .rocky worktree", async () => {
    ...
    const status = await getCheckoutStatus(result.worktreePath, { rockyHome });
    expect(status.isGit).toBe(true);
    expect(realpathSync.native(status.repoRoot)).toBe(realpathSync.native(result.worktreePath));
    expect(status.isDirty).toBe(true);
    expect(status.isRockyOwnedWorktree).toBe(true);
    expect(realpathSync.native(status.mainRepoRoot ?? "")).toBe(realpathSync.native(repoDir));

    const diff = await getCheckoutDiff(result.worktreePath, { mode: "uncommitted" }, { rockyHome });
    expect(diff.diff).toContain("-hello");
    expect(diff.diff).toContain("+worktree change");

    await commitAll(result.worktreePath, "worktree update");

    const cleanStatus = await getCheckoutStatus(result.worktreePath, { rockyHome });
    expect(cleanStatus.isDirty).toBe(false);
  ...
  it("returns checkout root metadata for .rocky worktrees", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("detects plain git worktrees from git alone", async () => {
    ...
    const status = await getCheckoutStatus(worktreeDir, { rockyHome });
    expect(status.isGit).toBe(true);
  ...
  it("merges the current branch into base from a worktree checkout", async () => {
    ...
    await mergeToBase(worktree.worktreePath, { baseRef: "main" }, { rockyHome });
    ...
    const statusAfterMerge = await getCheckoutStatus(worktree.worktreePath, { rockyHome });
  ...
  it("reports the base worktree cwd when merge-to-base mutates a separate checkout", async () => {
    ...
    const mutatedCwd = await mergeToBase(featureWorktree.worktreePath, {}, { rockyHome });
  ...
  it("disables GitHub features when gh is unavailable", async () => {
    execFileSync("git", ["remote", "add", "origin", "https://github.com/getrocky/rocky.git"], {
      cwd: repoDir,
    });
          ...
          checks: [
            {
              name: "server-tests",
              status: "success",
              url: "https://github.com/getrocky/rocky/actions/runs/123",
              workflow: "Server CI",
              duration: "2m 14s",
            },
          ],
  ...
  it("uses the tracked fork branch for PR worktree status lookup", async () => {
    ...
    execFileSync("git", ["remote", "add", "rocky-pr-345", "git@github.com:chethanuk/rocky.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "branch.chethanuk/main.remote", "rocky-pr-345"], {
      cwd: repoDir,
    });
    ...
    const github = createGitHubServiceForStatus(
      createPullRequestStatus({
        number: 345,
        url: "https://github.com/getrocky/rocky/pull/345",
        headRefName: "main",
      }),
      {
        onStatus: () => {},
  ...
  it("returns closed-unmerged PR status without marking it as merged", async () => {
    ...
    const status = await getPullRequestStatus(
      ...
      createGitHubServiceForStatus(
        createPullRequestStatus({
          url: "https://github.com/getrocky/rocky/pull/999",
  ...
  it("expires cached PR status after the TTL", async () => {
    ...
    try {
      ...
      github.getCurrentPullRequestStatus = async () => {
        callCount += 1;
        return createPullRequestStatus({
          url: `https://github.com/getrocky/rocky/pull/${callCount}`,
        });
      };
      const first = await getPullRequestStatus(repoDir, github);
      await sleep(80);
  ...
  it("uses stored baseRefName for Rocky worktrees (no heuristics)", async () => {
    ...
    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { rockyHome });
    expect(baseDiff.diff).toContain("feature.txt");
    expect(baseDiff.diff).not.toContain("file.txt");
  });

  it("excludes dirty working tree changes from Rocky worktree base diffs", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("merges to stored baseRefName when baseRef is not provided", async () => {
    ...
    // Create a Rocky worktree configured to use develop as base.
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "develop",
    ...
    await mergeToBase(worktree.worktreePath, {}, { rockyHome });
  ...
  it("falls back to the repository default branch for base-dependent operations when metadata is missing", async () => {
    ...
    const metadataPath = getRockyWorktreeMetadataPath(worktree.worktreePath);
    rmSync(metadataPath, { force: true });

    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { rockyHome });
    expect(baseDiff.diff).toContain("feature.txt");

    const shortstat = await getCheckoutShortstat(worktree.worktreePath, { rockyHome });
    expect(shortstat).toEqual({ additions: 1, deletions: 0 });
  });

  it("falls back to plain git checkout status when Rocky worktree metadata is missing", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  describe("parseWorktreeList", () => {
    it("parses porcelain worktree output", () => {
      const output = [
        ...
        "",
        "worktree /home/user/.rocky/worktrees/feature",
      ...
      expect(entries[1]).toEqual({
        path: "/home/user/.rocky/worktrees/feature",
  ...
  describe("isRockyWorktreePath", () => {
    it("matches Unix .rocky/worktrees/ paths", () => {
      expect(isRockyWorktreePath("/home/user/.rocky/worktrees/feature")).toBe(true);
    });

    it("matches Windows .rocky\\worktrees\\ paths", () => {
      expect(isRockyWorktreePath("C:\\Users\\dev\\.rocky\\worktrees\\feature")).toBe(true);
    });

    it("matches worktrees under a custom ROCKY_HOME", () => {
      const customRockyHome = process.platform === "win32" ? "C:\\rocky" : "/var/lib/rocky";
      const worktreePath =
        process.platform === "win32"
          ? win32.join(customRockyHome, "worktrees", "project", "feature")
          : `${customRockyHome}/worktrees/project/feature`;

      expect(
        isRockyWorktreePath(worktreePath, {
          rockyHome: customRockyHome,
        }),
      ).toBe(true);
    });

    it("rejects paths without .rocky/worktrees segment", () => {
      expect(isRockyWorktreePath("/home/user/repo")).toBe(false);
      expect(isRockyWorktreePath("C:\\Users\\dev\\repo")).toBe(false);
    });
  });
