...
import { readRockyConfigJson, resolveRockyConfigPath } from "./rocky-config-file.js";
export {
  RockyConfigRawSchema,
  RockyLifecycleCommandRawSchema,
  RockyScriptEntryRawSchema,
  RockyWorktreeConfigRawSchema,
  RockyConfigSchema,
  type RockyConfig,
  type RockyConfigRaw,
} from "@getrocky/protocol/rocky-config-schema";
import { RockyConfigSchema, type RockyConfig } from "@getrocky/protocol/rocky-config-schema";
import {
  normalizeBaseRefName,
  readRockyWorktreeMetadata,
  readRockyWorktreeRuntimePort,
  writeRockyWorktreeMetadata,
  writeRockyWorktreeRuntimeMetadata,
} from "./worktree-metadata.js";
import { runGitCommand } from "./run-git-command.js";
import { spawnProcess } from "./spawn.js";
import { resolveRockyHome } from "../server/rocky-home.js";
import { createExternalProcessEnv } from "../server/rocky-env.js";
import { parseGitRevParsePath, resolveGitRevParsePath } from "./git-rev-parse-path.js";
import { validateBranchSlug } from "@getrocky/protocol/branch-slug";
import { expandTilde } from "./path.js";

export { slugify, validateBranchSlug } from "@getrocky/protocol/branch-slug";
...
export interface WorktreeRuntimeEnv {
  [key: string]: string;
  ROCKY_SOURCE_CHECKOUT_PATH: string;
  ROCKY_ROOT_PATH: string;
  ROCKY_WORKTREE_PATH: string;
  ROCKY_BRANCH_NAME: string;
  ROCKY_WORKTREE_PORT: string;
}
...
export interface RockyWorktreeInfo {
  path: string;
  createdAt: string;
  branchName?: string;
  head?: string;
...
export interface RockyWorktreeOwnership {
  allowed: boolean;
  repoRoot?: string;
  worktreeRoot?: string;
  worktreePath?: string;
...
export type ReadRockyConfigResult =
  | { ok: true; config: RockyConfig | null }
  | { ok: false; configPath: string; error: unknown };

export function readRockyConfig(repoRoot: string): ReadRockyConfigResult {
  try {
    const json = readRockyConfigJson(repoRoot);
    if (json === null) {
      return { ok: true, config: null };
    }
    return { ok: true, config: RockyConfigSchema.parse(json) };
  } catch (error) {
    return { ok: false, configPath: resolveRockyConfigPath(repoRoot), error };
  }
}

export function rockyConfigParseError(failure: { configPath: string; error: unknown }): Error {
  const detail = failure.error instanceof Error ? failure.error.message : String(failure.error);
  return new Error(`Failed to parse rocky.json at ${failure.configPath}: ${detail}`, {
    cause: failure.error,
  });
}

function readRockyConfigOrThrow(repoRoot: string): RockyConfig | null {
  const result = readRockyConfig(repoRoot);
  if (!result.ok) {
    throw rockyConfigParseError(result);
  }
  return result.config;
}

export function getWorktreeSetupCommands(repoRoot: string): string[] {
  return readRockyConfigOrThrow(repoRoot)?.worktree?.setup ?? [];
}

export function getWorktreeTeardownCommands(repoRoot: string): string[] {
  return readRockyConfigOrThrow(repoRoot)?.worktree?.teardown ?? [];
}

export function getWorktreeTerminalSpecs(repoRoot: string): WorktreeTerminalConfig[] {
  const terminals = readRockyConfigOrThrow(repoRoot)?.worktree?.terminals;
...
export function getScriptConfigs(config: RockyConfig | null): Map<string, ScriptConfig> {
...
export async function runWorktreeSetupCommands(options: {
  ...
  // Read rocky.json from the worktree (it will have the same content as the source repo)
...
export async function resolveWorktreeRuntimeEnv(options: {
  ...
  let worktreePort = readRockyWorktreeRuntimePort(options.worktreePath);
  if (worktreePort === null) {
    worktreePort = await getAvailablePort();
    const metadata = readRockyWorktreeMetadata(options.worktreePath);
    if (metadata) {
      writeRockyWorktreeRuntimeMetadata(options.worktreePath, { worktreePort });
    }
  } else {
  ...
  return {
    ...
    // source checkout.
    ROCKY_SOURCE_CHECKOUT_PATH: repoRootPath,
    // Backward-compatible alias.
    ROCKY_ROOT_PATH: repoRootPath,
    ROCKY_WORKTREE_PATH: options.worktreePath,
    ROCKY_BRANCH_NAME: branchName,
    ROCKY_WORKTREE_PORT: String(worktreePort),
  };
}
...
export async function runWorktreeTeardownCommands(options: {
  ...
  const branchName =
    options.branchName ?? (await resolveBranchNameForWorktreePath(options.worktreePath));
  const worktreePort = readRockyWorktreeRuntimePort(options.worktreePath);
  ...
  const teardownEnv: NodeJS.ProcessEnv = createExternalProcessEnv(process.env, {
    ...
    ROCKY_WORKTREE_PATH: options.worktreePath,
    ROCKY_BRANCH_NAME: branchName,
    ...(worktreePort !== null ? { ROCKY_WORKTREE_PORT: String(worktreePort) } : {}),
  });
...
export function resolveRockyWorktreesBaseRoot(options?: WorktreeRootOptions): string {
  if (options?.worktreesRoot) {
    ...
    const home = options.rockyHome ? resolve(options.rockyHome) : resolveRockyHome();
...
export async function getRockyWorktreesRoot(
  cwd: string,
  rockyHome?: string,
  worktreesRoot?: string,
): Promise<string> {
  const baseRoot = resolveRockyWorktreesBaseRoot({ rockyHome, worktreesRoot });
...
export async function computeWorktreePath(
  ...
  const projectWorktreesRoot = await getRockyWorktreesRoot(cwd, rockyHome, worktreesRoot);
...
export async function isRockyOwnedWorktreeCwd(
  cwd: string,
  options?: WorktreeRootOptions,
): Promise<RockyWorktreeOwnership> {
  ...
  const worktreesBaseRoot = resolveRockyWorktreesBaseRoot(options);
  const rockyWorktreesPrefix = normalizePathForOwnership(worktreesBaseRoot) + sep;

  // Ownership is defined by the path living under <worktrees-root>/<hash>/<slug>[/...].
  // The <hash>/<slug> prefix is Rocky-private — nothing else writes there — so the
  // path shape alone is sufficient proof of ownership, even when git has already
  // forgotten about the worktree.
  if (!resolvedCwd.startsWith(rockyWorktreesPrefix)) {
    return {
      allowed: false,
  ...
  const relative = resolvedCwd.slice(rockyWorktreesPrefix.length);
...
type ParsedRockyWorktreeInfo = Omit<RockyWorktreeInfo, "createdAt">;

function parseWorktreeList(output: string): ParsedRockyWorktreeInfo[] {
  const entries: ParsedRockyWorktreeInfo[] = [];
  let current: ParsedRockyWorktreeInfo | null = null;
...
export async function listRockyWorktrees({
  cwd,
  rockyHome,
  worktreesRoot,
}: {
...
export async function resolveExistingWorktreeForSlug({
  ...
  const worktrees = await listRockyWorktrees({
    cwd: repoRoot,
    rockyHome,
    worktreesRoot,
  });
...
export async function resolveRockyWorktreeRootForCwd(
  cwd: string,
  options?: WorktreeRootOptions,
  ...
  const knownWorktrees = await listRockyWorktrees({
    cwd,
...
export async function deleteRockyWorktree({
  cwd,
  worktreePath,
  worktreeSlug,
  worktreesRoot,
  ...
  if (worktreesRoot) {
    ...
    resolvedWorktreesRoot = await getRockyWorktreesRoot(cwd, rockyHome, worktreesBaseRoot);
  } else {
    throw new Error("cwd or worktreesRoot is required to delete a Rocky worktree");
  }
  ...
  const resolvedWorktree =
    (
      await resolveRockyWorktreeRootForCwd(requestedPath, {
        rockyHome,
  ...
  if (!resolvedWorktree.startsWith(resolvedRoot)) {
    throw new Error("Refusing to delete non-Rocky worktree");
  }
...
export const createWorktree = async ({
  ...
  let worktreePath = join(await getRockyWorktreesRoot(cwd, rockyHome, worktreesRoot), worktreeSlug);
  ...
  writeRockyWorktreeMetadata(worktreePath, { baseRefName: sourcePlan.metadataBaseRefName });

  // If rocky.json exists in the main repo but wasn't checked into the worktree
  ...
  const mainConfigPath = join(cwd, "rocky.json");
  const worktreeConfigPath = join(worktreePath, "rocky.json");
  try {
    ...
    case "checkout-github-pr": {
      ...
      return {
        ...
        ...(source.pushRemoteUrl
          ? {
              pushRemote: {
                name: `rocky-pr-${source.githubPrNumber}`,
...
function normalizeRequiredBaseBranch(baseBranch: string): string {
  ...
  if (!normalizedBaseBranch) {
    throw new Error("Base branch is required when creating a Rocky worktree");
  }
  if (normalizedBaseBranch === "HEAD") {
    throw new Error("Base branch cannot be HEAD when creating a Rocky worktree");
  }
