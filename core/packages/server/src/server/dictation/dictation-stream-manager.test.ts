...
describe("DictationStreamManager (finish buffer-too-small tolerance)", () => {
  const env = {
    dictationDebug: process.env.ROCKY_DICTATION_DEBUG,
  };

  beforeEach(() => {
    vi.useFakeTimers();
    process.env.ROCKY_DICTATION_DEBUG = "false";
  });

  afterEach(() => {
    vi.useRealTimers();
    process.env.ROCKY_DICTATION_DEBUG = env.dictationDebug;
  });
...
describe("DictationStreamManager (provider-agnostic provider)", () => {
  ...
  it("uses ROCKY_DICTATION_LANGUAGE when set", async () => {
    const sttProvider = await startWithResolvedDictationLanguage({
      env: {
        ROCKY_DICTATION_LANGUAGE: "pt",
  ...
  it("treats empty ROCKY_DICTATION_LANGUAGE as unset", async () => {
    const sttProvider = await startWithResolvedDictationLanguage({
      env: {
        ROCKY_DICTATION_LANGUAGE: "  ",
      } as NodeJS.ProcessEnv,
    });
  ...
  it("auto-commits while streaming and assembles final transcript in segment order", async () => {
    const originalDebug = process.env.ROCKY_DICTATION_DEBUG;
    process.env.ROCKY_DICTATION_DEBUG = "false";
    ...
    try {
      ...
      if (originalDebug === undefined) {
        delete process.env.ROCKY_DICTATION_DEBUG;
      } else {
        process.env.ROCKY_DICTATION_DEBUG = originalDebug;
      }
    }
  });
  ...
  it("drops dangling uncommitted non-final transcripts when finishing after silence tail clear", async () => {
    vi.useFakeTimers();
    const previousDebug = process.env.ROCKY_DICTATION_DEBUG;
    process.env.ROCKY_DICTATION_DEBUG = "false";
    try {
      const session = new FakeRealtimeSession();
      const emitted: Array<{ type: string; payload: unknown }> = [];
      ...
      process.env.ROCKY_DICTATION_DEBUG = previousDebug;
      vi.useRealTimers();
    }
  });
});
