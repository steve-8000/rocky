...
export async function generateLocalPairingOffer(args: {
  ...
  const relayEndpoint = args.relayEndpoint ?? "relay.invalid:443";
  const relayPublicEndpoint = args.relayPublicEndpoint ?? relayEndpoint;
  const relayUseTls = args.relayUseTls ?? relayEndpoint === "relay.invalid:443";
  const relayPublicUseTls = args.relayPublicUseTls ?? relayUseTls;
  const appBaseUrl = args.appBaseUrl ?? "https://rocky.clab.one";
  const serverId = getOrCreateServerId(args.rockyHome, { logger: args.logger });
  const daemonKeyPair = await loadOrCreateDaemonKeyPair(args.rockyHome, args.logger);
