...
const RUN = process.env.ROCKY_SPEECH_E2E_DOWNLOAD === "1";
...
downloadTest(
  ...
  async () => {
    ...
    const rockyHomeRoot = mkdtempSync(path.join(tmpdir(), "rocky-speech-download-"));
    const modelsDir = path.join(rockyHomeRoot, ".rocky", "models", "local-speech");
