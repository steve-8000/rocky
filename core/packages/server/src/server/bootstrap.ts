...
import { createRockyWorktree as createRegisteredRockyWorktree } from "./rocky-worktree-service.js";
import { createRockyWorktreeWorkflow } from "./worktree-session.js";
...
export type RockyOpenAIConfig = OpenAiSpeechProviderConfig;
export type RockyLocalSpeechConfig = LocalSpeechProviderConfig;

export interface RockySpeechSttLanguages {
  dictation: string;
  voice: string;
}

export interface RockySpeechConfig {
  providers: RequestedSpeechProviders;
  sttLanguages?: RockySpeechSttLanguages;
  local?: RockyLocalSpeechConfig;
}
...
export interface RockyDaemonConfig {
  listen: string;
  rockyHome: string;
...
export async function createRockyDaemon(
  config: RockyDaemonConfig,
  rootLogger: Logger,
): Promise<RockyDaemon> {
  ...
  const serverId = getOrCreateServerId(config.rockyHome, { logger });
  // Rocky: boot-scoped secret for daemon→self MCP calls from injected agents.
  const internalMcpToken = randomUUID();
  const daemonKeyPair = await loadOrCreateDaemonKeyPair(config.rockyHome, logger);
  ...
  const allowedOrigins = new Set([
    ...
    // Packaged desktop renderers use the custom rocky:// protocol scheme.
    "rocky://app",
  ...
  const projectRegistry = new FileBackedProjectRegistry(
    path.join(config.rockyHome, "projects", "projects.json"),
    logger,
  );
  workspaceRegistry = new FileBackedWorkspaceRegistry(
    path.join(config.rockyHome, "projects", "workspaces.json"),
    logger,
  );
  ...
  if (mcpEnabled) {
    ...
    const createAgentMcpTransport = async (callerAgentId?: string) => {
      const agentMcpServer = await createAgentMcpServer({
        ...
        markWorkspaceArchiving: markWorkspaceArchivingExternal,
        clearWorkspaceArchiving: clearWorkspaceArchivingExternal,
        createRockyWorktree: async (input, serviceOptions) => {
          return createRockyWorktreeWorkflow(
            {
              rockyHome: config.rockyHome,
              worktreesRoot: config.worktreesRoot,
              createRockyWorktree: async (workflowInput, workflowOptions) => {
                return createRegisteredRockyWorktree(workflowInput, {
                  github,
        ...
        const onListening = () => {
          ...
          const logAndResolve = async () => {
            ...
            if (mcpBaseUrl && config.auth?.password) {
              ...
              mcpBaseUrl = `${mcpBaseUrl}?rockyToken=${internalMcpToken}`;
            }
            ...
            const relayEndpoint = config.relayEndpoint ?? "relay.invalid:443";
            const relayPublicEndpoint = config.relayPublicEndpoint ?? relayEndpoint;
            const relayUseTls = config.relayUseTls ?? relayEndpoint === "relay.invalid:443";
            const relayPublicUseTls = config.relayPublicUseTls ?? relayUseTls;
            const appBaseUrl = config.appBaseUrl ?? "https://rocky.clab.one";
