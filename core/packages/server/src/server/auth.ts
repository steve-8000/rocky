...
export function extractWsBearerProtocol(value: string | undefined): string | null {
  ...
  for (const protocol of value.split(",")) {
    ...
    if (segments[0] === "rocky" && segments[1] === "bearer" && segments.length >= 3) {
      return trimmed;
    }
  }
...
export function createRequireBearerMiddleware(
  ...
  return (req, res, next) => {
    ...
    // Rocky: agent-injected Rocky MCP clients authenticate with a boot-scoped
    // internal token carried in the URL (?rockyToken=). Only honored under
    ...
    if (
      ...
      typeof req.query.rockyToken === "string" &&
      timingSafeTokenEqual(req.query.rockyToken, internalToken)
    ) {
      next();
      return;
    }
