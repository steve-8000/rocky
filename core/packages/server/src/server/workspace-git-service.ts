...
import type { ProjectCheckoutLitePayload } from "@getrocky/protocol/messages";
...
import { listRockyWorktrees, type RockyWorktreeInfo } from "../utils/worktree.js";
...
const linuxWatchReaddirConcurrency =
  parseInt(process.env.ROCKY_LINUX_WATCH_READDIR_CONCURRENCY ?? "16", 10) || 16;
...
export type WorkspaceGitWorktreeInfo = RockyWorktreeInfo;
...
interface WorkspaceGitServiceDependencies {
  ...
  listRockyWorktrees: typeof listRockyWorktrees;
...
export class WorkspaceGitServiceImpl implements WorkspaceGitService {
  ...
  constructor(options: WorkspaceGitServiceOptions) {
    ...
    this.rockyHome = options.rockyHome;
  ...
  async getCheckout(cwd: string): Promise<ProjectCheckoutLitePayload> {
    ...
    try {
      ...
      return checkoutLiteFromGitSnapshot(normalizedCwd, {
        ...
        isRockyOwnedWorktree: status.isRockyOwnedWorktree,
  ...
  listStashes(
    ...
    const normalizedCwd = normalizeWorkspaceId(cwd);
    const rockyOnly = options?.rockyOnly !== false;
    const key = JSON.stringify(["stashes", normalizedCwd, rockyOnly]);
    return this.readAuxiliaryCache(this.stashListCache, key, readOptions, async () => {
      ...
      return parseWorkspaceGitStashList(stdout, { rockyOnly });
    });
  }
  ...
  async resolveRepoRoot(cwd: string, options?: WorkspaceGitReadOptions): Promise<string> {
    ...
    return snapshot.git.isRockyOwnedWorktree
  ...
  private async refreshGitSnapshot(
    ...
    target.latestGit = {
      ...
      isRockyOwnedWorktree: checkoutStatus.isRockyOwnedWorktree,
...
function parseWorkspaceGitStashList(
  ...
  for (const line of lines) {
    ...
    const prefix = "rocky-auto-stash:";
    const prefixIdx = subject.indexOf(prefix);
    const isRocky = prefixIdx >= 0;
    const branch = isRocky ? subject.slice(prefixIdx + prefix.length).trim() || null : null;

    if (options.rockyOnly && !isRocky) {
      continue;
    }

    entries.push({ index, message: subject, branch, isRocky });
  }
