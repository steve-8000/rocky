...
import {
  ...
  createRockyWorktree,
  type CreateRockyWorktreeDeps,
} from "./rocky-worktree-service.js";
import { readRockyWorktreeMetadata } from "../utils/worktree-metadata.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createRockyWorktree(
    {
      cwd: repoDir,
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createRockyWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createRockyWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
test("renames an eligible unnamed branch-off worktree once on first agent context", async () => {
  ...
  const created = await createRockyWorktree(
    {
      cwd: repoDir,
  ...
  expect(readRockyWorktreeMetadata(created.worktree.worktreePath)).toMatchObject({
    version: 2,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "rocky-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
interface TestDeps extends CreateRockyWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isRockyOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
