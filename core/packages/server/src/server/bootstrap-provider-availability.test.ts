...
describe("bootstrap provider availability", () => {
  ...
  test("loads a persisted Codex record without spawning a missing Codex binary", async () => {
    const { createRockyDaemon } = await import("./bootstrap.js");
    const root = await mkdtemp(path.join(os.tmpdir(), "rocky-bootstrap-provider-"));
    tempRoots.push(root);
    const binDir = await mkdtemp(path.join(os.tmpdir(), "rocky-bootstrap-provider-bin-"));
    ...
    const agentStoragePath = path.join(rockyHome, "agents");
