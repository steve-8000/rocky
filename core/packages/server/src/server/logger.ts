...
function resolveFilePath(rockyHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(rockyHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(rockyHome, configuredPath);
}
...
function resolveConfiguredRockyHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.rockyHome) {
    return options.rockyHome;
  }
  return resolveRockyHome();
}
...
export function resolveLogConfig(
  ...
  const rockyHome = resolveConfiguredRockyHome(options);
  ...
  const file =
    options?.file !== false && persistedLog?.file
      ? {
          ...
          path: resolveFilePath(rockyHome, persistedLog.file.path),
        }
      : undefined;
