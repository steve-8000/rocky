...
import { CLIENT_CAPS, type ClientCapability } from "@getrocky/protocol/client-capabilities";
import {
...
} from "@getrocky/protocol/binary-frames/index";
...
import { isRockyDictationDebugEnabled } from "./agent/recordings-debug.js";
...
import { getErrorMessage, getErrorMessageOr } from "@getrocky/protocol/error-utils";
import { getAgentStatusPriority } from "@getrocky/protocol/agent-state-bucket";
import type {
...
} from "../utils/rocky-config-file.js";
...
import {
  ...
  createRockyWorktree,
  type CreateRockyWorktreeInput,
  type CreateRockyWorktreeResult,
...
import {
  ...
  createRockyWorktreeWorkflow as createWorktreeWorkflow,
  type CreateRockyWorktreeSetupContinuationInput,
  type CreateRockyWorktreeWorkflowResult,
  handleCreateRockyWorktreeRequest as handleCreateWorktreeRequest,
  handleRockyWorktreeArchiveRequest as handleWorktreeArchiveRequest,
  handleRockyWorktreeListRequest as handleWorktreeListRequest,
...
export class Session {
  ...
  private readonly rockyHome: string;
  ...
  constructor(options: SessionOptions) {
    ...
    this.createAgentLifecycleDispatch = new CreateAgentLifecycleDispatch({
      ...
      createRockyWorktreeWorkflow: (input, workflowOptions) =>
        this.createRockyWorktreeWorkflow(input, workflowOptions),
  ...
  private isProviderVisibleToClient(_provider: string): boolean {
    // Rocky bundles its own client — all providers are always visible.
    return true;
  }
  ...
  private async handleReadProjectConfigRequest(
    ...
    const result = readRockyConfigForEdit(repoRoot);
    if (!result.ok) {
      this.sessionLogger.warn(
  ...
  private dispatchWorkspaceAndProjectMessage(
    ...
    switch (msg.type) {
      ...
      case "rocky_worktree_list_request":
        return this.handleRockyWorktreeListRequest(msg);
      case "rocky_worktree_archive_request":
        return this.handleRockyWorktreeArchiveRequest(msg);
      case "create_rocky_worktree_request":
        return this.handleCreateRockyWorktreeRequest(msg);
  ...
  private async buildAgentSessionConfig(
    ...
    setupContinuation?: CreateRockyWorktreeWorkflowResult["setupContinuation"];
  }> {
    return buildWorktreeAgentSessionConfig(
      {
        ...
        sessionLogger: this.sessionLogger,
        workspaceGitService: this.workspaceGitService,
        createRockyWorktree: (input, serviceOptions) =>
          this.createRockyWorktreeWorkflow(input, {
            ...serviceOptions,
            setupContinuation: {
              kind: "agent",
  ...
  private async handleDaemonGetStatusRequest(
    ...
    try {
      const pidInfo = await getPidLockInfo(this.rockyHome);
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          body: "Automated PR generated by Rocky.",
        };
      }
      throw error;
    }
  ...
  private static readonly ROCKY_STASH_PREFIX = "rocky-auto-stash:";
  ...
  private async handleStashSaveRequest(
    ...
    try {
      ...
      const message = branchLabel
        ? `${Session.ROCKY_STASH_PREFIX} ${branchLabel}`
        : `${Session.ROCKY_STASH_PREFIX} unnamed`;
  ...
  private async handleStashListRequest(
    ...
    const { cwd, requestId } = msg;
    const rockyOnly = msg.rockyOnly !== false;
    try {
      const entries = await this.workspaceGitService.listStashes(cwd, { rockyOnly });
  ...
  private async handleCheckoutMergeRequest(
    ...
    try {
      ...
      const mutatedCwd = await mergeToBase(
        ...
        { rockyHome: this.rockyHome, worktreesRoot: this.worktreesRoot },
      );
      await Promise.all([
  ...
  private async handleRockyWorktreeListRequest(
    msg: Extract<SessionInboundMessage, { type: "rocky_worktree_list_request" }>,
  ): Promise<void> {
  ...
  private async handleRockyWorktreeArchiveRequest(
    msg: Extract<SessionInboundMessage, { type: "rocky_worktree_archive_request" }>,
  ): Promise<void> {
      ...
      scripts:
        this.serviceProxy && this.scriptRuntimeStore
          ? buildWorkspaceScriptPayloads({
              ...
              workspaceDirectory: workspace.cwd,
              rockyConfig: readRockyConfigForProjection(workspace.cwd, this.sessionLogger),
  ...
  private buildWorkspaceGitRuntimePayload(
    ...
    return {
      ...
      isRockyOwnedWorktree: snapshot.git.isRockyOwnedWorktree,
  ...
  private async describeCreatedWorktreeWorkspace(
    result: CreateRockyWorktreeResult,
  ): Promise<WorkspaceDescriptorPayload> {
  ...
  private async createRockyWorktree(
    input: CreateRockyWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    },
  ): Promise<CreateRockyWorktreeResult> {
    const result = await createRockyWorktree(input, {
      github: this.github,
  ...
  private buildWorkspaceScriptPayloadSnapshot(
    ...
    return buildWorkspaceScriptPayloads({
      workspaceId,
      workspaceDirectory,
      rockyConfig: readRockyConfigForProjection(workspaceDirectory, this.sessionLogger),
      serviceProxy: this.serviceProxy,
      runtimeStore: this.scriptRuntimeStore,
      daemonPort: this.getDaemonTcpPort?.() ?? null,
      serviceProxyPublicBaseUrl: this.serviceProxyPublicBaseUrl,
    ...
    return handleCreateWorktreeRequest(
      {
        ...
        emit: (message) => this.emit(message),
        sessionLogger: this.sessionLogger,
        createRockyWorktreeWorkflow: (input) => this.createRockyWorktreeWorkflow(input),
      },
      request,
    );
  }

  private async createRockyWorktreeWorkflow(
    input: CreateRockyWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
      setupContinuation?: CreateRockyWorktreeSetupContinuationInput;
    },
  ): Promise<CreateRockyWorktreeWorkflowResult> {
    return createWorktreeWorkflow(
      {
        rockyHome: this.rockyHome,
        worktreesRoot: this.worktreesRoot,
        createRockyWorktree: (workflowInput, serviceOptions) =>
          this.createRockyWorktree(workflowInput, serviceOptions),
  ...
  private async handleArchiveWorkspaceRequest(
    ...
    try {
      ...
      if (existing.kind === "worktree") {
        throw new Error("Use worktree archive for Rocky worktrees");
      }
  ...
  private emit(msg: SessionOutboundMessage): void {
    ...
    if (
      ...
      (process.env.TTS_DEBUG_AUDIO_DIR || isRockyDictationDebugEnabled()) &&
      msg.payload.groupId &&
