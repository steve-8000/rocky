...
function getPidFilePath(rockyHome: string): string {
  return join(rockyHome, "rocky.pid");
}
...
export async function acquirePidLock(
  ...
  const pidPath = getPidFilePath(rockyHome);

  // Ensure rockyHome directory exists
  ...
  if (existingLock) {
    if (isPidRunning(existingLock.pid)) {
      ...
      throw new PidLockError(
        `Another Rocky daemon is already running (PID ${existingLock.pid}, started ${existingLock.startedAt})`,
        existingLock,
      );
    }
  ...
  const lockInfo: PidLockInfo = {
    ...
    listen,
    ...(process.env.ROCKY_DESKTOP_MANAGED === "1" ? { desktopManaged: true } : {}),
  };
  ...
  try {
    ...
    if (isErrnoException(err) && err.code === "EEXIST") {
      ...
      try {
        ...
        if (raceLock) {
          throw new PidLockError(
            `Another Rocky daemon is already running (PID ${raceLock.pid})`,
            raceLock,
          );
        }
...
export async function getPidLockInfo(rockyHome: string): Promise<PidLockInfo | null> {
  const pidPath = getPidFilePath(rockyHome);
  try {
    const content = await readFile(pidPath, "utf-8");
...
export async function isLocked(
  ...
  const info = await getPidLockInfo(rockyHome);
  if (!info) {
    return { locked: false };
  }
