...
import type { WorkspaceDescriptorPayload } from "@getrocky/protocol/messages";
import {
...
interface SessionHandlerInternals {
  ...
  createRockyWorktree(params: unknown): Promise<unknown>;
...
const rockyWorktreeServiceMocks = vi.hoisted(() => ({
  createRockyWorktree: vi.fn(),
}));
...
vi.mock("./rocky-worktree-service.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./rocky-worktree-service.js")>();
  return {
    ...actual,
    createRockyWorktree: rockyWorktreeServiceMocks.createRockyWorktree,
  };
});
...
describe("project config RPC authorization", () => {
  ...
  test("read_project_config_request accepts the same root with a trailing slash", async () => {
    ...
    writeFileSync(join(repoRoot, "rocky.json"), JSON.stringify({ worktree: { setup: "npm ci" } }));
    const messages: unknown[] = [];
    const session = createSessionForTest({
      messages,
  ...
  test("write_project_config_request emits stale and write-failed inline domain failures", async () => {
    ...
    writeFileSync(join(staleRoot, "rocky.json"), JSON.stringify({ worktree: { setup: "old" } }));
...
function createWorkspaceGitSnapshot(
  ...
  return {
    ...
    git: {
      ...
      remoteUrl: "https://github.com/getrocky/rocky.git",
...
`;
  ...
  function writeConfig(repoRoot: string, config: unknown): void {
    writeFileSync(join(repoRoot, "rocky.json"), `${JSON.stringify(config)}\n`);
  }

  async function generateCommitPromptWithConfig(config: unknown): Promise<string> {
    const repoRoot = makeRoot();
    if (typeof config === "string") {
      writeFileSync(join(repoRoot, "rocky.json"), config);
  ...
  test.each([
    ["rocky.json missing", undefined],
    ["rocky.json exists but invalid JSON", "{ nope"],
    ["rocky.json valid but missing metadataGeneration", {}],
...
`;
  ...
  test("keeps the PR fallback when structured generation fails", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/getrocky/rocky/pull/9",
      number: 9,
    });
    const session = createSessionForTest({ workspaceGitService, messages });
  ...
  test("forces workspace git and GitHub refresh after creating a pull request", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/getrocky/rocky/pull/2",
      number: 2,
    });
    const session = createSessionForTest({ github, workspaceGitService, messages });
...
describe("session branch validation", () => {
  ...
  test("does not validate tags as branches", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "rocky-session-branch-validation-"));
...
describe("session stash list handling", () => {
  test("lists stashes through the workspace git service", async () => {
    ...
    const entries = [
      {
        index: 0,
        message: "rocky-auto-stash: feature",
        branch: "feature",
        isRocky: true,
      },
    ];
...
describe("session rocky worktree creation handling", () => {
  test("forces workspace git refreshes for the source repo and created worktree", async () => {
    const workspaceGitService = { getSnapshot: vi.fn().mockResolvedValue({}) };
    const session = createSessionForTest({ workspaceGitService });
    rockyWorktreeServiceMocks.createRockyWorktree.mockResolvedValue({
      repoRoot: "/tmp/repo",
      worktree: {
        branchName: "feature/new-worktree",
        worktreePath: "/tmp/rocky/worktrees/new-worktree",
      },
      workspace: {
        workspaceId: "workspace-new-worktree",
        projectId: "project-repo",
        cwd: "/tmp/rocky/worktrees/new-worktree",
        kind: "worktree",
    ...
    await asSessionInternals(session).createRockyWorktree({
      cwd: "/tmp/repo",
    ...
    expect(workspaceGitService.getSnapshot).toHaveBeenCalledWith(
      "/tmp/rocky/worktrees/new-worktree",
      {
        force: true,
        reason: "create-worktree",
      },
  ...
  test("passes service-owned git metadata into workspace script spawning", async () => {
    ...
    const workspaceGitService = {
      ...
      getWorkspaceGitMetadata: vi.fn().mockResolvedValue({
        projectKind: "git",
        projectDisplayName: "getrocky/rocky",
...
describe("session pull request timeline handling", () => {
        ...
        items: [
          {
            ...
            number: 42,
            title: "Ship search",
            url: "https://github.com/getrocky/rocky/pull/42",
            state: "OPEN",
            body: null,
            labels: [],
            baseRefName: "main",
        ...
        items: [
          {
            ...
            url: "https://github.com/getrocky/rocky/pull/42#pullrequestreview-1",
  ...
  test.each([
    { prNumber: 0, repoOwner: "getrocky", repoName: "rocky" },
    { prNumber: -1, repoOwner: "getrocky", repoName: "rocky" },
    { prNumber: 42, repoOwner: "get rocky", repoName: "rocky" },
    { prNumber: 42, repoOwner: "getrocky/cli", repoName: "rocky" },
    { prNumber: 42, repoOwner: "get$rocky", repoName: "rocky" },
    { prNumber: 42, repoOwner: "getrocky", repoName: "pa seo" },
    { prNumber: 42, repoOwner: "getrocky", repoName: "rocky/app" },
    { prNumber: 42, repoOwner: "getrocky", repoName: "rocky!" },
