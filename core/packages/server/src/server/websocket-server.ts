...
import { asUint8Array, decodeTerminalStreamFrame } from "@getrocky/protocol/binary-frames/index";
...
} from "@getrocky/protocol/agent-attention-notification";
...
function requireWebSocketServices(params: {
  ...
  const { chatService, missionControlService, loopService, scheduleService, checkoutDiffManager } =
    params;
  if (!chatService) {
...
export class VoiceAssistantWebSocketServer {
  ...
  constructor(
    ...
    this.missionControlService = requiredServices.missionControlService;
    ...
    this.pushTokenStore = new PushTokenStore(pushLogger, join(rockyHome, "push-tokens.json"));
  ...
  private createSessionConnection(params: {
    ...
    const session = new Session({
      ...
      missionControlService: this.missionControlService ?? undefined,
