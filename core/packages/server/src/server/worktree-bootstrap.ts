...
export async function spawnWorkspaceScript(
  ...
  const configResult = readRockyConfig(repoRoot);
  if (!configResult.ok) {
    throw rockyConfigParseError(configResult);
  }
  const scriptConfigs = getScriptConfigs(configResult.config);
  const config = scriptConfigs.get(scriptName);
  if (!config) {
    throw new Error(`Script '${scriptName}' is not configured in rocky.json`);
  }
