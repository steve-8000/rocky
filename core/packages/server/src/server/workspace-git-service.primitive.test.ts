...
function createCheckoutFacts(
  ...
  return {
    ...
    rockyWorktree: { isRockyOwnedWorktree: false },
    storedBaseRef: null,
...
interface CreateServiceOptions {
  ...
  listRockyWorktrees?: ReturnType<typeof vi.fn>;
...
function buildDefaultServiceDeps() {
  return {
    ...
    listRockyWorktrees: vi.fn(async () => []),
  ...
  test("GitHub self-heal polling uses the fork PR head branch instead of the owner-prefixed local branch", async () => {
    ...
    const getCheckoutSnapshotFacts = vi.fn(async (cwd: string) =>
      createCheckoutFacts(cwd, {
        ...
        branchRemoteName: "rocky-pr-1285",
    ...
    const getCheckoutStatus = vi.fn(async (cwd: string) =>
      createCheckoutStatus(cwd, {
        currentBranch: "fork-owner/open-button-targets-active-file",
        remoteUrl: "git@github.com:getrocky/rocky.git",
      }),
    );
    const service = createService({
      getCheckoutSnapshotFacts,
...
describe("WorkspaceGitServiceImpl D2 read methods", () => {
  ...
  test("listStashes cold-loads, warms, forces, and coalesces per cwd", async () => {
    let nowMs = 0;
    const stashOutput = "stash@{0}\u0000rocky-auto-stash: feature\n";
    ...
    const first = service.listStashes(REPO_CWD, { rockyOnly: true });
    const second = service.listStashes(join(REPO_CWD, "."), { rockyOnly: true });
    await flushPromises();
    ...
    await expect(Promise.all([first, second])).resolves.toEqual([
      [{ index: 0, message: "rocky-auto-stash: feature", branch: "feature", isRocky: true }],
      [{ index: 0, message: "rocky-auto-stash: feature", branch: "feature", isRocky: true }],
    ]);

    nowMs = 1_000;
    await service.listStashes(REPO_CWD, { rockyOnly: true });
    expect(runGitCommand).toHaveBeenCalledTimes(1);

    await service.listStashes(REPO_CWD, { rockyOnly: true }, { force: true, reason: "test" });
    expect(runGitCommand).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees cold-loads, warms, forces, and coalesces per repo root", async () => {
    ...
    const listRockyWorktrees = vi.fn().mockResolvedValue(worktrees);
    const service = createService({
      listRockyWorktrees,
      now: () => new Date(nowMs),
    });
    ...
    expect(listRockyWorktrees).toHaveBeenCalledTimes(1);
    ...
    expect(listRockyWorktrees).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees shares one repo-root scoped read across sibling workspace cwds", async () => {
    ...
    const worktrees = [
      {
        path: join(tempDir, "rocky-home", "worktrees", "repo", "feature"),
        createdAt: "2026-04-12T00:00:00.000Z",
        branchName: "feature",
      },
    ];
    const listRockyWorktrees = vi.fn(async () => worktrees);
    const service = createService({
    ...
    try {
      ...
      expect(listRockyWorktrees).toHaveBeenCalledTimes(1);
      expect(listRockyWorktrees).toHaveBeenCalledWith({
  ...
  test("getWorkspaceGitMetadata derives reconciliation metadata from the snapshot cache", async () => {
    ...
    await expect(
      ...
      gitRemote: "https://github.com/getrocky/rocky.git",
      isWorktree: false,
      projectSlug: "rocky",
      repoRoot: REPO_CWD,
      currentBranch: "feature/service-metadata",
