...
describe("WorkspaceReconciliationService", () => {
  ...
  test("moves workspaces from a path-keyed duplicate project to the existing remote-keyed project", async () => {
    ...
    const canonicalWorktreeDir = path.join(repoDir, ".rocky", "worktrees", "focused-bat");
    const duplicateWorktreeDir = path.join(repoDir, ".rocky", "worktrees", "gigantic-blowfish");
