...
describe("daemon bearer validator", () => {
  ...
  test("extracts WebSocket rocky bearer subprotocol tokens", () => {
    const protocol = extractWsBearerProtocol("chat, rocky.bearer.secret.with.dots");

    expect(protocol).toBe("rocky.bearer.secret.with.dots");
    expect(extractWsBearerToken(protocol)).toBe("secret.with.dots");
    expect(extractWsBearerToken("rocky.other.secret")).toBeNull();
  });
});
