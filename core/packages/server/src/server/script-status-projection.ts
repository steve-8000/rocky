...
import { getScriptConfigs, isServiceScript, readRockyConfig } from "../utils/worktree.js";
...
export function readRockyConfigForProjection(
  workspaceDirectory: string,
  logger: Logger,
): RockyConfig | null {
  const result = readRockyConfig(workspaceDirectory);
  if (result.ok) {
    return result.config;
  }
  logger.warn(
    { configPath: result.configPath, workspaceDirectory, err: result.error },
    "Failed to parse rocky.json; treating workspace as having no scripts",
  );
  return null;
}
...
export function buildWorkspaceScriptPayloads(
  ...
  const scriptConfigs = getScriptConfigs(options.rockyConfig);
  ...
  return (workspaceId, scripts) => {
    void (async () => {
      ...
      const projected = buildWorkspaceScriptPayloads({
        workspaceId,
        workspaceDirectory,
        rockyConfig: readRockyConfigForProjection(workspaceDirectory, logger),
        serviceProxy,
        runtimeStore,
