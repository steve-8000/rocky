...
describe("generateBranchNameFromFirstAgentContext", () => {
  ...
  test("keeps the branch slug validator fallback when instructions are present", async () => {
    const repoRoot = createTempDir("rocky-branch-config-");
    const worktreeRoot = createTempDir("rocky-branch-worktree-");
    mkdirSync(path.join(worktreeRoot, ".git"));
    writeRockyWorktreeMetadata(worktreeRoot, { baseRefName: "main" });
    writeRockyWorktreeFirstAgentBranchAutoNameMetadata(worktreeRoot, {
...
async function generateBranchPromptWithConfig(config: unknown): Promise<{ prompt: string }> {
  ...
  if (typeof config === "string") {
    writeFileSync(path.join(repoRoot, "rocky.json"), config);
...
function writeConfig(repoRoot: string, config: unknown): void {
  writeFileSync(path.join(repoRoot, "rocky.json"), `${JSON.stringify(config)}\n`);
}
