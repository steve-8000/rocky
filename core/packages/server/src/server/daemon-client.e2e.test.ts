...
const localModelsDir =
  process.env.ROCKY_LOCAL_MODELS_DIR ?? path.join(homedir(), ".rocky", "models", "local-speech");
...
test("returns home-scoped directory suggestions", async () => {
  const insideHomeDir = mkdtempSync(path.join(homedir(), "rocky-dir-suggestion-"));
  const outsideHomeDir = mkdtempSync(path.join(tmpdir(), "rocky-dir-suggestion-outside-"));
