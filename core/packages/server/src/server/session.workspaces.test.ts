...
import type { AgentSnapshotPayload, SessionOutboundMessage } from "@getrocky/protocol/messages";
...
interface SessionTestAccess {
  ...
  handleCreateRockyWorktreeRequest(params: unknown): Promise<unknown>;
...
test("create_agent_request keeps requested child cwd when grouped under an existing parent workspace", async () => {
  const workdir = mkdtempSync(path.join(tmpdir(), "rocky-create-agent-cwd-"));
  try {
    ...
    const session = asTestSession(
      new Session({
        ...
        downloadTokenStore: asDownloadTokenStore(),
        pushTokenStore: asPushTokenStore(),
        rockyHome: path.join(workdir, "rocky-home"),
        agentManager,
        agentStorage,
        projectRegistry,
        workspaceRegistry,
...
test("workspace reconciliation reports archived workspaces to subscribed clients", async () => {
  const missingCwd = path.join(tmpdir(), `rocky-missing-workspace-${Date.now()}`);
...
test("create rocky worktree request returns a registered workspace descriptor", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const tempDir = realpathSync(mkdtempSync(path.join(tmpdir(), "session-worktree-test-")));
  const repoDir = path.join(tempDir, "repo");
  const rockyHome = path.join(tempDir, "rocky-home");
  ...
  try {
    await session.handleCreateRockyWorktreeRequest({
  ...
  const response = findByType(emitted, "create_rocky_worktree_response");
...
test("open_project_request does not match a new child directory to an existing parent workspace", async () => {
  ...
  const worktree = path.join(home, ".rocky", "worktrees", "project-config-lifecycle-textarea");
...
test("open_project_request reclassifies an archived directory workspace when git metadata becomes available", async () => {
  ...
  const projects = new Map<string, ReturnType<typeof createPersistedProjectRecord>>();
  const workspaces = new Map<string, ReturnType<typeof createPersistedWorkspaceRecord>>();
  const repoRoot = path.resolve("/Users/moboudra/dev/rocky");
  const cwd = path.join(
  ...
  const remoteProjectId = "remote:github.com/getrocky/rocky";
...
test.skip("opening a new worktree reconciles older local workspaces into the remote project", async () => {
  ...
  const worktreeWorkspaceId = path.join(mainWorkspaceId, ".rocky", "worktrees", "feature-a");
  ...
  session.buildProjectPlacement = async (cwd: string) => ({
    ...
    checkout: {
      ...
      worktreeRoot: cwd,
      isRockyOwnedWorktree: cwd !== mainWorkspaceId,
