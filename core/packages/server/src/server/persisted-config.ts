...
const DEFAULT_PERSISTED_CONFIG = PersistedConfigSchema.parse({
  ...
  daemon: {
    ...
    cors: {
      allowedOrigins: ["https://rocky.clab.one"],
    },
    relay: {
      enabled: true,
    },
...
function getConfigPath(rockyHome: string): string {
  return path.join(rockyHome, CONFIG_FILENAME);
}
...
export function loadPersistedConfig(rockyHome: string, logger?: LoggerLike): PersistedConfig {
  const log = getLogger(logger);
  const configPath = getConfigPath(rockyHome);
