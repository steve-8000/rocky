...
import { isLikelyExternalToolName } from "@getrocky/protocol/tool-name-normalization";
...
class FakeAgentSession implements AgentSession {
  ...
  private async runSlashCommand(
    ...
    if (this.providerName === "codex" && fullName.startsWith("prompts:")) {
      ...
      return {
        text: `ROCKY_OK ${args ?? ""}`.trim(),
        timeline: [{ type: "assistant_message", text: `ROCKY_OK ${promptId}` }],
        usage: { inputTokens: 1, outputTokens: 1 },
      };
    }
    ...
    return {
      text: "ROCKY_SKILL_OK",
      timeline: [{ type: "assistant_message", text: "ROCKY_SKILL_OK" }],
      usage: { inputTokens: 1, outputTokens: 1 },
    };
  }
