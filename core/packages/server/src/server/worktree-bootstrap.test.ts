...
describe("runAsyncWorktreeBootstrap", () => {
  ...
  function assertServiceTerminalCallSelfEnv(params: {
    ...
    expect(createTerminalCalls[0]?.env?.ROCKY_PORT).toEqual(expect.any(String));
    expect(createTerminalCalls[0]?.env?.HOST).toBe("127.0.0.1");
    expect(createTerminalCalls[0]?.env?.ROCKY_URL).toBe(
      "http://api--feature-socket-service--repo.localhost:7767",
    );
    expect(createTerminalCalls[0]?.env?.ROCKY_SERVICE_API_PORT).toBe(
      createTerminalCalls[0]?.env?.ROCKY_PORT,
    );
    expect(createTerminalCalls[0]?.env?.ROCKY_SERVICE_API_URL).toBe(
      "http://api--feature-socket-service--repo.localhost:7767",
    );
  }
  ...
  async function assertServiceTerminalCallPeerEnv(params: {
    ...
    expect(createTerminalCalls[0]?.env?.ROCKY_SERVICE_APP_SERVER_PORT).toBe(
      String(plannedAppServerPort),
    );
    expect(createTerminalCalls[0]?.env?.ROCKY_SERVICE_APP_SERVER_URL).toBe(
      "http://app-server--feature-socket-service--repo.localhost:7767",
    );
  }
  ...
  function commitRockyScripts(
    scripts: Record<string, { command: string; type?: "script" | "service" }>,
    message = "add script config",
  ): void {
    writeFileSync(join(repoDir, "rocky.json"), JSON.stringify({ scripts }));
    execFileSync("git", ["add", "rocky.json"], { cwd: repoDir, stdio: "pipe" });
  ...
  it("refreshes a stopped service port on respawn and updates the route", async () => {
    ...
    expect(createTerminalCalls[2]?.env?.ROCKY_SERVICE_WORKER_PORT).toBe(String(workerPort));
  ...
  it("fails normalized service env name collisions before terminal creation", async () => {
    ...
    expect(createTerminalCalls).toHaveLength(1);
    expect(createTerminalCalls[0]?.env).toHaveProperty("ROCKY_SERVICE_APP_SERVER_PORT");
    expect(createTerminalCalls[0]?.env).toHaveProperty("ROCKY_SERVICE_WORKER_PORT");
  });
  ...
  it("binds services to the network when the daemon listens on a non-loopback host", async () => {
    ...
    expect(createTerminalCalls[0]?.env?.ROCKY_URL).toBe(
      "http://web--feature-remote-service--repo.localhost:7767",
    );
  });
});
