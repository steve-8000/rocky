...
function buildTurns(scenario: RewindCase): ClaudeTurnSpec[] {
  ...
  return Array.from({ length: scenario.turnCount }, (_, offset) => {
    ...
    return {
      index,
      promptToken: `ROCKY_RW_${prefix}_T${index}`,
      doneToken: `ROCKY_RW_${prefix}_T${index}_DONE`,
  ...
  test("emits only the deliberate fork session switch after conversation rewind and continue", async () => {
    ...
    try {
      await sendClaudeReplyTurn(
        harness,
        session,
        "ROCKY_RW_NO_ROUNDTRIP_T1. Reply exactly: ROCKY_RW_NO_ROUNDTRIP_T1_DONE",
      );
      await sendClaudeReplyTurn(
        harness,
        session,
        "ROCKY_RW_NO_ROUNDTRIP_T2. Reply exactly: ROCKY_RW_NO_ROUNDTRIP_T2_DONE",
      );

      const beforeTimeline = await fetchTimelineItems(harness.client, session.agentId);
      const targetMessageId = userMessageIdForToken(beforeTimeline, "ROCKY_RW_NO_ROUNDTRIP_T2");
      ...
      await sendClaudeReplyTurn(
        harness,
        session,
        "ROCKY_RW_NO_ROUNDTRIP_T3. Reply exactly: ROCKY_RW_NO_ROUNDTRIP_T3_DONE",
      );
