...
const RUN = process.env.ROCKY_GIT_DIFF_BOTTLENECK_E2E === "1";
const LARGE_CHANGESET_SIZE = Number.parseInt(
  process.env.ROCKY_GIT_DIFF_BOTTLENECK_FILE_COUNT ?? "1200",
  10,
);

function tmpRepo(): string {
  return mkdtempSync(path.join(tmpdir(), "rocky-git-diff-bottleneck-"));
}
