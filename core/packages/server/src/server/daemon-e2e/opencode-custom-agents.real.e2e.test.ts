...
async function createHarness(): Promise<{
  client: DaemonClient;
  daemon: Awaited<ReturnType<typeof createTestRockyDaemon>>;
}> {
  ...
  test("custom agents defined in opencode.json appear in availableModes", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        agent: {
          "rocky-e2e-custom": {
            description: "Custom agent for Rocky daemon E2E test",
            mode: "primary",
          },
        },
      }),
    ...
    try {
      ...
      const custom = snapshot.availableModes.find((m) => m.id === "rocky-e2e-custom");
      expect(custom).toBeDefined();
      expect(custom!.description).toBe("Custom agent for Rocky daemon E2E test");
