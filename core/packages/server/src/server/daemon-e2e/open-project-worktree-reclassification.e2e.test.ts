...
import { createTestRockyDaemon, type TestRockyDaemon } from "../test-utils/rocky-daemon.js";
import {
...
const cleanupDaemons = new Set<TestRockyDaemon>();
...
test("openProject reclassifies an existing directory workspace into its parent git project", async () => {
  const previousSupervised = process.env.ROCKY_SUPERVISED;
  process.env.ROCKY_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "rocky-open-project-repo-")));
    const worktreeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "rocky-open-project-worktree-")),
    );
    const rockyHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "rocky-open-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(worktreeRoot);
    cleanupPaths.add(rockyHomeRoot);

    execSync("git init -b main", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.email 'test@getrocky.dev'", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.name 'Rocky Test'", { cwd: repoRoot, stdio: "pipe" });
    ...
    const projectsPath = path.join(rockyHome, "projects", "projects.json");
    const workspacesPath = path.join(rockyHome, "projects", "workspaces.json");
    ...
    const daemon = await createTestRockyDaemon({ rockyHomeRoot, cleanup: false });
    ...
    process.env.ROCKY_SUPERVISED = previousSupervised;
  }
}, 30_000);
