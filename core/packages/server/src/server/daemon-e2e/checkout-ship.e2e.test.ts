...
function initGitRepo(repoDir: string): void {
  ...
  execSync("git config user.email 'rocky-test@example.com'", {
    cwd: repoDir,
    stdio: "pipe",
  });
  execSync("git config user.name 'Rocky Test'", {
    cwd: repoDir,
    stdio: "pipe",
  });
    ...
    async () => {
      ...
      try {
        ...
        const worktree = await createLegacyWorktreeForTest({
          ...
          rockyHome: ctx.daemon.rockyHome,
        });
        ...
        const worktreeList = await ctx.client.getRockyWorktreeList({
          cwd: repoDir,
        });
        ...
        const archiveResult = await ctx.client.archiveRockyWorktree({
          worktreePath: worktree.worktreePath,
        });
        expect(archiveResult.error).toBeNull();
        expect(archiveResult.success).toBe(true);

        const worktreeListAfter = await ctx.client.getRockyWorktreeList({
          cwd: repoDir,
        });
        expect(
