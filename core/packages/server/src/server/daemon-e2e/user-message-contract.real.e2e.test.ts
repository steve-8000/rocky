...
describe.each(CONTRACT_CASES)("daemon E2E (real $provider) - user_message contract", (entry) => {
  let canRun = false;
  let daemon: TestRockyDaemon | null = null;
  ...
  beforeEach(async (context) => {
    ...
    daemon = await createTestRockyDaemon({
  ...
  test(
    ...
    async () => {
      ...
      const prompt = `ROCKY_USER_MESSAGE_CONTRACT_${entry.provider.toUpperCase()}. Reply exactly: OK.`;
