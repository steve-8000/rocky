...
test("runs rocky.json setup asynchronously and reports status via timeline tool_call", async () => {
  ...
  execSync("git -c commit.gpgsign=false commit -m 'add rocky.json'", {
    cwd: repoRoot,
    stdio: "pipe",
  });
  ...
  expect(agent.cwd).toContain(path.join(".rocky", "worktrees"));
  ...
  const completed = await waitForTimelineToolCall(
    collector.messages,
    agent.id,
    (item) => item.name === "rocky_worktree_setup" && item.status === "completed",
    20000,
  );
...
test("bootstraps configured worktree terminals after setup succeeds", async () => {
  await withShell("/bin/sh", async () => {
    ...
    const setupCommand =
      'while [ ! -f "$ROCKY_WORKTREE_PATH/allow-setup" ]; do sleep 0.05; done; echo "done" > "$ROCKY_WORKTREE_PATH/setup-done.txt"; echo "$ROCKY_WORKTREE_PORT" > "$ROCKY_WORKTREE_PATH/setup-port.txt"';
    writeFileSync(
      path.join(repoRoot, "rocky.json"),
      JSON.stringify({
        worktree: {
    ...
    const terminalsBootstrapToolCall = await waitForTimelineToolCall(
      collector.messages,
      agent.id,
      (item) => item.name === "rocky_worktree_terminals" && item.status === "completed",
      30000,
    );
    ...
    ctx.client.sendTerminalInput(manualTerminalId, {
      type: "input",
      data: 'echo "$ROCKY_WORKTREE_PORT" > "$ROCKY_WORKTREE_PATH/manual-terminal-port.txt"\r',
    });
    await waitForPathExists({
...
test("reports failures via timeline tool_call without deleting the created worktree", async () => {
  ...
  const setupCommand =
    'echo "started" > "$ROCKY_WORKTREE_PATH/setup-start.txt"; sleep 0.1; echo "boom" 1>&2; exit 7';
  writeFileSync(
    path.join(repoRoot, "rocky.json"),
    JSON.stringify({
      worktree: {
  ...
  const started = await waitForTimelineToolCall(
    collector.messages,
    agent.id,
    (item) => item.name === "rocky_worktree_setup" && item.status === "running",
    10000,
  );
...
test("creates agent in ~/.rocky/worktrees/{hash} when worktree is requested", async () => {
  const cwd = tmpCwd();
  ...
  expect(realpathSync(agent.cwd)).toBe(
    realpathSync(path.join(ctx.daemon.rockyHome, "worktrees", projectHash, "worktree-test")),
  );
  expect(existsSync(agent.cwd)).toBe(true);
...
test("archives worktree by running teardown commands and shutting down worktree terminals", async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      worktree: {
        ...
        teardown: [`echo "$ROCKY_WORKTREE_PATH" > "${teardownMarkerPath}"`],
      },
    }),
  );
  execSync("git add rocky.json", { cwd: repoRoot, stdio: "pipe" });
  ...
  const archive = await ctx.client.archiveRockyWorktree({
