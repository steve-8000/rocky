...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    process.env.ROCKY_PRIMARY_LAN_IP = "192.168.1.12";
    ...
    try {
      ...
      expect(offerUrl.startsWith("https://rocky.clab.one/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.invalid:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "rocky-offer-home-"));

    const { logger: logger1 } = createCapturingLogger();
    const daemon1 = await createTestRockyDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      rockyHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      ...
      const daemon2 = await createTestRockyDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        rockyHomeRoot: tempHomeRoot,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "rocky-offer-e2e-"));
    ...
    const env = {
      ...process.env,
      ROCKY_HOME: tempHome,
      ROCKY_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
      ROCKY_DICTATION_ENABLED: "0",
      ROCKY_VOICE_MODE_ENABLED: "0",
      ROCKY_LOG_FORMAT: "json",
    };
