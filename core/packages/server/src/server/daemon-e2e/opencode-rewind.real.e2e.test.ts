...
import type { AgentLifecycleStatus } from "@getrocky/protocol/agent-lifecycle";
...
async function launchOpenCodeRewindSession(
  ...
  execFileSync("git", ["config", "user.email", "rocky-test@example.com"], {
    cwd,
    stdio: "ignore",
  });
  execFileSync("git", ["config", "user.name", "Rocky Test"], { cwd, stdio: "ignore" });
...
function editPrompt(input: {
  ...
  return [
    `ROCKY_OPENCODE_REWIND_PROMPT_${input.promptToken}.`,
...
describe("daemon E2E (real opencode) - rewind", () => {
  ...
  test("records exactly one plain-text user message for a real OpenCode turn", async () => {
    ...
    const prompt = "ROCKY_OPENCODE_PLAIN_TEXT_DUP_CHECK. Reply exactly: OPENCODE_PLAIN_TEXT_DONE";
  ...
  test("rewinds a real OpenCode read-only turn without changing files", async () => {
    ...
    try {
      await harness.client.sendMessage(
        session.agentId,
        [
          "ROCKY_OPENCODE_REWIND_PROMPT_READ_ONLY.",
  ...
  test("rewinds every file from a single real OpenCode multi-tool edit turn", async () => {
    ...
    try {
      await harness.client.sendMessage(
        session.agentId,
        [
          "ROCKY_OPENCODE_REWIND_PROMPT_MULTI_EDIT.",
