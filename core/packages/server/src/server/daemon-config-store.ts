...
export type { MutableDaemonConfig, MutableDaemonConfigPatch } from "@getrocky/protocol/messages";

type MutableDaemonConfig = import("@getrocky/protocol/messages").MutableDaemonConfig;
type MutableDaemonConfigPatch = import("@getrocky/protocol/messages").MutableDaemonConfigPatch;
...
export class DaemonConfigStore {
  ...
  constructor(rockyHome: string, initial: MutableDaemonConfig, logger?: LoggerLike) {
  ...
  private persistConfig(config: MutableDaemonConfig): void {
    const persisted = loadPersistedConfig(this.rockyHome, this.logger);
    const nextPersisted = mergeMutableConfigIntoPersistedConfig({
      persisted,
      mutable: config,
    });
    savePersistedConfig(this.rockyHome, nextPersisted, this.logger);
  }
}
