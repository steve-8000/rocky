...
export function buildWorkspaceServiceEnv(
  ...
  const env: Record<string, string> = {
    ...
    ROCKY_PORT: String(selfPeer.port),
  };
  ...
  if (selfProxyUrl) {
    env.ROCKY_URL = selfProxyUrl;
  }

  for (const peer of options.peers) {
    const envName = normalizeServiceEnvName(peer.scriptName);
    env[`ROCKY_SERVICE_${envName}_PORT`] = String(peer.port);
    ...
    if (peerProxyUrl) {
      env[`ROCKY_SERVICE_${envName}_URL`] = peerProxyUrl;
    }
  }
