...
describe("isVoicePermissionAllowed", () => {
  test("allows direct speak tool names across provider conventions", () => {
    ...
    expect(isVoicePermissionAllowed(buildRequest({ name: "rocky_voice.speak" }))).toBe(true);
    expect(isVoicePermissionAllowed(buildRequest({ name: "mcp__rocky_voice__speak" }))).toBe(true);
  });

  test("denies non-speak tool names", () => {
    expect(isVoicePermissionAllowed(buildRequest({ name: "mcp__rocky__create_agent" }))).toBe(
      false,
    );
    expect(isVoicePermissionAllowed(buildRequest({ name: "rocky_create_agent" }))).toBe(false);
  });

  test("denies non-tool permission kinds", () => {
    const result = isVoicePermissionAllowed(
      buildRequest({ kind: "mode", name: "mcp__rocky__create_agent" }),
    );
  ...
  test("denies wrapper tools even when metadata references speak", () => {
    const denied = isVoicePermissionAllowed(
      buildRequest({
        name: "codextool",
        metadata: {
          questions: [{ question: "Allow codextool to call rocky_voice.speak for user feedback?" }],
        },
      }),
    );
