...
import {
  markRockyWorktreeFirstAgentBranchAutoNameAttempted,
  readRockyWorktreeMetadata,
  writeRockyWorktreeFirstAgentBranchAutoNameMetadata,
} from "../utils/worktree-metadata.js";
import type { WorktreeCreationIntent } from "./resolve-worktree-creation-intent.js";
import { buildAgentBranchNameSeed } from "./agent/prompt-attachments.js";
import type { FirstAgentContext } from "@getrocky/protocol/messages";

export interface CreateRockyWorktreeInput extends CreateWorktreeCoreInput {
  projectId?: string;
}

export interface CreateRockyWorktreeResult {
...
export type CreateRockyWorktreeFn = (
...
export interface CreateRockyWorktreeDeps extends CreateWorktreeCoreDeps {
  projectRegistry: Pick<ProjectRegistry, "get" | "upsert">;
  workspaceRegistry: Pick<WorkspaceRegistry, "get" | "list" | "upsert">;
  workspaceGitService: WorkspaceGitService;
}

export async function createRockyWorktree(
  input: CreateRockyWorktreeInput,
  deps: CreateRockyWorktreeDeps,
): Promise<CreateRockyWorktreeResult> {
...
export async function attemptFirstAgentBranchAutoName(options: {
  ...
  let metadata: ReturnType<typeof readRockyWorktreeMetadata>;
  try {
    metadata = readRockyWorktreeMetadata(options.cwd);
  } catch {
    return { attempted: false, renamed: false, branchName: null };
  }
  if (
  ...
  if ((await getCurrentBranchImpl(options.cwd)) !== placeholderBranchName) {
    markRockyWorktreeFirstAgentBranchAutoNameAttempted(options.cwd);
...
function maybeMarkFirstAgentBranchAutoNameEligible(options: {
  ...
  writeRockyWorktreeFirstAgentBranchAutoNameMetadata(createdWorktree.worktree.worktreePath, {
...
async function upsertWorkspaceForWorktree(options: {
  ...
  repoRoot: string;
  worktree: WorktreeConfig;
  deps: Pick<CreateRockyWorktreeDeps, "projectRegistry" | "workspaceRegistry">;
