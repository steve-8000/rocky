...
import { archiveRockyWorktree } from "./rocky-worktree-archive-service.js";
import {
  buildAgentSessionConfig,
  createRockyWorktreeWorkflow,
  handleRockyWorktreeArchiveRequest,
  handleRockyWorktreeListRequest,
  resolveGitCreateBaseBranch,
  runWorktreeSetupInBackground,
  handleCreateRockyWorktreeRequest,
...
import {
  createRockyWorktree as createRockyWorktreeService,
  type CreateRockyWorktreeFn,
...
function createWorkflowForRequestTest(options: {
  rockyHome: string;
  createRockyWorktree?: CreateRockyWorktreeFn;
  ...
  return async (input: Parameters<CreateRockyWorktreeFn>[0]) => {
    const createRockyWorktree =
      options.createRockyWorktree ?? createRockyWorktreeForTest({ rockyHome: options.rockyHome });
    return createRockyWorktreeWorkflow(
      {
        rockyHome: options.rockyHome,
        createRockyWorktree,
...
function createRockyWorktreeForTest(options: {
  rockyHome: string;
  events?: string[];
}): CreateRockyWorktreeFn {
  ...
  return (input, serviceOptions) => {
    return createRockyWorktreeService(input, {
      github: createGitHubServiceStub(),
...
describe("handleRockyWorktreeListRequest", () => {
  test("lists worktrees through the workspace git service", async () => {
    const emitted: SessionOutboundMessage[] = [];
    const workspaceGitService = {
      listWorktrees: vi.fn().mockResolvedValue([
        {
          path: "/tmp/rocky-home/worktrees/repo/feature",
    ...
    await handleRockyWorktreeListRequest(
      {
    ...
    expect(emitted).toContainEqual({
      ...
      payload: {
        worktrees: [
          {
            worktreePath: "/tmp/rocky-home/worktrees/repo/feature",
            createdAt: "2026-04-12T00:00:00.000Z",
            branchName: "feature",
            head: "abc123",
          },
...
describe("create-agent worktree setup boundary", () => {
  test("agent setup continuation starts setup for the created agent timeline", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const rockyHome = path.join(tempDir, ".rocky");
    ...
    try {
      const result = await createRockyWorktreeWorkflow(
        {
          rockyHome,
          createRockyWorktree: createRockyWorktreeForTest({ rockyHome }),
...
function createGitRepo(options?: { rockyConfig?: Record<string, unknown> }) {
...
describe("runWorktreeSetupInBackground", () => {
  ...
  test("archives the pending workspace and emits a failed snapshot when setup cannot start", async () => {
    ...
    writeFileSync(path.join(repoDir, "rocky.json"), "{ invalid json\n");
    execFileSync("git", ["add", "rocky.json"], { cwd: repoDir, stdio: "pipe" });
    ...
    expect(progressMessages[1]?.payload.error).toMatch(
      /Failed to parse rocky\.json at .*rocky\.json/,
    );
    expect(progressMessages[1]?.payload.detail.commands).toEqual([]);
    expect(snapshots.get(workspaceId)).toMatchObject({
      status: "failed",
      error: expect.stringMatching(/Failed to parse rocky\.json at .*rocky\.json/),
    });
    ...
    const snapshots = new Map([
      [
        "ws-feature-a",
        {
          ...
          detail: {
            type: "worktree_setup",
            worktreePath: "/repo/.rocky/worktrees/feature-a",
...
describe("handleCreateRockyWorktreeRequest", () => {
  const cleanupPaths: string[] = [];
  ...
  test("checks out the GitHub PR branch when githubPrNumber is supplied", async () => {
    ...
    await handleCreateRockyWorktreeRequest(
      {
        rockyHome,
        ...
        emit: (message) => emitted.push(message),
        sessionLogger: logger,
        createRockyWorktreeWorkflow: createWorkflowForRequestTest({ rockyHome }),
      },
      {
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "create_rocky_worktree_response" }> =>
        message.type === "create_rocky_worktree_response",
    );
  ...
  test("buildAgentSessionConfig passes prompt and attachment context into worktree creation", async () => {
    const createRockyWorktree = vi.fn(async () => ({
      worktree: {
    ...
    expect(createRockyWorktree).toHaveBeenCalledWith(
  ...
  test("createRockyWorktreeForTest forwards the default branch resolver for branch-off intents", async () => {
    const { tempDir, repoDir } = createGitRepo();
    cleanupPaths.push(tempDir);
    const rockyHome = path.join(tempDir, ".rocky");
    const resolveDefaultBranch = vi.fn(async () => "main");

    const result = await createRockyWorktreeForTest({ rockyHome })(
      {
        cwd: repoDir,
      ...
      await handleCreateRockyWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createRockyWorktreeWorkflow: createWorkflowForRequestTest({
            rockyHome,
            createRockyWorktree: createRockyWorktreeForTest({ rockyHome, events }),
          }),
  ...
  test("creates the worktree before emitting the response", async () => {
    ...
    try {
      await handleCreateRockyWorktreeRequest(
        {
          ...
          createRockyWorktreeWorkflow: createWorkflowForRequestTest({
            rockyHome,
            createRockyWorktree: async (input) => {
              const result = await createRockyWorktreeForTest({ rockyHome })(input);
      ...
      expect(emitted.map((message) => message.type).slice(0, 2)).toEqual([
        "create_rocky_worktree_response",
        "workspace_update",
      ]);
...
describe("archiveRockyWorktree", () => {
  ...
  test("runs agent close and terminal teardown concurrently and removes the worktree", async () => {
    ...
    const archivedAgents = await archiveRockyWorktree(
      {
        rockyHome,
        github: createGitHubServiceStub(),
        agentManager: {
  ...
  test("emits archiving upserts during worktree archive request until final remove", async () => {
    ...
    await handleRockyWorktreeArchiveRequest(
      {
        rockyHome,
        github: createGitHubServiceStub(),
        workspaceGitService: {
    ...
    expect(
      emitted.find((message) => message.type === "rocky_worktree_archive_response"),
    ).toMatchObject({
      payload: {
        success: true,
        error: null,
  ...
  test("archives the workspace record even when the teardown script fails", async () => {
    const teardownLogPath = isPlatform("win32")
      ? 'Set-Content -Path (Join-Path $env:ROCKY_SOURCE_CHECKOUT_PATH "teardown-start.log") -Value "started"'
  ...
  test("succeeds when git has forgotten about the worktree (no repoRoot)", async () => {
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "rocky_worktree_archive_response" }> =>
        message.type === "rocky_worktree_archive_response",
    );
