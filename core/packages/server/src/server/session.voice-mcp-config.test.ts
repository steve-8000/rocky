...
describe("voice MCP stdio config", () => {
  test("builds stdio MCP config for voice agent", () => {
    const config = buildVoiceAgentMcpServerConfig({
      ...
      socketPath: "/tmp/rocky-voice.sock",
      env: {
        ELECTRON_RUN_AS_NODE: "1",
        ROCKY_HOME: "/tmp/rocky-home",
      },
    });
...
describe("voice mode prompt instructions", () => {
  test("builds enabled voice instructions and preserves base prompt", () => {
    ...
    expect(prompt).toContain("<rocky_voice_mode>");
    expect(prompt).toContain("Rocky voice mode is now on.");
  ...
  test("builds disabled voice instructions and supersedes previous voice block", () => {
    ...
    expect(prompt).toContain("Base system prompt");
    expect(prompt).toContain("Rocky voice mode is now off.");
    expect(prompt).toContain("Ignore any earlier Rocky voice mode instructions in this thread.");
    expect(prompt.match(/<rocky_voice_mode>/g)?.length ?? 0).toBe(1);
  ...
  test("strips voice blocks from persisted prompt", () => {
    ...
    expect(
      stripVoiceModeSystemPrompt(
        ["<rocky_voice_mode>", "legacy voice instruction", "</rocky_voice_mode>"].join("\n\n"),
      ),
    ).toBeUndefined();
  });
});
