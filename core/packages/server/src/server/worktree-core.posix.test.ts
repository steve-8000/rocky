...
function createGitRepo(): { tempDir: string; repoDir: string; rockyHome: string } {
  ...
  return { tempDir, repoDir, rockyHome };
}

function createGitRepoWithDevBranch(): { tempDir: string; repoDir: string; rockyHome: string } {
  const { tempDir, repoDir, rockyHome } = createGitRepo();
...
function createGitRepoWithOriginMain(): { tempDir: string; repoDir: string; rockyHome: string } {
  const { tempDir, repoDir, rockyHome } = createGitRepo();
...
function createGitHubPrRemoteRepo(): { tempDir: string; repoDir: string; rockyHome: string } {
  const { tempDir, repoDir, rockyHome } = createGitRepo();
...
function createForkGitHubPrRemoteRepo(): {
  ...
  return { tempDir, repoDir, headRemoteDir, rockyHome };
}
...
describe.skipIf(isPlatform("win32"))("worktree-core POSIX-only", () => {
  describe("createWorktreeCore", () => {
    ...
    test("creates branch-off worktrees from origin main without tracking origin main", async () => {
      const { tempDir, repoDir, rockyHome } = createGitRepoWithOriginMain();
      cleanupPaths.push(tempDir);
    ...
    test("checks out an explicit GitHub PR branch with legacy RPC fields", async () => {
      const { tempDir, repoDir, rockyHome } = createGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
    ...
    test("branches off an explicit refName base", async () => {
      const { tempDir, repoDir, rockyHome } = createGitRepoWithDevBranch();
      cleanupPaths.push(tempDir);
    ...
    test("checks out a fork PR whose head branch collides with local main", async () => {
      const { tempDir, repoDir, headRemoteDir, rockyHome } = createForkGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
      const github = {
    ...
    test("uses a unique local branch when the same fork PR branch already exists", async () => {
      ...
      expect(
        execFileSync("git", ["config", "--get", "remote.rocky-pr-526.push"], {
    ...
    test("reuses an existing branch-off worktree for the same slug", async () => {
      ...
      const first = await createCoreWorktree(
        { cwd: repoDir, worktreeSlug: "reused-worktree", rockyHome, runSetup: false },
        deps,
      );
      const second = await createCoreWorktree(
        { cwd: repoDir, worktreeSlug: "reused-worktree", rockyHome, runSetup: false },
