...
function bootstrapFromEnvironment(): BootstrapResult {
  try {
    const rockyHome = resolveRockyHome();
    const config = loadConfig(rockyHome);
    const logger = createRootLogger({ log: config.log }, { rockyHome, file: false });
    return { rockyHome, logger, config };
  } catch (err) {
...
async function main() {
  ...
  let daemon: Awaited<ReturnType<typeof createRockyDaemon>> | null = null;
  ...
  const handleLifecycleIntent = (intent: DaemonLifecycleIntent) => {
    if (intent.type === "shutdown") {
      ...
      if (sendSupervisorLifecycleMessage({ type: "rocky:shutdown" })) {
        return;
      }
  ...
  try {
    daemon = await createRockyDaemon(
      {
        ...config,
  ...
  try {
    ...
    sendSupervisorLifecycleMessage({ type: "rocky:ready", listen });
  } catch (err) {
