...
describe("hostnames (vite-style)", () => {
  it("allows localhost by default", () => {
    expect(isHostnameAllowed("localhost:7767", undefined)).toBe(true);
  });

  it("allows subdomains of .localhost by default", () => {
    expect(isHostnameAllowed("foo.localhost:7767", undefined)).toBe(true);
  });

  it("allows IP addresses by default", () => {
    expect(isHostnameAllowed("127.0.0.1:7767", undefined)).toBe(true);
    expect(isHostnameAllowed("[::1]:7767", undefined)).toBe(true);
  });

  it("rejects non-default hosts when no allowlist is provided", () => {
    expect(isHostnameAllowed("evil.com:7767", undefined)).toBe(false);
  });

  it("allows any host when set to true", () => {
    expect(isHostnameAllowed("evil.com:7767", true)).toBe(true);
  });

  it("supports leading-dot patterns", () => {
    const hostnames = [".example.com"];
    expect(isHostnameAllowed("example.com:7767", hostnames)).toBe(true);
    expect(isHostnameAllowed("foo.example.com:7767", hostnames)).toBe(true);
    expect(isHostnameAllowed("foo.bar.example.com:7767", hostnames)).toBe(true);
    expect(isHostnameAllowed("notexample.com:7767", hostnames)).toBe(false);
  });
