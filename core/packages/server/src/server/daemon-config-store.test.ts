...
describe("DaemonConfigStore", () => {
  ...
  test("patch persists provider enabled flags into config.json", () => {
    const rockyHome = mkdtempSync(path.join(tmpdir(), "rocky-daemon-config-store-"));
    tempDirs.push(rockyHome);

    const initial = loadPersistedConfig(rockyHome);
  ...
  test("patch persists custom ACP provider overrides into config.json", () => {
    ...
    const persisted = loadPersistedConfig(rockyHome);
    expect(persisted.agents?.providers?.["rocky-e2e-acp"]).toEqual({
      extends: "acp",
      label: "Rocky E2E ACP",
      description: "E2E ACP provider fixture",
      command: ["npx", "-y", "--version"],
