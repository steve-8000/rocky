...
import { handleCreateRockyWorktreeRequest } from "./worktree-session.js";
...
describe("wire compatibility", () => {
  ...
  test("legacy worktree request shape normalizes to the same internal input as the new shape", async () => {
    ...
    const dependencies = {
      ...
      createRockyWorktreeWorkflow: workflow.create.bind(workflow),
    };
      ...
      attachments: [
        {
          ...
          url: "https://github.com/getrocky/rocky/issues/55",
        },
      ],
    });
    ...
    if (legacyRequest.type !== "create_rocky_worktree_request") {
      throw new Error("Expected legacy worktree request");
    }
    if (newRequest.type !== "create_rocky_worktree_request") {
      throw new Error("Expected new worktree request");
    }

    await handleCreateRockyWorktreeRequest(dependencies, legacyRequest);
    await handleCreateRockyWorktreeRequest(dependencies, newRequest);
