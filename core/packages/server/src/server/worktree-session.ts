...
import type { ArchiveRockyWorktreeDependencies } from "./rocky-worktree-archive-service.js";
import { toWorktreeWireError } from "./worktree-errors.js";
import {
  archiveRockyWorktreeCommand,
  createRockyWorktreeCommand,
  listRockyWorktreesCommand,
...
interface CreateRockyWorktreeInBackgroundDependencies {
  rockyHome?: string;
  worktreesRoot?: string;
...
interface CreateRockyWorktreeWorkflowDependencies extends CreateRockyWorktreeInBackgroundDependencies {
  createRockyWorktree: (
    input: CreateRockyWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
...
export type CreateRockyWorktreeSetupContinuationInput =
  | { kind: "workspace" }
...
export type CreateRockyWorktreeWorkflowResult = CreateRockyWorktreeResult & {
  setupContinuation?: AgentWorktreeSetupContinuation;
};

export type CreateRockyWorktreeWorkflowFn = (
  input: CreateRockyWorktreeInput,
  options?: {
    resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    setupContinuation?: CreateRockyWorktreeSetupContinuationInput;
...
interface HandleCreateRockyWorktreeRequestDependencies {
  rockyHome?: string;
  worktreesRoot?: string;
...
export async function buildAgentSessionConfig(
  ...
  if (normalized.createWorktree) {
    ...
    const createdWorktree = await dependencies.createRockyWorktree(
      {
        cwd,
...
export async function handleRockyWorktreeListRequest(
  dependencies: {
    emit: EmitSessionMessage;
    rockyHome?: string;
    workspaceGitService: WorkspaceGitService;
  ...
  try {
    const worktrees = await listRockyWorktreesCommand(
...
export async function handleRockyWorktreeArchiveRequest(
  dependencies: Omit<
  ...
  try {
    const result = await archiveRockyWorktreeCommand(dependencies, {
      requestId,
...
export async function handleCreateRockyWorktreeRequest(
  dependencies: HandleCreateRockyWorktreeRequestDependencies,
  request: Extract<SessionInboundMessage, { type: "create_rocky_worktree_request" }>,
): Promise<void> {
  try {
    const commandResult = await createRockyWorktreeCommand(
      {
        rockyHome: dependencies.rockyHome,
        worktreesRoot: dependencies.worktreesRoot,
        createRockyWorktreeWorkflow: dependencies.createRockyWorktreeWorkflow,
      },
      {
        cwd: request.cwd,
...
export async function createRockyWorktreeWorkflow(
  dependencies: CreateRockyWorktreeWorkflowDependencies,
  input: CreateRockyWorktreeInput,
  options?: {
    resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    setupContinuation?: CreateRockyWorktreeSetupContinuationInput;
...
export async function runWorktreeSetupInBackground(
  dependencies: CreateRockyWorktreeInBackgroundDependencies,
  options: {
    requestCwd: string;
    repoRoot: string;
    workspaceId: string;
