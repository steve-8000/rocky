  ...
  describe("runAsyncWorktreeBootstrap", () => {
    ...
    it("keeps only the final carriage-return-updated content in command logs", async () => {
      ...
      const persistedSetupItem = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" && item.name === "rocky_worktree_setup",
      );
    ...
    it("shares the same worktree runtime port across setup and bootstrap terminals", async () => {
      writeFileSync(
        ...
        JSON.stringify({
          worktree: {
            setup: ['echo "$ROCKY_WORKTREE_PORT" > setup-port.txt'],
            terminals: [
              {
                name: "Port Terminal",
                command: "true",
      ...
      expect(registeredEnvs[0]?.env.ROCKY_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalEnvs.length).toBeGreaterThan(0);
      expect(createTerminalEnvs[0]?.ROCKY_WORKTREE_PORT).toBe(setupPort);

      const terminalToolCall = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" &&
          item.name === "rocky_worktree_terminals" &&
    ...
    it("injects real peer service env into terminal-backed services", async () => {
      ...
      expect(apiEnv.ROCKY_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:7767",
      );
      expect(apiEnv.ROCKY_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:7767",
      );
      expect(apiEnv.ROCKY_SERVICE_API_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.ROCKY_SERVICE_WEB_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.ROCKY_URL).toBe(apiEnv.ROCKY_SERVICE_API_URL);
      expect(apiEnv.ROCKY_PORT).toBe(apiEnv.ROCKY_SERVICE_API_PORT);
      expect(apiEnv).not.toHaveProperty("PORT");

      expect(webEnv.ROCKY_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:7767",
      );
      expect(webEnv.ROCKY_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:7767",
      );
      expect(webEnv.ROCKY_SERVICE_API_PORT).toBe(apiEnv.ROCKY_SERVICE_API_PORT);
      expect(webEnv.ROCKY_SERVICE_WEB_PORT).toBe(apiEnv.ROCKY_SERVICE_WEB_PORT);
      expect(webEnv.ROCKY_URL).toBe(webEnv.ROCKY_SERVICE_WEB_URL);
      expect(webEnv.ROCKY_PORT).toBe(webEnv.ROCKY_SERVICE_WEB_PORT);
      expect(webEnv).not.toHaveProperty("PORT");

      const apiPort = Number(apiEnv.ROCKY_SERVICE_API_PORT);
      const webPort = Number(apiEnv.ROCKY_SERVICE_WEB_PORT);
