...
import { createRockyDaemon, type RockyDaemonConfig } from "../bootstrap.js";
...
describe("agent MCP end-to-end (offline)", () => {
  test("create_agent runs initial prompt and affects filesystem", async () => {
    const rockyHome = await mkdtemp(path.join(os.tmpdir(), "rocky-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "rocky-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "rocky-agent-cwd-"));
    const port = await getAvailablePort();

    const daemonConfig: RockyDaemonConfig = {
    ...
    const daemon = await createRockyDaemon(daemonConfig, pino({ level: "silent" }));
    await daemon.start();
  ...
  test("create_agent auto-injects rocky MCP by default and can be disabled", async () => {
    const rockyHome = await mkdtemp(path.join(os.tmpdir(), "rocky-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "rocky-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "rocky-agent-cwd-"));
    const port = await getAvailablePort();
    ...
    const disabledRockyHome = await mkdtemp(path.join(os.tmpdir(), "rocky-home-disabled-"));
    const disabledStaticDir = await mkdtemp(path.join(os.tmpdir(), "rocky-static-disabled-"));
    const disabledAgentCwd = await mkdtemp(path.join(os.tmpdir(), "rocky-agent-cwd-disabled-"));
    const disabledPort = await getAvailablePort();
    const disabledDaemonConfig: RockyDaemonConfig = {
      listen: `127.0.0.1:${disabledPort}`,
      rockyHome: disabledRockyHome,
      corsAllowedOrigins: [],
      hostnames: true,
      mcpEnabled: true,
      ...
      mcpDebug: false,
      agentClients: createTestAgentClients(),
      agentStoragePath: path.join(disabledRockyHome, "agents"),
    };
    const disabledDaemon = await createRockyDaemon(disabledDaemonConfig, pino({ level: "silent" }));
    ...
    try {
      ...
      expect(disabledAgent?.config.mcpServers?.rocky).toBeUndefined();
    } finally {
      if (agentId) {
        await client.callTool({ name: "kill_agent", args: { agentId } });
      }
      ...
      await rm(disabledRockyHome, { recursive: true, force: true });
  ...
  test("create_agent with worktree is async and boots terminals only after setup success", async () => {
    const rockyHome = await mkdtemp(path.join(os.tmpdir(), "rocky-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "rocky-static-"));
    const repoRoot = await mkdtemp(path.join(os.tmpdir(), "rocky-worktree-repo-"));
    const port = await getAvailablePort();
    ...
    try {
      ...
      const setupCommand =
        'while [ ! -f "$ROCKY_WORKTREE_PATH/allow-setup" ]; do sleep 0.05; done; echo "done" > "$ROCKY_WORKTREE_PATH/setup-done.txt"';
      await writeFile(
        path.join(repoRoot, "rocky.json"),
        JSON.stringify({
          worktree: {
            setup: [setupCommand],
            terminals: [
      ...
      execSync("git add rocky.json", { cwd: repoRoot, stdio: "pipe" });
