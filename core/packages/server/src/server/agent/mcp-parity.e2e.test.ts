...
let tempRoot: string;
let daemonHandle: TestRockyDaemon;
...
beforeAll(async () => {
  ...
  daemonHandle = await createTestRockyDaemon();
...
describe("Suite A: Core Fixes", () => {
  ...
  test("agentManager.createAgent injects rocky MCP using the daemon listen target", async () => {
    let agentId: string | null = null;
    try {
