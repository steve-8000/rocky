...
import type {
  ...
  StoredSchedule,
  UpdateScheduleInput,
} from "@getrocky/protocol/schedule/types";
...
function configureOpenCodeProviderStub(
  ...
  const opencodeModes: AgentMode[] = [
    ...
    { id: "rocky-custom", label: "Rocky Custom", description: "Custom OpenCode agent" },
  ];
  ...
  const customOpenCodeModes: AgentMode[] = [
    ...opencodeModes,
    { id: "rocky-custom", label: "Rocky Custom" },
  ];
...
function createRockyWorktreeForMcpTest(options: {
  ...
  return async (input, serviceOptions) => {
    ...
    const result = await createRockyWorktreeService(input, {
      github,
...
describe("create_agent MCP tool", () => {
  ...
  it("registers and broadcasts a workspace when create_agent creates a worktree", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "rocky-mcp-worktree-"));
    const repoDir = join(tempDir, "repo");
    const rockyHome = join(tempDir, ".rocky");
    ...
    try {
      ...
      const server = await createAgentMcpServer({
        ...
        providerSnapshotManager: createOpenCodeManager().manager,
        rockyHome,
        createRockyWorktree: createRockyWorktreeForMcpTest({
          rockyHome,
          broadcasts,
          createdWorkspaceIds,
          setupContinuations,
  ...
  it("creates a create_agent branch-off worktree without invoking the legacy metadata branch rename", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "rocky-mcp-agent-worktree-name-context-"));
    const repoDir = join(tempDir, "repo");
    const rockyHome = join(tempDir, ".rocky");
    const broadcasts: string[] = [];
    ...
    try {
      ...
      const server = await createAgentMcpServer({
        ...
        providerSnapshotManager: createOpenCodeManager().manager,
        rockyHome,
        createRockyWorktree: createRockyWorktreeForMcpTest({ rockyHome, broadcasts }),
  ...
  it("does not auto-rename a create_agent checkout worktree from the initial prompt", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "rocky-mcp-agent-checkout-name-context-"));
    const repoDir = join(tempDir, "repo");
    const rockyHome = join(tempDir, ".rocky");
    const broadcasts: string[] = [];
    const workspaceGitService = {
  ...
  it("passes create_agent GitHub PR worktrees through workspace creation without metadata branch rename", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const startedAgentSetupIds: string[] = [];
    const createRockyWorktree = vi.fn(
      async (
        input: CreateRockyWorktreeInput,
        options?: Parameters<CreateRockyWorktreeWorkflowFn>[1],
      ) => ({
        worktree: {
          branchName: "pr-123",
  ...
  it("registers and broadcasts a workspace when create_worktree creates a worktree", async () => {
    const { agentManager, agentStorage } = createTestDeps();
    const tempDir = await mkdtemp(join(tmpdir(), "rocky-mcp-create-worktree-"));
    const repoDir = join(tempDir, "repo");
    const rockyHome = join(tempDir, ".rocky");
    const broadcasts: string[] = [];
    const setupContinuations: Array<"workspace" | "agent" | undefined> = [];
  ...
  it("forces a workspace git snapshot refresh when archive_worktree deletes a worktree", async () => {
    ...
    const tempDir = realpathSync.native(
      await mkdtemp(join(tmpdir(), "rocky-mcp-archive-worktree-")),
    );
    const repoDir = join(tempDir, "repo");
    const rockyHome = join(tempDir, ".rocky");
  ...
  it("archives a worktree by slug", async () => {
    const { agentManager, agentStorage } = createTestDeps();
    const tempDir = realpathSync.native(
      await mkdtemp(join(tmpdir(), "rocky-mcp-archive-worktree-slug-")),
    );
    const repoDir = join(tempDir, "repo");
    const rockyHome = join(tempDir, ".rocky");
  ...
  it("routes list_worktrees through WorkspaceGitService", async () => {
    ...
    const workspaceGitService = {
      ...
      listWorktrees: vi.fn(async () => [
        {
          path: "/tmp/rocky/worktrees/repo/feature",
          branchName: "feature",
  ...
  it("allows caller agents to override cwd and applies caller context labels", async () => {
    const { agentManager, agentStorage, spies } = createTestDeps();
    const baseDir = await mkdtemp(join(tmpdir(), "rocky-mcp-test-"));
