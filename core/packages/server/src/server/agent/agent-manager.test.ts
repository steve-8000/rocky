  ...
  override async createSession(
    ...
    const script = `
      process.stdout.write(JSON.stringify({
        ...
        agentId: process.env.ROCKY_AGENT_ID ?? null
      }));
    `;
...
test("createAgent injects rocky MCP server when manager has an MCP base URL", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  ...
  const manager = new AgentManager({
    ...
    registry: storage,
    logger,
    mcpBaseUrl: "http://127.0.0.1:7767/mcp/agents",
    idFactory: () => "00000000-0000-4000-8000-000000000103",
  });
  ...
  expect(snapshot.config.mcpServers).toEqual({
    rocky: {
      type: "http",
      url: `http://127.0.0.1:7767/mcp/agents?callerAgentId=${snapshot.id}`,
    },
    custom: {
      type: "stdio",
      command: "custom-mcp",
...
test("createAgent preserves a user-provided rocky MCP config", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  ...
  const resumed = await manager.resumeAgentFromPersistence(handle, {
    ...
    mcpServers: {
      rocky: {
        type: "stdio",
        command: "node",
        args: ["/tmp/mcp-bridge.mjs", "--socket", "/tmp/rocky.sock"],
      },
    },
  });
...
test("user_message events wrapping a rocky-system envelope are not added to the timeline", async () => {
...
test("user_message events wrapping a rocky-system envelope are not restored during history replay", async () => {
