...
describe("OpenCodeAgentClient adapter smoke tests", () => {
  ...
  test("custom agents defined in opencode.json appear in available modes", async () => {
    ...
    openCodeClient.appAgentsResponse = {
      data: [
        {
          name: "rocky-test-custom",
          description: "Custom agent defined for Rocky integration test",
          mode: "primary",
        },
    ...
    const custom = modes.find((mode) => mode.id === "rocky-test-custom");
    expect(custom).toBeDefined();
    expect(custom!.label).toBe("Rocky-test-custom");
    expect(custom!.description).toBe("Custom agent defined for Rocky integration test");
    ...
    try {
      const session = await client.createSession({
        ...
        mcpServers: {
          rocky: {
            type: "http",
            url: "http://127.0.0.1:7767/mcp/agents?callerAgentId=test-agent",
          },
        },
      });
  ...
  test("fails the turn when OpenCode reports MCP add failure in data payload", async () => {
    ...
    try {
      ...
      await expect(collectTurnEvents(streamSession(session, "hello"))).rejects.toThrow(
        /Failed to add OpenCode MCP server 'rocky': SSE error/,
      );
