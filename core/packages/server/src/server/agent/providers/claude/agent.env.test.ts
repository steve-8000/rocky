...
describe("Claude SDK env", () => {
  test("forwards launch-context env through Claude process env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        ROCKY_AGENT_ID: "00000000-0000-4000-8000-000000000201",
        ROCKY_TEST_FLAG: "launch-value",
      },
    };
    ...
    try {
      ...
      expect(capturedEnv?.ROCKY_AGENT_ID).toBe(launchContext.env?.ROCKY_AGENT_ID);
      expect(capturedEnv?.ROCKY_TEST_FLAG).toBe(launchContext.env?.ROCKY_TEST_FLAG);
    } finally {
      await session.close();
    }
  });
  ...
  test("forwards launch-context env through Claude resume env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        ROCKY_AGENT_ID: "00000000-0000-4000-8000-000000000202",
        ROCKY_TEST_FLAG: "resume-launch-value",
      },
    };
    const queryFactory = vi.fn(({ options }: ClaudeQueryInput) => {
      capturedEnv = options.env;
