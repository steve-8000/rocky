...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "ROCKY_ENV_SENTINEL_SECRET";
  const runtimeSecret = "ROCKY_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "ROCKY_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.ROCKY_TEST_SENTINEL_SECRET;
  process.env.ROCKY_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        ROCKY_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.ROCKY_TEST_SENTINEL_SECRET;
    } else {
      process.env.ROCKY_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
