  ...
  it("maps Glob calls as search detail using pattern input", () => {
    const item = expectMapped(
      mapClaudeCompletedToolCall({
        ...
        name: "Glob",
        input: { pattern: "**/.claude/commands/rocky*" },
        output: {
          durationMs: 7,
          numFiles: 2,
  ...
  it("maps Grep calls as search detail using pattern input", () => {
    const item = expectMapped(
      mapClaudeCompletedToolCall({
        ...
        input: {
          pattern: '\\\\\\"cli\\\\\\""',
          path: "/Users/moboudra/dev/rocky/packages/desktop/src",
          output_mode: "content",
          "-n": true,
        },
        output: {
