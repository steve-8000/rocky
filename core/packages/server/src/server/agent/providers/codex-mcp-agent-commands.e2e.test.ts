...
describe("codex agent commands E2E", () => {
  ...
  test("lists available slash commands for a codex agent", async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "---",
        "",
        "Say hello to $NAME and then output exactly ROCKY_OK.",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    writeFileSync(
      ...
      [
        ...
        "---",
        "",
        "When invoked, respond with exactly ROCKY_SKILL_OK.",
        "",
      ].join("\n"),
      "utf8",
    );
  ...
  test("executes a custom prompt via normal sendMessage path (prompts:*)", async () => {
    ...
    const promptsDir = path.join(codexHome, "prompts");
    mkdirSync(promptsDir, { recursive: true });
    const promptPath = path.join(promptsDir, "rocky-test-sayok.md");
    writeFileSync(
      promptPath,
      [
        "---",
    ...
    await ctx.client.sendMessage(agent.id, "/prompts:rocky-test-sayok NAME=world");
    const state = await ctx.client.waitForFinish(agent.id, 30_000);

    expect(state.status).toBe("idle");
    expect(state.lastMessage).toContain("ROCKY_OK");
  ...
  test("sendMessage routes known slash commands through standard run/stream", async () => {
    ...
    mkdirSync(promptsDir, { recursive: true });
    writeFileSync(
      path.join(promptsDir, "rocky-test-sayok.md"),
      ["---", "description: Say OK", "---", "", "Output exactly: ROCKY_OK", ""].join("\n"),
      "utf8",
    );
    process.env.CODEX_HOME = codexHome;
    ...
    try {
      ...
      expect(state.status).toBe("idle");
      expect(state.lastMessage).toContain("ROCKY_OK rocky-test-sayok");
    } finally {
      if (prevCodexHome === undefined) {
        delete process.env.CODEX_HOME;
      } else {
  ...
  test("sendMessage keeps unknown slash input as plain prompt text", async () => {
    ...
    expect(state.lastMessage).not.toContain("ROCKY_SKILL_OK");
  }, 30_000);
