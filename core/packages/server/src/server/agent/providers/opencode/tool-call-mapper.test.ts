...
describe("opencode tool-call mapper", () => {
  ...
  it("unwraps OpenCode XML read output into file content", () => {
    const item = expectMapped(
      mapOpencodeToolCall({
        ...
        status: "completed",
        input: { filePath: "/Users/moboudra/dev/rocky/docs/release.md" },
        output: [
          "<path>/Users/moboudra/dev/rocky/docs/release.md</path>",
          "<type>file</type>",
          "<content>",
          "1: # Release",
          "2:",
    ...
    expect(item.detail).toEqual({
      type: "read",
      filePath: "/Users/moboudra/dev/rocky/docs/release.md",
      content: [
        "1: # Release",
        "2:",
        "3: All workspaces share one version and release together.",
  ...
  it("maps completed write calls with OpenCode success text into canonical detail", () => {
    const item = expectMapped(
      mapOpencodeToolCall({
        ...
        status: "completed",
        input: {
          filePath: "/Users/moboudra/.rocky/worktrees/1luy0po7/cold-ladybug/dummy.txt",
          content: "hello world\n",
        },
  ...
  it("maps completed edit calls with OpenCode camelCase input and success text", () => {
    const item = expectMapped(
      mapOpencodeToolCall({
        ...
        status: "completed",
        input: {
          filePath: "/Users/moboudra/dev/rocky/packages/website/src/data/agent-pages.ts",
  ...
  it("does not apply cross-provider speak normalization in opencode mapper", () => {
    ...
    expect(item.name).toBe("rocky_voice.speak");
    expect(item.detail).toEqual({
      type: "unknown",
      input: { text: "Voice response from OpenCode." },
      output: { ok: true },
