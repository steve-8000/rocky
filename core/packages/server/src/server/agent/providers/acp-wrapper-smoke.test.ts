...
for (const config of wrappers) {
  ...
  wrapperDescribe(`real ${config.id} ACP wrapper smoke`, () => {
    ...
    test("(e) real assistant tool call becomes a Rocky tool snapshot and observes permission flow when emitted", async () => {
      const trace = createTrace(config);
      installWireCapture(trace);
      let session: ACPAgentSession | null = null;
      ...
      try {
        ...
        trace.criteria.toolSnapshot = {
          outcome: "PASS",
          detail:
            "Read-file prompt produced ACP tool_call/tool_call_update notifications and Rocky tool snapshots.",
        };
