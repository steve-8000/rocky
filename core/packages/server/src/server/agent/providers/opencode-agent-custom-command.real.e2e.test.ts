...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through Rocky using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "rocky-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: Rocky issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
