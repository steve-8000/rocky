...
describe("PiRpcAgentClient", () => {
  ...
  test("maps extension, prompt, and skill commands to Rocky slash commands", async () => {
    const { pi, session } = await createSession();
    ...
    const session = await client.createSession(
      createConfig({
        mcpServers: {
          rocky: {
            type: "http",
            url: "http://127.0.0.1:7767/mcp/agents?callerAgentId=agent-1",
          },
          localSecret: {
            type: "stdio",
            command: "node",
