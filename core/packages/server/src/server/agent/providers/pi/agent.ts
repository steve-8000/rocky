...
const ROCKY_PI_TREE_EXTENSION_COMMAND = "rocky_tree";
const ROCKY_PI_CAPTURE_EXTENSION_COMMAND = "rocky_capture_entries";
const ROCKY_PI_ENTRY_CAPTURE_MARKER = "ROCKY_ENTRY_CAPTURE";
const ROCKY_PI_COMMAND_RESULT_MARKER = "ROCKY_COMMAND_RESULT";
const ROCKY_PI_EXTENSION_RESULT_TIMEOUT_MS = 10_000;
...
function createPiMcpConfigFile(servers: Record<string, McpServerConfig>): PiMcpConfigFile {
  const dir = mkdtempSync(join(tmpdir(), "rocky-pi-mcp-"));
...
function createPiRockyExtensionFile(): PiTempFile {
  const dir = mkdtempSync(join(tmpdir(), "rocky-pi-extension-"));
  const filePath = join(dir, "rocky-integration.mjs");
  writeFileSync(
    filePath,
    `
	...
	function emitCommandResult(ctx, requestId, result) {
	  ctx.ui.notify(
	    "${ROCKY_PI_COMMAND_RESULT_MARKER} " + JSON.stringify({ requestId, ...result }),
	    result.ok ? "info" : "error",
	  );
	}
	
	export default function rockyIntegration(pi) {
	  ...
	  pi.registerCommand("${ROCKY_PI_CAPTURE_EXTENSION_COMMAND}", {
	    description: "Internal Rocky entry capture bridge",
	  ...
	  pi.registerCommand("${ROCKY_PI_TREE_EXTENSION_COMMAND}", {
	    description: "Internal Rocky tree navigation bridge",
	    handler: async (args, ctx) => {
	      const payload = decodePayload(args.trim());
	      try {
...
export class PiRpcAgentSession implements AgentSession {
  ...
  private async runPiTreeExtensionCommand(targetId: string): Promise<unknown> {
    ...
    await this.runtimeSession.prompt(`/${ROCKY_PI_TREE_EXTENSION_COMMAND} ${payload}`);
  ...
  private async requestEntryCapture(reason: string): Promise<void> {
    ...
    await this.runtimeSession.prompt(`/${ROCKY_PI_CAPTURE_EXTENSION_COMMAND} ${payload}`);
    await resultPromise;
  }
  ...
  private waitForExtensionResult(requestId: string): Promise<unknown> {
    return new Promise((resolve, reject) => {
      ...
      }, ROCKY_PI_EXTENSION_RESULT_TIMEOUT_MS);
  ...
  private handleEntryCaptureMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, ROCKY_PI_ENTRY_CAPTURE_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleCommandResultMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, ROCKY_PI_COMMAND_RESULT_MARKER);
    if (!payload) {
      return false;
    }
    if (typeof payload.requestId !== "string") {
...
export class PiRpcAgentClient implements AgentClient {
  ...
  async createSession(
    ...
    const rockyExtension = createPiRockyExtensionFile();
    ...
    try {
      runtimeSession = await this.runtime.startSession({
        ...
        extensionPaths: [rockyExtension.path],
      });
    } catch (error) {
      mcpConfig?.cleanup();
      rockyExtension.cleanup();
      throw error;
    }
    try {
      return new PiRpcAgentSession({
        ...
        cleanup: combineCleanup([mcpConfig?.cleanup, rockyExtension.cleanup]),
      });
    } catch (error) {
    ...
    try {
      ...
      return {
        diagnostic: formatProviderDiagnostic("Pi", [
          ...
          { label: "Rocky MCP tools", value: mcpToolsValue },
