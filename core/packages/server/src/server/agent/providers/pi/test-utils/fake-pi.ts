...
export class FakePiSession implements PiRuntimeSession {
  ...
  private handleEntryCaptureCommand(message: string): void {
    const prefix = "/rocky_capture_entries ";
    if (!message.startsWith(prefix)) {
      return;
    }
    const payload = JSON.parse(
  ...
  emitEntryCapture(requestId?: string, reason = "test"): void {
    this.emit({
      ...
      method: "notify",
      message: `ROCKY_ENTRY_CAPTURE ${JSON.stringify({
        reason,
        requestId,
  ...
  private emitExtensionCommandResult(
    ...
    this.emit({
      ...
      method: "notify",
      message: `ROCKY_COMMAND_RESULT ${JSON.stringify({ requestId, ...result })}`,
    });
  }
}
