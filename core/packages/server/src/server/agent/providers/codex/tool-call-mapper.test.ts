...
describe("codex tool-call mapper", () => {
  ...
  it("normalizes codex rocky speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
  ...
  it("normalizes codex rocky_voice.speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
        ...
        server: "rocky_voice",
        tool: "speak",
        arguments: { text: "Voice response from Codex via rocky_voice." },
        result: { ok: true },
      }),
    );
    ...
    expect(item.detail).toEqual({
      type: "unknown",
      input: "Voice response from Codex via rocky_voice.",
      output: null,
    });
  });

  it("normalizes codex rocky speak tool-call names and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallEnvelope({
