...
const capturePath = process.env.ROCKY_FAKE_CODEX_CAPTURE;
let buffer = "";
    ...
    providerOverrides: {
      [providerId]: {
        ...
        env: {
          ...
          ROCKY_FAKE_CODEX_CAPTURE: capturedRequestsPath,
        },
      },
    },
  });
...
describe("Codex app-server provider", () => {
  ...
  test("resolves Codex skill slash commands into app-server skill input", async () => {
          ...
          data: [
            {
              ...
              skills: [
                {
                  name: "rocky-implement",
                  description: "Execute an existing Rocky plan.",
                  path: "/tmp/skills/rocky-implement/SKILL.md",
                },
              ],
              errors: [],
            },
    ...
    await session.startTurn("/rocky-implement in a worktree, remember to use Claude for the UI");
    ...
    expect(turnStartCall?.[1]).toEqual(
      expect.objectContaining({
        input: [
          ...
          {
            type: "text",
            text: "$rocky-implement in a worktree, remember to use Claude for the UI",
            text_elements: [],
          },
        ],
      }),
    ...
    const commands = await listCommandsFromFakeCodex([
      {
        ...
        path: "/Users/test/.agents/skills/rocky/SKILL.md",
      },
      {
        name: "rocky",
        description: "Shared orchestration skill.",
        path: "/Users/test/.codex/skills/rocky/SKILL.md",
      },
    ]);

    expect(commands.filter((command) => command.name === "rocky")).toEqual([
      {
        name: "rocky",
        description: "Shared orchestration skill.",
        argumentHint: "",
  ...
  test("builds app-server env from launch-context env overrides", () => {
    const launchContext: AgentLaunchContext = {
      env: {
        ROCKY_AGENT_ID: "00000000-0000-4000-8000-000000000301",
        ROCKY_TEST_FLAG: "codex-launch-value",
      },
    };
    const env = buildCodexAppServerEnv(
      {
        env: {
          ROCKY_AGENT_ID: "runtime-value",
          ROCKY_TEST_FLAG: "runtime-test-value",
        },
      },
      launchContext.env,
    );

    expect(env.ROCKY_AGENT_ID).toBe(launchContext.env?.ROCKY_AGENT_ID);
    expect(env.ROCKY_TEST_FLAG).toBe(launchContext.env?.ROCKY_TEST_FLAG);
  });
    ...
    expect(events).toEqual([
      {
        ...
        item: {
          type: "assistant_message",
          text: "![Image](/tmp/rocky image.png)",
        },
      },
    ]);
  });
  ...
  test("materializes imageGeneration base64 results before rendering markdown", () => {
    ...
    expect(source).toMatch(/rocky-attachments[\\/].+\.png$/);
