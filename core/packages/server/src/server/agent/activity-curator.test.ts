  ...
  it("renders terminal tool calls as one-line command summaries", () => {
    const timeline: AgentTimelineItem[] = [
      toolCallItem({
        ...
        detail: {
          type: "plain_text",
          label: `skills/rocky-chat/bin/chat.sh post --room storage-revamp --body $'first line
...
second line'`,
    ...
    expect(result).toContain(
      "[Terminal] skills/rocky-chat/bin/chat.sh post --room storage-revamp --body $'first line second line'",
    );
  ...
  it("falls back to input json for likely external tools", () => {
    ...
    expect(result).toBe('[rocky__create_agent] {"cwd":"/tmp/repo","initialPrompt":"do the thing"}');
  });
