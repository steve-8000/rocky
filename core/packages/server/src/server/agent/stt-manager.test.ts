...
describe("STTManager", () => {
  ...
  it("uses ROCKY_VOICE_LANGUAGE over ROCKY_DICTATION_LANGUAGE", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses ROCKY_DICTATION_LANGUAGE when ROCKY_VOICE_LANGUAGE is unset", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.ROCKY_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.ROCKY_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.ROCKY_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.ROCKY_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
