...
import type {
  ...
  CreateRockyWorktreeWorkflowResult,
...
interface CreateAgentLifecycleDispatchDependencies {
  ...
  createRockyWorktreeWorkflow: CreateRockyWorktreeWorkflowFn;
...
export class CreateAgentLifecycleDispatch {
  ...
  }): Promise<CreateRockyWorktreeWorkflowResult | null> {
  ...
  registerAutoArchiveIfRequested(input: {
    ...
    agentId: string;
    createdWorktree: CreateRockyWorktreeWorkflowResult | null;
  }): void {
  ...
  private async createWorktreeForTarget(
    ...
    const baseInput = {
      ...
      firstAgentContext,
      runSetup: false,
      rockyHome: this.dependencies.rockyHome,
      worktreesRoot: this.dependencies.worktreesRoot,
    } as const;

    switch (target.mode) {
      case "branch-off":
        return this.dependencies.createRockyWorktreeWorkflow(
          {
            ...baseInput,
  ...
  private async archiveAutoCreatedWorktree(options: {
    ...
    const ownership = await isRockyOwnedWorktreeCwd(options.worktreePath, {
      rockyHome: this.dependencies.rockyHome,
      worktreesRoot: this.dependencies.worktreesRoot,
    });
    if (!ownership.allowed) {
      throw new Error("Auto-created worktree is not a Rocky-owned worktree");
    }
