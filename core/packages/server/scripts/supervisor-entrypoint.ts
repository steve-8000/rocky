...
function resolveWorkerExecArgv(workerEntry: string, devMode: boolean): string[] {
  ...
  const devArgs = [
    ...
    "--report-directory=/tmp/rocky-reports",
  ];
  const inspectArg = process.env.ROCKY_NODE_INSPECT ?? "--inspect";
...
function resolvePackagedNodeEntrypointRunnerPath(currentScriptPath: string): string | null {
  const packageMarker = `${path.sep}node_modules${path.sep}@getrocky${path.sep}server${path.sep}`;
...
async function main(): Promise<void> {
  ...
  const rockyHome = resolveRockyHome(workerEnv);
  const persistedConfig = loadPersistedConfig(rockyHome);
  const supervisorLogFile = resolveSupervisorLogFile(rockyHome, persistedConfig, workerEnv);

  try {
    await acquirePidLock(rockyHome, null, {
  ...
  const releaseLock = async (): Promise<void> => {
    ...
    lockReleased = true;
    await releasePidLock(rockyHome, {
      ownerPid: process.pid,
    });
  };
  ...
  runSupervisor({
    ...
    onWorkerReady: async ({ listen }) => {
      await updatePidLock(rockyHome, { listen }, { ownerPid: process.pid });
    },
