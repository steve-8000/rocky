...
export function resolveSupervisorLogFile(
  ...
  const envRotateSize = env.ROCKY_LOG_ROTATE_SIZE?.trim();
  const envRotateMaxFiles = parseOptionalPositiveInteger(env.ROCKY_LOG_ROTATE_COUNT);
  let logPath = path.join(rockyHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (configuredPath) {
    logPath = path.isAbsolute(configuredPath)
      ? configuredPath
      : path.resolve(rockyHome, configuredPath);
  }
