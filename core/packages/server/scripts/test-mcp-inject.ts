...
async function verifyInjectedMcpForProvider(
  ...
  try {
    const prompt = [
      ...
      "If you have a tool called list_agents or create_agent from a rocky MCP server, call list_agents once.",
      "After checking, reply with exactly ROCKY_MCP_FOUND.",
      "If you do not have those tools, reply with exactly ROCKY_MCP_NOT_FOUND.",
    ...
    if (!assistantText.includes("ROCKY_MCP_FOUND")) {
      throw new Error(
        `Expected assistant to confirm Rocky MCP availability. Assistant text:\n${assistantText}`,
      );
    }
...
async function main(): Promise<void> {
  ...
  const rootCwd = await mkdtemp(path.join(os.tmpdir(), "rocky-mcp-inject-real-"));
