...
import { createTestRockyDaemon } from "../src/server/test-utils/rocky-daemon.js";
...
function parseArgs(): { sourceHome: string; frozenHomeRoot: string | null; scenario: Scenario } {
  let sourceHome = process.env.ROCKY_BENCHMARK_SOURCE_HOME ?? path.join(os.homedir(), ".rocky");
  let frozenHomeRoot = process.env.ROCKY_BENCHMARK_FROZEN_HOME_ROOT ?? null;
  let scenario = (process.env.ROCKY_BENCHMARK_SCENARIO ?? "snapshotOnly") as Scenario;
...
async function freezeHome(sourceHome: string, requestedRoot: string | null): Promise<string> {
  const frozenHomeRoot = requestedRoot ?? mkdtempSync(path.join(os.tmpdir(), "rocky-real-home-"));
  if (process.env.ROCKY_BENCHMARK_REUSE_FROZEN_HOME === "1") {
    return frozenHomeRoot;
  }
  const frozenHome = path.join(frozenHomeRoot, ".rocky");
...
async function main(): Promise<void> {
  ...
  const daemon = await createTestRockyDaemon({ rockyHomeRoot: frozenHomeRoot, cleanup: false });
