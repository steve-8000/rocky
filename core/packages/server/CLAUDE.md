# AGENTS.md — Rocky Server Development Guide

For AI coding agents working in `packages/server`. Supplements [CLAUDE.md](../CLAUDE.md) at the repo root.

## Project Overview

Rocky is a mobile + CLI app for monitoring and controlling local AI coding agents (Claude Code, Codex, GitHub Copilot, OpenCode, Pi). The daemon runs on your machine, manages agent processes, and streams their output over WebSocket to clients.
...
1. **NEVER restart the daemon on port 7767** — it kills your own process
...
Agent state persists to `$ROCKY_HOME/agents/{cwd-with-dashes}/{agent-id}.json`  
Daemon logs: `$ROCKY_HOME/daemon.log`
...
```bash
tail -f $ROCKY_HOME/daemon.log      # Daemon logs
