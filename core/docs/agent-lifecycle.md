...
Agents can launch other agents via the agent-scoped `create_agent` MCP tool. Agent-scoped creation is always asynchronous. By default, the daemon stamps the created agent with a label `rocky.parent-agent-id` pointing back at the agent that created it. The client surfaces that as `agent.parentAgentId`.

Agent-scoped `create_agent` accepts `detached: true` for agents that should stand on their own. The daemon still uses the creating agent for cwd/config inheritance, but does not write `rocky.parent-agent-id`.
...
`create_agent_request` can opt an agent into `autoArchive`. In that mode the daemon archives the agent after the first terminal turn event (`turn_completed`, `turn_failed`, or `turn_canceled`). If the same request created a Rocky worktree through its `worktree` field, auto-archive archives that worktree too, which removes the agent records inside the worktree.
...
5. **Cascade-archive children** — any agent whose `rocky.parent-agent-id` label matches the archived agent gets archived too, recursively
...
| `labels["rocky.parent-agent-id"]` | `string?`     | Parent agent ID, set automatically by agent-scoped `create_agent` unless `detached: true` |
