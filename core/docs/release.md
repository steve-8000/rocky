...
- Betas publish npm packages with `--tag beta`, so `npm install @getrocky/cli@beta` opts in while plain `npm install @getrocky/cli` stays on `latest`
...
```jsonc
// mcp__rocky__create_heartbeat arguments
{
...
- `version:all:*` bumps root + syncs workspace versions and `@getrocky/*` dependency versions
...
The changelog is shown on the Rocky homepage. Write it for **end users**, not developers.
...
Format: append `([#123](https://github.com/getrocky/rocky/pull/123) by [@user](https://github.com/user))` at the end of each bullet. For changes spanning multiple PRs or contributors:

```markdown
- Voice mode now works on tablets with proper microphone permissions. ([#210](https://github.com/getrocky/rocky/pull/210), [#215](https://github.com/getrocky/rocky/pull/215) by [@alice](https://github.com/alice), [@bob](https://github.com/bob))
```

Rules:

- **Always link the PR number** as `[#N](https://github.com/getrocky/rocky/pull/N)`.
