# Adding a New Provider to Rocky
...
Pi is a process-backed provider. Rocky requires the user to have the `pi` binary installed and talks to it through `pi --mode rpc`; the server package does not embed Pi's SDK/runtime packages.

Rocky's per-agent and daemon-wide system prompts are passed to Pi with `--append-system-prompt`, so Pi keeps its default coding prompt while receiving Rocky's additional instructions.

Pi MCP support depends on the open-source `pi-mcp-adapter` extension being loaded for the agent cwd. Probe with Pi RPC `get_commands`; the adapter registers an extension command named `mcp` (often with `sourceInfo.source` containing `pi-mcp-adapter`). When Rocky injects MCP servers into Pi, write a per-agent MCP config and pass it with `--mcp-config` instead of modifying user or project MCP files. For local HTTP servers such as Rocky's own `/mcp/agents` endpoint, explicitly disable adapter OAuth (`auth: false`, `oauth: false`) in the generated config.
...
OpenCode owns user message IDs. Do not pass Rocky-generated IDs to OpenCode prompt APIs; let OpenCode create `msg*` IDs and record the user timeline item from the `message.updated` event.
...
rocky run --provider my-provider

# Launch with a specific model and mode
rocky run --provider my-provider --model some-model --mode default

# List running agents
rocky ls -a -g

# Check if the provider reports models
rocky models --provider my-provider
```
