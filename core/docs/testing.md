...
App-level Playwright browser E2E lives in `packages/app/e2e/*.spec.ts` and runs via `npm run test:e2e --workspace=@getrocky/app` (separate from Vitest E2E). App Playwright specs that hit real providers use `*.real.spec.ts` and run through `npm run test:e2e:real --workspace=@getrocky/app`; the default app E2E project ignores that suffix so CI does not need provider credentials.
...
- Server: `packages/server/src/test-utils/vitest-setup.ts` loads `.env.test`, sets `ROCKY_SUPERVISED=0`, and disables Git/SSH prompts. Add new global env shims here, not in individual tests.
