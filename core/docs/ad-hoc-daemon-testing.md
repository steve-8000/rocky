...
do not use `createRockyDaemon` as a product launch path.
...
import { createRockyDaemon } from "./bootstrap.js";
import { DaemonClient } from "./test-utils/daemon-client.js";

const logger = pino({ level: "warn" });
const rockyHomeRoot = await mkdtemp(path.join(os.tmpdir(), "rocky-test-"));
...
const daemon = await createRockyDaemon(
  {
    ...
    relayEnabled: false,
    relayEndpoint: "relay.rocky.sh:443",
    appBaseUrl: "https://app.rocky.sh",
...
For simpler cases, `createTestRockyDaemon` + `DaemonClient` handles temp dirs and port selection:
...
The test helper does **not** expose `providerOverrides`. In test harnesses, use `createRockyDaemon` directly when you need it (see quick start above).
