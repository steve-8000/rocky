...
`ROCKY_HOME` is the directory that holds runtime state (agents, sockets, daemon log). Resolution rules:

- The **server itself** (e.g. when launched by the desktop app or `npm run start`) defaults to `~/.rocky` (see `packages/server/src/server/rocky-home.ts`).
- **`npm run dev` from a git worktree** derives a stable home like `~/.rocky-<worktree-name>` and, on first run, seeds it from `~/.rocky` by copying agent/project JSON metadata and `config.json`. Checkout/worktree directories are not copied.
- **`npm run dev` from the main checkout** (not a worktree) uses a fresh `mktemp` directory under `$TMPDIR` and removes it on exit. Set `ROCKY_HOME` explicitly to keep state across runs.

Override knobs:

```bash
ROCKY_HOME=~/.rocky-blue npm run dev          # explicit home
ROCKY_DEV_SEED_HOME=/path/to/home npm run dev # seed from a different source home
ROCKY_DEV_RESET_HOME=1 npm run dev            # clear and reseed the derived worktree home
```
...
Override the port with `ROCKY_ELECTRON_REMOTE_DEBUGGING_PORT` when `9223` is busy.
...
Captured samples are exposed on `globalThis.__ROCKY_RENDER_PROFILE__`. Call
`globalThis.__ROCKY_RESET_RENDER_PROFILE__?.()` after warm-up and before the
...
reason counts are exposed on `globalThis.__ROCKY_RENDER_PROFILE_REASONS__`.
...
```bash
ROCKY_PROFILE_SERVER_ID=<server-id> \
ROCKY_PROFILE_WORKSPACE_ID=<workspace-path> \
ROCKY_PROFILE_AGENT_ID=<agent-id> \
  npm run profile:workspace-tabs --workspace=@getrocky/app
```
...
```bash
ROCKY_PROFILE_APP_URL=http://localhost:19010 # Expo web URL
ROCKY_PROFILE_SWITCH_COUNT=1                # number of agent/terminal switch pairs
ROCKY_PROFILE_SWITCH_WAIT_MS=250            # delay after each click
ROCKY_PROFILE_IDLE_WAIT_MS=3000             # idle baseline before switching
ROCKY_PROFILE_DUMP_COMMITS=1                # include per-commit profiler samples
```
...
Check `$ROCKY_HOME/daemon.log` for daemon logs. The default level is `info`; set
`ROCKY_LOG_LEVEL=trace` before launching the daemon when you need full provider,
...
`$ROCKY_HOME/config.json` win first. Without persisted config, the optional
`ROCKY_LOG_ROTATE_SIZE` and `ROCKY_LOG_ROTATE_COUNT` env vars override the
defaults. The default rotation is `10m` x `3` files everywhere.

## rocky.json service scripts
...
| Variable                    | Value                                                                                                                     |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `ROCKY_SERVICE_<NAME>_URL`  | Proxied URL for a declared peer service. Prefer this for peer discovery; it survives peer restarts.                       |
| `ROCKY_SERVICE_<NAME>_PORT` | Raw ephemeral port for a declared peer service. Use only as a bypass escape hatch; it can go stale if that peer restarts. |
| `ROCKY_URL`                 | Self alias for `ROCKY_SERVICE_<SELF>_URL`.                                                                                |
| `ROCKY_PORT`                | Self alias for `ROCKY_SERVICE_<SELF>_PORT`.                                                                               |
...
```json
{
  "scripts": {
    "web": {
      "type": "service",
      "command": "PORT=$ROCKY_PORT npm run dev:web"
    }
  }
}
```
...
`npm run dev`, `npm run dev:server`, and `npm run dev:app` build the workspace packages they need once, then keep `@getrocky/protocol` and `@getrocky/client` fresh with TypeScript watch builds while the daemon or Expo runs. If you change protocol schemas or client code outside those watch workflows, rebuild the producer before trusting runtime behavior.
...
Use `npm run cli` to run the in-repo CLI from source (`npx tsx packages/cli/src/index.ts`). The globally installed `rocky` binary on macOS is a symlink into the installed Rocky desktop app, not this checkout — use it to drive the desktop's built-in daemon, but use `npm run cli` when you want to talk to the CLI you are editing.
...
```
$ROCKY_HOME/agents/{cwd-with-dashes}/{agent-id}.json
```

Find an agent by ID:

```bash
find $ROCKY_HOME/agents -name "{agent-id}.json"
```

Find by content:

```bash
rg -l "some title text" $ROCKY_HOME/agents/
```
...
directory to Cloudflare Pages with `npm run deploy:web --workspace=@getrocky/app`.
...
Do not add service-worker caching casually. Rocky is a live control surface for
