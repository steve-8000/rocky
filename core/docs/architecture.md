...
Rocky is a client-server system for monitoring and controlling local AI coding agents. The daemon runs on your machine, manages agent processes, and streams their output in real time over WebSocket. Clients (mobile app, CLI, desktop app) connect to the daemon to observe and interact with agents.

Your code never leaves your machine. Rocky is local-first.
...
The heart of Rocky. A Node.js process that:
...
| `server/agent/agent-storage.ts` | File-backed JSON persistence at `$ROCKY_HOME/agents/`                        |
...
and clients. Server, app, CLI, and `@getrocky/client` all depend on this package;
it does not depend on the server.

### `packages/client` — Daemon client library and SDK facade

Owns the low-level daemon WebSocket driver plus the higher-level `RockyClient`
facade. App and CLI may import the low-level driver from
`@getrocky/client/internal/daemon-client` during migration, while new SDK-shaped
code imports from `@getrocky/client`.
...
Commander.js CLI with Docker-style commands. Common agent operations are also exposed at the top level (e.g. `rocky ls`, `rocky run`).

- `rocky agent ls/run/import/attach/logs/stop/delete/send/inspect/wait/archive/reload/update/mode`
- `rocky daemon start/stop/restart/status/pair/set-password`
- `rocky chat ls/create/inspect/post/read/wait/delete`
- `rocky terminal ls/create/capture/send-keys/kill`
- `rocky loop run/ls/inspect/logs/stop`
- `rocky schedule create/ls/inspect/update/pause/resume/run-once/logs/delete`
- `rocky permit allow/deny/ls`
- `rocky provider ls/models`
...
- Self-hosted relays opt into TLS with `daemon.relay.useTls` or `ROCKY_RELAY_USE_TLS=true`; the public (client-facing) TLS setting can be overridden independently via `daemon.relay.publicUseTls` or `ROCKY_RELAY_PUBLIC_USE_TLS`
...
TanStack Router + Cloudflare Workers. Serves rocky.sh.
...
- Agent state persists to `$ROCKY_HOME/agents/{cwd-with-dashes}/{agent-id}.json` (timeline rows live alongside the record)
...
- Handle their own authentication (Rocky does not manage API keys)
...
`$ROCKY_HOME` defaults to `~/.rocky`. The most important files:
...
├── rocky.sock / rocky.pid                      # Local IPC socket and pidfile
└── daemon.log                                  # Daemon trace logs (rotated)
```

## Deployment models

1. **Local daemon** (default): `rocky daemon start` on `127.0.0.1:6767`
