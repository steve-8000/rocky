...
Rocky uses **file-based JSON persistence** instead of a traditional database. All data is validated at runtime with Zod schemas. Most stores write atomically (write to temp file, then rename); a few still use plain `writeFile` — see each section. There is no schema-versioning/migration framework — schemas rely on optional fields with defaults for forward compatibility, with a small amount of inline normalization in `persisted-config.ts` for legacy provider/speech entries.

All server-side stores live under `$ROCKY_HOME` (defaults to `~/.rocky`).
...
├── rocky.pid                            # Daemon PID lock file
...
**Path:** `$ROCKY_HOME/agents/{project-dir}/{agentId}.json`
...
| `labels`             | `Record<string, string>`                 | Key-value labels (default `{}`). `rocky.parent-agent-id` set automatically when launched via the `create_agent` MCP tool — see [agent-lifecycle.md](./agent-lifecycle.md) |
...
```
{
  ...
  worktrees?: {
    root?: string            // optional root for new worktrees; defaults to $ROCKY_HOME/worktrees
  },
  providers: {
...
`agents.metadataGeneration.providers` controls the preferred structured-generation fallback order for daemon-side metadata tasks such as commit messages, PR text, branch names, and generated agent titles. Entries are tried first in the configured order, then Rocky falls through to dynamically discovered defaults and finally the current selection when available.
...
**Path:** `$ROCKY_HOME/schedules/{id}.json`
...
**Path:** `$ROCKY_HOME/chat/rooms.json`
...
**Path:** `$ROCKY_HOME/loops/loops.json`
...
**Path:** `$ROCKY_HOME/projects/projects.json`
...
**Path:** `$ROCKY_HOME/projects/workspaces.json`
...
**Path:** `$ROCKY_HOME/push-tokens.json`
...
These small files are not validated as full Zod schemas but are persisted under `$ROCKY_HOME` for daemon identity and runtime coordination.

| Path                  | Format                                                         | Notes                                                                             |
| --------------------- | -------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `server-id`           | Plain text, e.g. `srv_<base64url>`                             | Stable per-`$ROCKY_HOME` daemon ID. Overridable via `ROCKY_SERVER_ID` env.        |
...
| `rocky.pid`           | JSON `{ pid, startedAt, ... }`                                 | PID lock; prevents two daemons sharing one `$ROCKY_HOME`.                         |
...
**AsyncStorage key:** `rocky-drafts` (version 2)
...
**IndexedDB database:** `rocky-attachment-bytes`, object store: `attachments`
