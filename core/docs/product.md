...
What Rocky is, who it's for, and where it's going.

## What is Rocky

Rocky is a next-generation development environment built around agents. One interface to run, monitor, and interact with coding agents across desktop, mobile, terminal, and web.

The development workflow is shifting from manually editing files to orchestrating agents that do the editing. Rocky is built for that workflow.
...
- **BYOK** — Bring your own keys. Use your subsidized plans and first-party provider pricing. Rocky adds zero cost on top.
...
Rocky is a client-server system. The daemon (Node.js) runs on your machine, manages agent processes, and streams output in real time over WebSocket. Clients connect to the daemon — locally or remotely.
