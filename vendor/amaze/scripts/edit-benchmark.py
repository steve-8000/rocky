...
Select the edit variant via the AMAZE_EDIT_VARIANT env var (e.g. `vim`,
`hashline`, `replace`, `patch`, `apply_patch`) or `--variant`.

Examples:
	AMAZE_EDIT_VARIANT=vim   scripts/edit-benchmark.py
...
def _resolve_variant() -> str:
	...
	variant = cli_variant or os.environ.get("AMAZE_EDIT_VARIANT")
	if not variant:
		raise SystemExit("edit-benchmark: set AMAZE_EDIT_VARIANT=<variant> or pass --variant <variant>.")
	return variant
...
def build_spec(variant: str) -> BenchmarkSpec:
	...
	return BenchmarkSpec(
		...
		tools=("edit", "read"),
		env={"AMAZE_EDIT_VARIANT": variant},
