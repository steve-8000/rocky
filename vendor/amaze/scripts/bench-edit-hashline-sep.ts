 ...
 * of `AMAZE_HL_SEP` separator values and models, with a fixed concurrency cap.
 *
...
async function runJob(job: Job): Promise<JobResult> {
	...
	sink.write(`# cmd: AMAZE_HL_SEP=${JSON.stringify(job.sep)} bun run bench:edit \\\n`);
	sink.write(
	...
	const proc = Bun.spawn({
		...
		cwd: repoRoot,
		env: { ...process.env, AMAZE_HL_SEP: job.sep },
		stdout: "pipe",
		stderr: "pipe",
	});
