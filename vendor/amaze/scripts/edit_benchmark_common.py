...
from amaze_rpc import MessageEndEvent, MessageStartEvent, MessageUpdateEvent, RpcClient, ToolExecutionStartEvent  # noqa: E402
...
def resolve_repo_amaze_bin() -> str | None:
...
def resolve_amaze_bin(raw: str | None) -> str:
    if raw:
        return raw
    repo_bin = resolve_repo_amaze_bin()
    if repo_bin:
        return repo_bin
    found = shutil.which("amaze")
    if not found:
        raise SystemExit("Could not find `amaze` on PATH and could not resolve the repo CLI. Set --amaze-bin or AMAZE_BIN.")
    return found
...
async def run_all(spec: BenchmarkSpec, args: argparse.Namespace) -> dict[str, dict[str, Any]]:
    amaze_bin = resolve_amaze_bin(args.amaze_bin)
...
def parse_args(description: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--amaze-bin",
        default=os.environ.get("AMAZE_BIN"),
