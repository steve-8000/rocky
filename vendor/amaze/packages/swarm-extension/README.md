...
Multi-agent orchestration for amaze. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full amaze subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
```
.swarm_<name>/
  state/amazepeline.json    # Live pipeline + per-agent status
...
```bash
# Quick status
cat workspace/.swarm_mypipeline/state/amazepeline.json | python -m json.tool
...
Each agent is a full amaze session. It can:
...
src/swarm/
  ...
  executor.ts         Spawns agents via amaze's runSubprocess
