...
Stateful agent with tool execution and event streaming. Built on `@amaze/ai`.

## Installation

```bash
npm install @amaze/agent-core
```
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@amaze/ai`).
...
```typescript
import { Agent, streamProxy } from "@amaze/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@amaze/agent-core";
