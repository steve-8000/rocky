 ...
 *
 * Uses AMAZE_CONFIG_DIR (default ".amaze") for the config root and
 * AMAZE_CODING_AGENT_DIR to override the agent directory.
 *
...
/** Get the config directory name relative to home (e.g. ".amaze" or AMAZE_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.AMAZE_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".amaze/agent" or AMAZE_CONFIG_DIR + "/agent"). */
...
let dirs = new DirResolver(process.env.AMAZE_CODING_AGENT_DIR);
...
export function setAgentDir(dir: string): void {
	...
	process.env.AMAZE_CODING_AGENT_DIR = dir;
}
...
export function getPluginsLockfile(): string {
	return path.join(getPluginsDir(), `${APP_NAME}-plugins.lock.json`);
}
...
/** Get the PR worktrees directory (~/.amaze/wt). */
...
/** Get the worktree base directory (~/.amaze/wt). */
export function getWorktreeBaseDir(): string {
	return dirs.rootSubdir("wt", "data");
}

/** Get the path to a worktree directory (~/.amaze/wt/<project>/<id>). */
 ...
 * Honors the `AMAZE_GITHUB_CACHE_DB` env var when set so tests can isolate the
 * cache file without touching the rest of the config root.
 */
export function getGithubCacheDbPath(): string {
	const override = process.env.AMAZE_GITHUB_CACHE_DB;
