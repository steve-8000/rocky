...
- Added a per-release version sentinel napi export (`__amazeNativesV{major}_{minor}_{patch}`). The Rust `js_name` is bumped in lock-step with the package version by `scripts/release.ts`; the JS loader computes the expected name from `package.json#version` and throws an actionable error when the on-disk `.node` doesn't expose it. This converts the silent `<sym> is not a function` crash from a stale addon into a load-time failure pointing at the real fix.
- Added `applyBashFixups(command)` — a synchronous brush-parser-driven rewrite that strips trailing `| head|tail …`, redundant `2>&1`, and the `|&` shorthand from top-level pipelines, returning `{ command, stripped }`. Replaces the hand-rolled top-level mask scanner in `coding-agent`; tokenization, quoting, heredocs, command substitution, and nested compound commands are now handled by the real shell AST instead of regex/character-walking. Lives in `amaze_shell::fixup` on the Rust side.

### Fixed

- Fixed `<sym> is not a function` crashes on Windows after `bun install -g @amaze/coding-agent` updates while an `amaze` process was running. Bun cannot overwrite a locked `node_modules/@amaze/natives/native/amaze_natives.win32-x64.node` and silently keeps the old binary alongside the new ESM wrapper, so the next launch loads mismatched code. The loader now mirrors the addon into `~/.amaze/natives/<version>/` on Windows npm installs and prefers that copy at load time — each version gets its own filesystem path, so future updates land in `node_modules` unchallenged. The new version sentinel detects any remaining drift up front.

### Fixed

- Fixed `$env:NAME` PowerShell references being collapsed to `:NAME` when brush forwarded a command to a PowerShell (or any) subprocess. `amaze-shell` now defines `env=$env` as a non-exported global on every brush session so the bash parameter expansion of `$env` yields the literal `$env`, leaving `$env:NAME` intact. User-driven assignments (`env=prod`) push their own command-scope binding and shadow the fallback, preserving the bash POSIX contract. ([#1079](https://github.com/can1357/amaze/issues/1079))
...
- Removed the host Zig CPU contract wrapper (`zig-safe-wrapper.ts`) and its `ZIG`/`AMAZE_NATIVE_REAL_ZIG`/`AMAZE_NATIVE_ZIG_TARGET`/`AMAZE_NATIVE_ZIG_CPU` env handling, since the `zlob` Rust dependency that required Zig is gone
...
- Removed `AMAZE_DEV` loader diagnostic env var and associated console logging in the native addon loader
...
- Added Dart support to `astGrep` and `astEdit` through the native tree-sitter Dart grammar ([#748](https://github.com/can1357/amaze/pull/748) by [@0fflineuser](https://github.com/0fflineuser))
...
- Removed `getIndentation`, `getDefaultTabWidth`, and `setDefaultTabWidth` (moved to `@amaze/utils`)
...
- Removed inline utils helpers and dependency on `@amaze/utils` from native module loader
...
- Added `AMAZE_NATIVE_VARIANT` environment variable to override CPU variant selection at runtime
...
- Changed native addon filename scheme to include CPU variant suffix for x64 builds (e.g., `amaze_natives.linux-x64-modern.node`)
...
- Removed fallback untagged `amaze_natives.node` binary creation for native builds; platform-tagged variants are now required
...
- Added `AMAZE_DEV` environment variable support for loading and debugging development native builds
...
- Changed debug builds to output to `amaze_natives.dev.node` instead of mixing with release artifacts
