...
/**
 * Native addon loader for `@amaze/natives`.
 *
 * Owns every step between "Node imports `native/index.js`" and "the right
 * `amaze_natives.<platform>-<arch>*.node` is required, validated, and returned":
 ...
 *
 * Background (issue #823): `bun build --compile --define AMAZE_COMPILED=true`
 * substitutes the bare identifier `AMAZE_COMPILED`, NOT `process.env.AMAZE_COMPILED`,
...
export function detectCompiledBinary({ embeddedAddon, env, importMetaUrl }) {
	...
	if (env && env.AMAZE_COMPILED) return true;
...
export function getAddonFilenames({ tag, arch, variant }) {
	const defaultFilename = `amaze_natives.${tag}.node`;
	if (arch !== "x64" || !variant) return [defaultFilename];
	const baselineFilename = `amaze_natives.${tag}-baseline.node`;
	const modernFilename = `amaze_natives.${tag}-modern.node`;
	if (variant === "modern") {
 ...
 * `node_modules/@amaze/natives/native/`, leaving an old binary next to a
...
function getVariantOverride() {
	const value = process.env.AMAZE_NATIVE_VARIANT;
...
function validateLoadedBindings(ctx, bindings, candidate) {
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @amaze/natives@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	if (ctx.isCompiledBinary) {
		...
		const downloadHints = ctx.addonFilenames
			.map(filename => {
				const downloadUrl = `https://github.com/can1357/amaze/releases/latest/download/${filename}`;
				const targetPath = path.join(ctx.versionedDir, filename);
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @amaze/natives\n" +
...
function initLoaderContext() {
	...
	// the package version (`__amazeNativesV{major}_{minor}_{patch}`).
	// `scripts/release.ts` bumps the name in `crates/natives/src/lib.rs` in
	...
	const versionSentinelExport = `__amazeNativesV${packageVersion.replace(/[^A-Za-z0-9]/g, "_")}`;
...
export function loadNative() {
	...
	throw new Error(
		`Failed to load amaze_natives native addon for ${ctx.addonLabel}.\n\nTried:\n${details}\n\n${buildHelpMessage(ctx)}`,
	);
}
