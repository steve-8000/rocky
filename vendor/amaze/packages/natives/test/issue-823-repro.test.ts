/**
 * Regression for https://github.com/can1357/amaze/issues/823.
 *
 ...
 * `amaze_natives.linux-x64-*.node`. Root cause: the old loader's
 * `isCompiledBinary` detection relied on signals that are unreliable in a Bun
 * standalone binary:
 *   - `process.env.AMAZE_COMPILED` — never set, because `bun build --compile
 *     --define AMAZE_COMPILED=true` substitutes the bare identifier, not
...
describe("issue 823: standalone-binary native loader path resolution", () => {
	it("detects compiled-binary mode from embedded-addon presence when env and url markers are absent", () => {
		...
		// - `process.env.AMAZE_COMPILED` is undefined (the build flag does not substitute property accesses).
			...
			detectCompiledBinary({
				embeddedAddon: {
					...
					files: [
						{
							variant: "modern",
							filename: "amaze_natives.linux-x64-modern.node",
							filePath: "/$bunfs/root/packages/natives/native/amaze_natives.linux-x64-modern.node",
						},
					],
				},
				env: {},
		...
		// Env override (e.g. user-set AMAZE_COMPILED=1) still wins.
		expect(
			detectCompiledBinary({
				embeddedAddon: null,
				env: { AMAZE_COMPILED: "1" },
	...
	it("places embedded-extracted candidates ahead of build-host candidates for linux-x64 standalone", () => {
		...
		const versionedModern = path.join(versionedDir, "amaze_natives.linux-x64-modern.node");
		const versionedBaseline = path.join(versionedDir, "amaze_natives.linux-x64-baseline.node");
		const userDataModern = path.join(userDataDir, "amaze_natives.linux-x64-modern.node");
		const buildHostModern = path.join(nativeDir, "amaze_natives.linux-x64-modern.node");
	...
	it("does not probe user-data candidates when running outside a standalone binary", () => {
		...
		expect(candidates).not.toContain(path.join(versionedDir, "amaze_natives.linux-x64-baseline.node"));
		expect(candidates).not.toContain(path.join(userDataDir, "amaze_natives.linux-x64-baseline.node"));
	});
});
