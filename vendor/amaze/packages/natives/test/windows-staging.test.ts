 ...
 * `node_modules/@amaze/natives/native/amaze_natives.win32-x64.node` during
...
const winWorkspaceNativeDir = "C:\\Users\\Admin\\dev\\amaze\\packages\\natives\\native";
const posixNodeModulesNativeDir = "/home/u/proj/node_modules/@amaze/natives/native";
...
describe("windows native addon staging", () => {
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		...
		const versionedBaseline = path.join(versionedDir, "amaze_natives.win32-x64-baseline.node");
		const versionedDefault = path.join(versionedDir, "amaze_natives.win32-x64.node");
		const nodeModulesBaseline = path.join(winNodeModulesNativeDir, "amaze_natives.win32-x64-baseline.node");
		...
		// addon anyway).
		const userDataBaseline = path.join(userDataDir, "amaze_natives.win32-x64-baseline.node");
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedBaseline = path.join(versionedDir, "amaze_natives.linux-x64-baseline.node");
		const nodeModulesBaseline = path.join(posixNodeModulesNativeDir, "amaze_natives.linux-x64-baseline.node");
		expect(candidates).not.toContain(versionedBaseline);
		expect(candidates).toContain(nodeModulesBaseline);
	});
});

describe("natives version sentinel", () => {
	it("Rust `js_name` matches the package version", async () => {
		...
		const libRs = await Bun.file(path.join(import.meta.dir, "../../../crates/natives/src/lib.rs")).text();
		const sentinelMatch = libRs.match(/js_name = "(__amazeNativesV[A-Za-z0-9_]+)"/);
		expect(sentinelMatch, 'Rust sentinel `js_name = "__amazeNativesV…"` not found in lib.rs').not.toBeNull();
		const expected = `__amazeNativesV${packageJson.version.replace(/[^A-Za-z0-9]/g, "_")}`;
