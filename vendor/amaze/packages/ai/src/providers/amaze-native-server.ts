 ...
 * (containerized amaze, the swarm extension, rocky's sidecar auth-gateway).
...
export interface AmazeNativeParsedRequest {
	modelId: string;
	context: Context;
...
/**
 * Parse a amaze-native request body. Validation is intentionally minimal — only
 ...
 */
export function parseRequest(body: unknown, _headers?: Headers): AmazeNativeParsedRequest {
	...
	// `stream` defaults to true — amaze-native clients overwhelmingly stream, and
 ...
 *
 * No per-event re-shaping: the amaze-native client is ai itself, so the
 ...
 * targets (containerized amaze → host gateway, rocky slot → amaze-auth-gateway
 ...
 * provider-specific status taxonomy — amaze-native callers consume `type`
 * directly.
 */
