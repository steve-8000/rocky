/**
 * Client half of the amaze-native auth-gateway protocol.
 *
 * Dispatches a {@link streamSimple}-shaped request to an `amaze auth-gateway`
 * via `POST /v1/amaze/stream`, reads the SSE event stream back, and pushes the
 ...
 * way.
 *
 * Activated when a {@link Model} has `transport: "amaze-native"` set; the
 * dispatch hook lives in `streamSimple()` (see `../stream.ts`). Used by
 * containerized amaze deployments (rocky slots, the swarm extension) that
...
/**
 * Resolve the `/v1/amaze/stream` endpoint URL from the model's `baseUrl`.
 * Trims a trailing slash so concatenation can't double-slash; throws when
 * the baseUrl is missing (transport=amaze-native without a gateway target is
 * a configuration error, not a runtime recoverable one).
 */
function resolveStreamUrl(model: Model<Api>): string {
	if (!model.baseUrl) {
		throw new Error(
			`amaze-native transport requires \`baseUrl\` on model ${model.id} (set it on the provider config in models.yml)`,
		);
	}
	return `${model.baseUrl.replace(/\/+$/, "")}/v1/amaze/stream`;
}
...
/**
 * Stream a turn through an `amaze auth-gateway` over the amaze-native protocol.
 *
 ...
 * non-streaming path.
 */
export function streamAmazeNative<TApi extends Api>(
	model: Model<TApi>,
	context: Context,
	...
	void (async () => {
		...
		try {
			...
			if (!sawTerminal) {
				...
				// `amaze-native-server.encodeStream`.
