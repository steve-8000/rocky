 ...
 * pure protocol translation: foreign wire → amaze Context → ai stream() →
...
import * as piNative from "../providers/amaze-native-server";
...
function classifyGatewayError(err: unknown): { status: number; type: string; message: string } {
	...
	// Custom ai errors may attach a numeric `status` property; honor it
...
async function handleFormatEndpoint(
	...
	// Parse + validate against the strict format schema, rebuild as amaze's
	// canonical Context, dispatch through ai's streamSimple, encode the
	// canonical event stream back to the inbound format. There is no
	// passthrough fast-path — every request flows through ai so that
...
/**
 * Pi-native fast path: `POST /v1/amaze/stream`. Accepts the canonical ai
 ...
 * SDKs (llm-git, anthropic-sdk, openai-sdk), and bridging back to amaze-native
 ...
 * path.
 */
async function handleAmazeNative(bootOpts: AuthGatewayBootOptions, req: Request, peer: string): Promise<Response> {
	const controller = mirrorRequestAbort(req);
	...
	let parsed: piNative.AmazeNativeParsedRequest;
	try {
	...
	try {
		...
		const classified = classifyGatewayError(error);
		logger.warn("auth-gateway streamSimple threw", { format: "amaze-native", error: classified.message, peer });
		return piNative.formatError(classified.status, classified.type, classified.message);
	}
	...
	if (!parsed.stream) {
		try {
			...
			if (controller.signal.aborted) return aborted();
			const classified = classifyGatewayError(error);
			logger.warn("auth-gateway non-streaming aborted", { format: "amaze-native", error: classified.message, peer });
			return piNative.formatError(classified.status, classified.type, classified.message);
		}
	}
	if (controller.signal.aborted) return aborted();
	...
	const server = Bun.serve({
		...
		fetch: async (req): Promise<Response> => {
			...
			try {
				...
				if (req.method === "POST" && pathname === "/v1/amaze/stream") {
					return withCors(await handleAmazeNative(opts, req, peer), req);
				}
