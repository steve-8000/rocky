...
async function login(provider: OAuthProvider): Promise<void> {
	...
	try {
		...
		console.log(`\nCredentials saved to ~/.amaze/agent/agent.db`);
	} finally {
		store.close();
		rl.close();
	}
...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		console.log(`Usage: bunx @amaze/ai <command> [provider]
...
Examples:
  ...
  bunx @amaze/ai logout anthropic   # logout from specific provider
  bunx @amaze/ai status             # show logged-in providers
  bunx @amaze/ai list               # list providers
`);
		return;
	}
	...
	if (command === "status") {
		...
		try {
			...
			if (providers.length === 0) {
				...
				console.log(`Use 'bunx @amaze/ai login' to authenticate.`);
			} else {
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some(p => p.id === provider)) {
			...
			console.error(`Use 'bunx @amaze/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'bunx @amaze/ai --help' for usage`);
	process.exit(1);
}
