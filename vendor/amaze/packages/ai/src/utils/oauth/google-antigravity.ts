...
function getRequiredEnv(name: "AMAZE_GOOGLE_ANTIGRAVITY_CLIENT_ID" | "AMAZE_GOOGLE_ANTIGRAVITY_CLIENT_SECRET"): string {
	const value = $env[name];
	if (!value) {
		throw new Error(`${name} is required for Google Antigravity OAuth login`);
	}
	return value;
}
...
export async function loginAntigravity(ctrl: OAuthController): Promise<OAuthCredentials> {
	return runGoogleOAuthLogin(ctrl, {
		clientId: getRequiredEnv("AMAZE_GOOGLE_ANTIGRAVITY_CLIENT_ID"),
		clientSecret: getRequiredEnv("AMAZE_GOOGLE_ANTIGRAVITY_CLIENT_SECRET"),
		authUrl: AUTH_URL,
		tokenUrl: TOKEN_URL,
		scopes: SCOPES,
...
export async function refreshAntigravityToken(refreshToken: string, projectId: string): Promise<OAuthCredentials> {
	const response = await fetch(TOKEN_URL, {
		...
		body: new URLSearchParams({
			client_id: getRequiredEnv("AMAZE_GOOGLE_ANTIGRAVITY_CLIENT_ID"),
			client_secret: getRequiredEnv("AMAZE_GOOGLE_ANTIGRAVITY_CLIENT_SECRET"),
