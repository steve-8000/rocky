...
- Applies to provider-facing tool schemas produced by:
  ...
  - `adapt.ts` — thin composer wrapping `tryEnforceStrictSchema` for provider call sites, plus the `AMAZE_NO_STRICT` env flag callers consult to opt out of strict mode.
