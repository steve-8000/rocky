 ...
 *
 * `AMAZE_OPENAI_STREAM_IDLE_TIMEOUT_MS` is accepted as a backward-compatible alias.
 * Set `AMAZE_STREAM_IDLE_TIMEOUT_MS=0` to disable the watchdog.
 */
export function getStreamIdleTimeoutMs(): number | undefined {
	return normalizeIdleTimeoutMs(
		$env.AMAZE_STREAM_IDLE_TIMEOUT_MS ?? $env.AMAZE_OPENAI_STREAM_IDLE_TIMEOUT_MS,
 ...
 *
 * Set `AMAZE_OPENAI_STREAM_IDLE_TIMEOUT_MS=0` to disable the watchdog.
 */
export function getOpenAIStreamIdleTimeoutMs(): number | undefined {
	return normalizeIdleTimeoutMs(
		$env.AMAZE_OPENAI_STREAM_IDLE_TIMEOUT_MS ?? $env.AMAZE_STREAM_IDLE_TIMEOUT_MS,
		DEFAULT_STREAM_IDLE_TIMEOUT_MS,
	);
}
 ...
 *
 * Set `AMAZE_STREAM_FIRST_EVENT_TIMEOUT_MS=0` to disable the watchdog.
 */
export function getStreamFirstEventTimeoutMs(idleTimeoutMs?: number): number | undefined {
	...
	return normalizeIdleTimeoutMs($env.AMAZE_STREAM_FIRST_EVENT_TIMEOUT_MS, fallback);
}
