 ...
 * (override via `AMAZE_E2E_GATEWAY_URL`) AND the bearer token file exists at
...
const GATEWAY_URL = Bun.env.AMAZE_E2E_GATEWAY_URL ?? "http://127.0.0.1:4000";
const TOKEN_PATH = path.join(os.homedir(), ".amaze", "auth-gateway.token");
...
const MODEL = Bun.env.AMAZE_E2E_OPENAI_RESPONSES_MODEL ?? "gpt-5.3-codex";
