...
import type { Api, Context, Model, Tool, ToolResultMessage } from "@amaze/ai";
import { complete, getBundledModel } from "@amaze/ai";
import type { OptionsForApi } from "@amaze/ai/types";
...
async function handleToolWithTextAndImageResult<TApi extends Api>(model: Model<TApi>, options?: OptionsForApi<TApi>) {
	...
	if (textContent && textContent.type === "text") {
		const lowerContent = textContent.text.toLowerCase();
		// Should mention details from the text (diameter/amazexels)
