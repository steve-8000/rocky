...
import { enrichModelThinking } from "@amaze/ai/model-thinking";
import {
...
import { getAgentDir, setAgentDir, TempDir } from "@amaze/utils";

const originalFetch = global.fetch;
const originalAgentDir = getAgentDir();
const originalWebSocket = global.WebSocket;
const originalCodexWebSocketRetryBudget = Bun.env.AMAZE_CODEX_WEBSOCKET_RETRY_BUDGET;
const originalCodexWebSocketRetryDelayMs = Bun.env.AMAZE_CODEX_WEBSOCKET_RETRY_DELAY_MS;
const originalCodexWebSocketIdleTimeoutMs = Bun.env.AMAZE_CODEX_WEBSOCKET_IDLE_TIMEOUT_MS;
const originalCodexWebSocketFirstEventTimeoutMs = Bun.env.AMAZE_CODEX_WEBSOCKET_FIRST_EVENT_TIMEOUT_MS;
const originalCodexWebSocketV2 = Bun.env.AMAZE_CODEX_WEBSOCKET_V2;
...
afterEach(() => {
	...
	restoreEnv("AMAZE_CODEX_WEBSOCKET_RETRY_BUDGET", originalCodexWebSocketRetryBudget);
	restoreEnv("AMAZE_CODEX_WEBSOCKET_RETRY_DELAY_MS", originalCodexWebSocketRetryDelayMs);
	restoreEnv("AMAZE_CODEX_WEBSOCKET_IDLE_TIMEOUT_MS", originalCodexWebSocketIdleTimeoutMs);
	restoreEnv("AMAZE_CODEX_WEBSOCKET_FIRST_EVENT_TIMEOUT_MS", originalCodexWebSocketFirstEventTimeoutMs);
	restoreEnv("AMAZE_CODEX_WEBSOCKET_V2", originalCodexWebSocketV2);
	vi.restoreAllMocks();
});
...
describe("openai-codex streaming", () => {
	...
	it("falls back to SSE when websocket connect fails", async () => {
		...
		const token = `aaa.${payload}.bbb`;
		Bun.env.AMAZE_CODEX_WEBSOCKET_RETRY_BUDGET = "0";
		Bun.env.AMAZE_CODEX_WEBSOCKET_RETRY_DELAY_MS = "1";
		const sse = `${[
			`data: ${JSON.stringify({ type: "response.content_part.added", part: { type: "output_text", text: "" } })}`,
			`data: ${JSON.stringify({ type: "response.output_text.delta", delta: "Hello" })}`,
			`data: ${JSON.stringify({ type: "response.output_item.done", item: { type: "message", id: "msg_1", role: "assistant", status: "completed", content: [{ type: "output_text", text: "Hello" }] } })}`,
	...
	it("uses websocket v2 beta header when v2 mode is enabled", async () => {
		const tempDir = TempDir.createSync("@pi-codex-stream-");
		setAgentDir(tempDir.path());
		Bun.env.AMAZE_CODEX_WEBSOCKET_V2 = "1";
	...
	it("falls back to SSE when a prewarmed websocket never produces a first event", async () => {
		const tempDir = TempDir.createSync("@pi-codex-stream-");
		setAgentDir(tempDir.path());
		Bun.env.AMAZE_CODEX_WEBSOCKET_FIRST_EVENT_TIMEOUT_MS = "10";
		Bun.env.AMAZE_CODEX_WEBSOCKET_RETRY_BUDGET = "0";
	...
	it("retries websocket stream closes before surfacing transport errors", async () => {
		const tempDir = TempDir.createSync("@pi-codex-stream-");
		setAgentDir(tempDir.path());
		Bun.env.AMAZE_CODEX_WEBSOCKET_RETRY_BUDGET = "1";
		Bun.env.AMAZE_CODEX_WEBSOCKET_RETRY_DELAY_MS = "1";
