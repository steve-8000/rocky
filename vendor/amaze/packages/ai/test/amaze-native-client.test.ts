...
import { streamAmazeNative } from "../src/providers/amaze-native-client";
...
describe("streamAmazeNative request shape", () => {
	it("POSTs `{modelId, context, options, stream:true}` to `<baseUrl>/v1/amaze/stream`", async () => {
		...
		const stream = streamAmazeNative(fakeModel(), baseContext, {
			apiKey: "gw-bearer",
			fetch: fetchImpl,
			temperature: 0.7,
		});
		...
		expect(captured.url).toBe("http://llm-gateway.internal:4000/v1/amaze/stream");
	...
	it("normalizes trailing slashes on `baseUrl` so the endpoint never double-slashes", async () => {
		...
		await streamAmazeNative(fakeModel({ baseUrl: "http://llm-gateway.internal:4000///" }), baseContext, {
			apiKey: "k",
			fetch: fetchImpl,
		}).result();
		expect(captured.url).toBe("http://llm-gateway.internal:4000/v1/amaze/stream");
	...
	it("forwards `model.headers` and lets a caller-supplied Authorization win", async () => {
		...
		await streamAmazeNative(
			fakeModel({ headers: { "x-amaze-slot": "rocky-1", Authorization: "Bearer model-wins" } }),
			baseContext,
			{ apiKey: "options-loses", fetch: fetchImpl },
		).result();

		const headers = captured.init?.headers as Record<string, string>;
		expect(headers["x-amaze-slot"]).toBe("rocky-1");
	...
	it("throws synchronously when `baseUrl` is missing", async () => {
		...
		const stream = streamAmazeNative(broken, baseContext, { apiKey: "k" });
		await expect(stream.result()).rejects.toThrow(/baseUrl/);
	});
});

describe("streamAmazeNative event flow", () => {
	it("pushes parsed events verbatim and resolves `.result()` on terminal `done`", async () => {
		...
		const stream = streamAmazeNative(fakeModel(), baseContext, { apiKey: "k", fetch: fetchImpl });
	...
	it("forwards the caller's AbortSignal to the underlying fetch", async () => {
		...
		const controller = new AbortController();
		await streamAmazeNative(fakeModel(), baseContext, {
			apiKey: "k",
			fetch: fetchImpl,
			signal: controller.signal,
		}).result();
