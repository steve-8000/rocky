...
// of internal pi-* packages. `@amaze` is intentionally included so that
...
const AMAZE_SCOPE_ALIASES = ["amaze", "mariozechner", "earendil-works"] as const;

// Internal pi-* package basenames bundled inside the amaze binary.
const AMAZE_PACKAGE_NAMES = ["agent-core", "ai", "coding-agent", "natives", "tui", "utils"] as const;

const AMAZE_SCOPE_ALTERNATION = AMAZE_SCOPE_ALIASES.join("|");
const AMAZE_PACKAGE_ALTERNATION = AMAZE_PACKAGE_NAMES.join("|");
...
const AMAZE_SUBPATH_REMAPS: ReadonlyMap<string, string> = new Map<string, string>([
	// `@mariozechner/ai/oauth` re-exported `./utils/oauth/index.js`.
	// Our ai keeps the implementation under `utils/oauth` but never added a
	// root-level re-export, so map the upstream subpath onto it directly.
	["ai/oauth", "ai/utils/oauth"],
]);

const LEGACY_PI_SPECIFIER_FILTER = new RegExp(
	`^@(?:${AMAZE_SCOPE_ALTERNATION})/(?:${AMAZE_PACKAGE_ALTERNATION})(?:/.*)?$`,
);
const LEGACY_PI_IMPORT_SPECIFIER_REGEX = new RegExp(
	`((?:from\\s+|import\\s*\\(\\s*)["'])(@(?:${AMAZE_SCOPE_ALTERNATION})/(?:${AMAZE_PACKAGE_ALTERNATION})(?:/[^"'()\\s]+)?)(["'])`,
	"g",
);
const LEGACY_PI_FILE_PREFIX = "amaze-legacy-pi-file:";
const LEGACY_PI_FILE_NAMESPACE = "amaze-legacy-pi-file";
...
function remapLegacyPiSpecifier(specifier: string): string | null {
	...
	const remappedSubpath = AMAZE_SUBPATH_REMAPS.get(rest) ?? rest;
...
export async function loadLegacyPiModule(resolvedPath: string): Promise<unknown> {
	const root = path.join(os.tmpdir(), "amaze-legacy-pi-file", Bun.hash(resolvedPath).toString(36));
...
export function installLegacyPiSpecifierShim(): void {
	...
	Bun.plugin({
		...
		setup(build) {
			...
			build.onResolve({ filter: /^amaze-legacy-pi-file:/, namespace: "file" }, args => ({
