...
import { type Component, padding, truncateToWidth, visibleWidth } from "@amaze/tui";
import { formatNumber, getProjectDir } from "@amaze/utils";
...
export class FooterComponent implements Component {
	...
	render(width: number): string[] {
		...
		const usingOAuth = state.model ? this.session.modelRegistry.isUsingOAuth(state.model) : false;
		const normalizedPremiumRequests = Math.round((totalPremiumRequests + Number.EPSILON) * 100) / 100;
		if (totalCost || usingOAuth || normalizedPremiumRequests) {
