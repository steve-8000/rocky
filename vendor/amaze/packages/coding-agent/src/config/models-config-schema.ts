...
const ProviderConfigSchema = z.object({
	...
	/**
	 * Streaming transport override. When set to `"amaze-native"`, Amaze dispatches
	 * every model under this provider via the auth-gateway's
	 * `POST /v1/amaze/stream` endpoint instead of the per-provider SDK. The
	 * provider's `baseUrl` must point at a compatible `amaze auth-gateway`
	 * and `apiKey` must carry the gateway bearer.
	 */
	transport: z.literal("amaze-native").optional(),
});
