...
import { $which, isRecord, logger } from "@amaze/utils";
import { YAML } from "bun";
import { getConfigDirPaths, getPackageDir } from "../config";
...
function resolvePackageCommand(command: string): string | null {
	const packageDir = getPackageDir();
	const packagePath = path.join(packageDir, "node_modules", ".bin", command);
	const resolvedPackagePath = resolveLocalCommand(packagePath);
	if (resolvedPackagePath) return resolvedPackagePath;

	const workspacePath = path.resolve(packageDir, "..", "..", "node_modules", ".bin", command);
	return resolveLocalCommand(workspacePath);
}

/**
 * Resolve a command to an executable path.
 * Checks project-local bin directories first, then bundled package bins, then falls back to $PATH.
...
export function resolveCommand(command: string, cwd: string): string | null {
	...
	const resolvedPackagePath = resolvePackageCommand(command);
...
function getConfigPaths(cwd: string): string[] {
	...
	// Project config directories (.amaze/, .pi/, .claude/)
	...
	// User config directories (~/.amaze/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .amaze/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.amaze/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
