import { getProjectDir, logger } from "@amaze/utils";
...
export interface PythonExecutorOptions {
	 ...
	 * dir. When present, exported to the kernel as `AMAZE_ARTIFACTS_DIR` and
	 * preferred over `AMAZE_SESSION_FILE`-derived paths.
	 */
	artifactsDir?: string;
...
function buildKernelEnv(options: {
	...
	if (options.sessionFile) env.AMAZE_SESSION_FILE = options.sessionFile;
	if (options.artifactsDir) env.AMAZE_ARTIFACTS_DIR = options.artifactsDir;
	if (options.bridge && options.bridgeSessionId) {
		env.AMAZE_TOOL_BRIDGE_URL = options.bridge.url;
		env.AMAZE_TOOL_BRIDGE_TOKEN = options.bridge.token;
		env.AMAZE_TOOL_BRIDGE_SESSION = options.bridgeSessionId;
	}
