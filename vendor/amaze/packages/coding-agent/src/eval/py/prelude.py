...
if "__omp_prelude_loaded__" not in globals():
    ...
    def output(
        *ids: str,
        ...
        """
        # Prefer AMAZE_ARTIFACTS_DIR so subagents resolve through the parent's
        # shared artifacts dir; fall back to deriving from AMAZE_SESSION_FILE
        # for legacy callers / top-level sessions where the two coincide.
        artifacts_dir = os.environ.get("AMAZE_ARTIFACTS_DIR")
        if not artifacts_dir:
            session_file = os.environ.get("AMAZE_SESSION_FILE")
            if not session_file:
    ...
    if all(
        _k in os.environ
        for _k in ("AMAZE_TOOL_BRIDGE_URL", "AMAZE_TOOL_BRIDGE_TOKEN", "AMAZE_TOOL_BRIDGE_SESSION")
    ):
        tool = _ToolProxy(
            os.environ["AMAZE_TOOL_BRIDGE_URL"],
            os.environ["AMAZE_TOOL_BRIDGE_TOKEN"],
            os.environ["AMAZE_TOOL_BRIDGE_SESSION"],
        )
