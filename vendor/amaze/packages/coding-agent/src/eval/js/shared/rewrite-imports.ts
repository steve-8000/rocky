...
// `__amaze_import__` helper, which resolves the specifier against the active session cwd. A real
...
function buildAmazeImportCall(sourceLiteral: string, optionsLiteral: string | undefined): string {
	// Route every static import through the worker-injected `__amaze_import__` helper so the
	// specifier resolves against the session cwd (and `with`-attribute imports keep working).
	return optionsLiteral
		? `__amaze_import__(${sourceLiteral}, ${optionsLiteral})`
		: `__amaze_import__(${sourceLiteral})`;
}
...
function buildOptionsLiteral(node: BabelImportDeclaration): string | undefined {
	...
	// Native dynamic import takes options as `{ with: { ... } }`. `__amaze_import__` forwards the
...
export function rewriteImports(code: string): string {
	...
	// Top-level static `import` declarations become `await __amaze_import__(...)` calls.
	...
	// Dynamic `import(...)` expressions (anywhere) get their callee swapped for `__amaze_import__`
	...
	walkNodes(ast, node => {
		...
		if (!callee || callee.type !== "Import" || typeof callee.start !== "number" || typeof callee.end !== "number")
			return;
		edits.push({ start: callee.start, end: callee.end, text: "__amaze_import__" });
	});
