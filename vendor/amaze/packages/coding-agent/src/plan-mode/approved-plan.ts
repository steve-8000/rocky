...
const REQUIRED_APPROVAL_SECTIONS = ["Objective", "Scope", "Execution Plan", "Verification"] as const;
const PLACEHOLDER_ONLY_LINE = /^[ \t]*(?:TODO|TBD|\.\.\.|placeholder|\[TODO\])[ \t]*$/im;

function hasMarkdownSection(content: string, level: "#" | "##", title: string): boolean {
	const escaped = title.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
	return new RegExp(`^[ \\t]*${level}[ \\t]+${escaped}[ \\t]*$`, "im").test(content);
}

export function validatePlanContentForApproval(content: string): void {
	if (!content.trim()) {
		throw new ToolError("Plan file is empty. Write the finalized plan before requesting approval.");
	}
	if (!/^[ \t]*#[ \t]+\S.*$/m.test(content)) {
		throw new ToolError("Plan must include a top-level # title before approval.");
	}
	for (const section of REQUIRED_APPROVAL_SECTIONS) {
		if (!hasMarkdownSection(content, "##", section)) {
			throw new ToolError(`Plan must include a ## ${section} section before approval.`);
		}
	}
	if (PLACEHOLDER_ONLY_LINE.test(content)) {
		throw new ToolError("Plan still contains unresolved placeholder content.");
	}
}
