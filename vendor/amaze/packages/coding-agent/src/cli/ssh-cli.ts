...
import { APP_NAME, CONFIG_DIR_NAME, getSSHConfigPath } from "@amaze/utils";
...
async function handleList(cmd: SSHCommandArgs): Promise<void> {
	...
	if (hasProject) {
		process.stdout.write(chalk.bold(`Project SSH Hosts (${CONFIG_DIR_NAME}/ssh.json):\n`));
	...
	if (hasUser) {
		process.stdout.write(chalk.bold(`User SSH Hosts (~/${CONFIG_DIR_NAME}/agent/ssh.json):\n`));
