 ...
 * usage widget, rocky containers, …) make provider API calls without ever
 ...
 * `AMAZE_AUTH_BROKER_URL` / `auth.broker.url` precedence used elsewhere).
 *
 * Sub-verbs:
...
import { APP_NAME, getConfigRootDir, isEnoent, VERSION } from "@amaze/utils";
...
async function runServe(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			`"${APP_NAME} auth-gateway serve" requires AMAZE_AUTH_BROKER_URL (or \`auth.broker.url\`/\`auth.broker.token\` in config.yml). The gateway is itself a broker client.`,
		);
	}
...
async function runStatus(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		...
		if (flags.json) {
			...
			process.stdout.write(`${chalk.yellow("No broker configured.")} Set AMAZE_AUTH_BROKER_URL.\n`);
			process.stdout.write(
