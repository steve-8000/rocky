...
export async function listGrievances(options: ListGrievancesOptions): Promise<void> {
	...
	if (!db) {
		if (options.json) {
			...
			console.log(
				chalk.dim("No grievances database found. Enable auto-QA with AMAZE_AUTO_QA=1 or the dev.autoqa setting."),
			);
		}
		return;
	}
 ...
 *
 * Requires endpoint configuration (default `qa.amaze.sh/v1/grievances`).
 */
export async function pushGrievances(options: PushGrievancesOptions): Promise<void> {
	...
	try {
		...
		if (result.skipped) {
			console.log(
				chalk.yellow(
					"Push skipped — no endpoint configured. Set `dev.autoqaPush.endpoint` or `AMAZE_AUTO_QA_PUSH_URL`.",
				),
			);
			return;
		}
