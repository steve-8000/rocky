...
import { formatNumber, formatPercent } from "@amaze/utils";
...
function appendAgentStats(
	...
	if (opts.contextTokens && opts.contextTokens > 0) {
		if (opts.contextWindow && opts.contextWindow > 0) {
			const ratio = Math.min(opts.contextTokens / opts.contextWindow, 1);
			line += `${theme.sep.dot}${theme.fg("dim", `${formatPercent(ratio)}/${formatNumber(opts.contextWindow)}`)}`;
		} else {
			line += `${theme.sep.dot}${theme.fg("dim", `${formatNumber(opts.contextTokens)} ctx`)}`;
		}
		if (opts.tokens > 0) {
...
function extractYieldMemoryNote(value: unknown): string | undefined {
	if (!Array.isArray(value)) return undefined;
	for (let i = value.length - 1; i >= 0; i--) {
		const item = value[i];
		if (!item || typeof item !== "object") continue;
		const note = (item as { memoryNote?: unknown }).memoryNote;
...
function renderAgentProgress(
	...
	if (progress.extractedToolData) {
		...
		const memoryNote = extractYieldMemoryNote(progress.extractedToolData.yield);
		if (memoryNote) {
			lines.push(
				`${continuePrefix}${theme.fg("dim", `Memory note: ${truncateToWidth(replaceTabs(memoryNote), 120)}`)}`,
			);
		}
...
function renderAgentResult(result: SingleResult, isLast: boolean, expanded: boolean, theme: Theme): string[] {
	...
	const memoryNote = extractYieldMemoryNote(result.extractedToolData?.yield);
