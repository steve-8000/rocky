...
import { $which, APP_NAME, getToolsDir, logger, ptree, TempDir } from "@amaze/utils";
...
// CLI packages installed via uv/amazep
...
export function getToolPath(tool: ToolName): string | null {
	// Check uv/amazep-installed CLI packages first
...
export async function ensureTool(tool: ToolName, silentOrOptions?: EnsureToolOptions): Promise<string | undefined> {
	...
	// Handle uv/amazep-installed CLI packages
	const pythonConfig = PYTHON_TOOLS[tool];
	if (pythonConfig) {
		if (!silent) {
			logger.debug(`${pythonConfig.name} not found. Installing via uv/amazep...`);
		}
