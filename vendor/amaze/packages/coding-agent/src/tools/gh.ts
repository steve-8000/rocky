...
import { getWorktreesDir, isEnoent, prompt, untilAborted } from "@amaze/utils";
...
async function resolvePrBranchPushTarget(
	...
	const headRef = await git.config.getBranch(repoRoot, localBranch, "amazePrHeadRef", signal);
	if (!headRef) {
	...
	const prUrl = await git.config.getBranch(repoRoot, localBranch, "amazePrUrl", signal);
	const maintainerCanModifyValue = await git.config.getBranch(
		repoRoot,
		localBranch,
		"amazePrMaintainerCanModify",
		signal,
	);
	const isCrossRepositoryValue = await git.config.getBranch(repoRoot, localBranch, "amazePrIsCrossRepository", signal);
...
async function checkoutPullRequest(
	...
	return git.withRepoLock(
		...
		async () => {
			...
			await git.config.setBranch(repoRoot, localBranch, "amazePrHeadRef", headRefName, signal);
			await git.config.setBranch(repoRoot, localBranch, "amazePrUrl", data.url ?? "", signal);
			await git.config.setBranch(
				repoRoot,
				localBranch,
				"amazePrIsCrossRepository",
