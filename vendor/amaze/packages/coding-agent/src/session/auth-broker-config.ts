 ...
 *   1. `AMAZE_AUTH_BROKER_URL` / `AMAZE_AUTH_BROKER_TOKEN` env vars.
...
import { getAgentDir, getConfigRootDir, isEnoent, logger } from "@amaze/utils";
...
/** Path to the local bearer token file. Created on the broker host by `amaze auth-broker token`. */
...
export async function resolveAuthBrokerConfig(): Promise<AuthBrokerClientConfig | null> {
	const envUrl = process.env.AMAZE_AUTH_BROKER_URL;
	const envToken = process.env.AMAZE_AUTH_BROKER_TOKEN;
	...
	if (!token) {
		throw new Error(
			`AMAZE_AUTH_BROKER_URL is set (${url}) but no bearer token is available. ` +
				`Set AMAZE_AUTH_BROKER_TOKEN, the \`auth.broker.token\` config entry, or place one at ${getAuthBrokerTokenFilePath()}.`,
		);
	}
	return { url, token };
}
