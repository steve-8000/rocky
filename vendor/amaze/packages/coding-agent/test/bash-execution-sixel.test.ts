...
import { BashExecutionComponent } from "@amaze/coding-agent/modes/components/bash-execution";
import { getThemeByName, setThemeInstance } from "@amaze/coding-agent/modes/theme/theme";
import { sanitizeWithOptionalSixelPassthrough } from "@amaze/coding-agent/utils/sixel";
...
describe("BashExecutionComponent SIXEL sanitization", () => {
	const originalForceProtocol = Bun.env.AMAZE_FORCE_IMAGE_PROTOCOL;
	const originalAllowPassthrough = Bun.env.AMAZE_ALLOW_SIXEL_PASSTHROUGH;
	...
	afterEach(() => {
		if (originalForceProtocol === undefined) delete Bun.env.AMAZE_FORCE_IMAGE_PROTOCOL;
		else Bun.env.AMAZE_FORCE_IMAGE_PROTOCOL = originalForceProtocol;
		if (originalAllowPassthrough === undefined) delete Bun.env.AMAZE_ALLOW_SIXEL_PASSTHROUGH;
		else Bun.env.AMAZE_ALLOW_SIXEL_PASSTHROUGH = originalAllowPassthrough;
	});

	it("preserves SIXEL output when passthrough gates are enabled", () => {
		Bun.env.AMAZE_FORCE_IMAGE_PROTOCOL = "sixel";
		Bun.env.AMAZE_ALLOW_SIXEL_PASSTHROUGH = "1";
	...
	it("strips SIXEL control escapes when passthrough gates are disabled", () => {
		delete Bun.env.AMAZE_FORCE_IMAGE_PROTOCOL;
		delete Bun.env.AMAZE_ALLOW_SIXEL_PASSTHROUGH;
