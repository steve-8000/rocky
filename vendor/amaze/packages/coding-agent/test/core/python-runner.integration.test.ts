 ...
 *
 * Gated by `AMAZE_PYTHON_INTEGRATION=1` so CI without a real Python interpreter
 * (or sandboxes where subprocess spawning is restricted) does not fail.
 */
import { afterEach, describe, expect, it } from "bun:test";
import * as path from "node:path";
import { disposeAllKernelSessions, executePythonWithKernel } from "@amaze/coding-agent/eval/py/executor";
import { PythonKernel } from "@amaze/coding-agent/eval/py/kernel";
import { TempDir } from "@amaze/utils";

const SHOULD_RUN = Bun.env.AMAZE_PYTHON_INTEGRATION === "1";
