...
import { ExtensionRunner } from "@amaze/coding-agent/extensibility/extensions/runner";
...
import { getProjectAgentDir, TempDir, withTimeout } from "@amaze/utils";
...
describe("AgentSession auto-compaction queue resume", () => {
	...
	it("forwards todo reminder lifecycle signals to extensions", async () => {
		...
		const serializedMessages = JSON.stringify(session.agent.state.messages);
		expect(serializedMessages).toContain("Incomplete tasks remain.");
		expect(serializedMessages).toContain("Continue the first actionable remaining task now.");
		expect(serializedMessages).toContain("[Status: 0/1 completed, 1 remaining]");
		expect(continueSpy).toHaveBeenCalledTimes(1);
