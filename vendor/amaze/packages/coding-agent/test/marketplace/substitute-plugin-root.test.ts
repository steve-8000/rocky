...
import { substitutePluginRoot } from "@amaze/coding-agent/discovery/substitute-plugin-root";
...
const AMAZE_VAR = "$" + "{AMAZE_PLUGIN_ROOT}";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces AMAZE_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${AMAZE_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${AMAZE_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
