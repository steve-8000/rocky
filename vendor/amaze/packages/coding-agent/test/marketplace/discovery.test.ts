 ...
 * @amaze/natives (native Rust addon via glob). We cannot call it here.
 *
...
import type { InstalledPluginEntry } from "@amaze/coding-agent/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
	readInstalledPluginsRegistry,
	writeInstalledPluginsRegistry,
} from "@amaze/coding-agent/extensibility/plugins/marketplace";

// ── Inline validator ───────────────────────────────────────────────────────────
//
// Mirrors parseClaudePluginsRegistry() in discovery/helpers.ts exactly.
// Kept here to avoid importing helpers.ts (which pulls in @amaze/natives).
...
beforeEach(() => {
	...
	ompRegistryPath = path.join(tmpHome, AMAZE_CONFIG_DIR, "plugins", "installed_plugins.json");
...
describe("AMAZE registry path contract", () => {
	...
	it("AMAZE config dir name is .amaze", () => {
		...
		expect(AMAZE_CONFIG_DIR).toBe(".amaze");
	});
});
