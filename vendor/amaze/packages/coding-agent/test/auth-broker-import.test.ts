...
import { type AuthBrokerServerHandle, AuthStorage, SqliteAuthCredentialStore, startAuthBroker } from "@amaze/ai";
import { getAgentDbPath, setAgentDir } from "@amaze/utils";
...
describe("auth-broker import (CLIProxyAPI)", () => {
	...
	beforeEach(async () => {
		originalAgentDir = process.env.AMAZE_AGENT_DIR;
	...
	afterEach(async () => {
		...
		if (originalAgentDir === undefined) delete process.env.AMAZE_AGENT_DIR;
		else process.env.AMAZE_AGENT_DIR = originalAgentDir;
...
describe("auth-broker import (broker-routed)", () => {
	...
	beforeEach(async () => {
		savedEnv.AMAZE_AUTH_BROKER_URL = process.env.AMAZE_AUTH_BROKER_URL;
		savedEnv.AMAZE_AUTH_BROKER_TOKEN = process.env.AMAZE_AUTH_BROKER_TOKEN;
		...
		process.env.AMAZE_AUTH_BROKER_URL = handle.url;
		process.env.AMAZE_AUTH_BROKER_TOKEN = token;
	});
	...
	afterEach(async () => {
		...
		await fs.rm(cliproxyDir, { recursive: true, force: true });
		for (const key of ["AMAZE_AUTH_BROKER_URL", "AMAZE_AUTH_BROKER_TOKEN"] as const) {
