...
import { type SkillPromptDetails, stripInternalDetailsFields } from "@amaze/coding-agent/session/messages";
import { type CustomMessageEntry, SessionManager } from "@amaze/coding-agent/session/session-manager";
	...
	it("F1: strips __pendingDisplayTag from persisted details while preserving all other SkillPromptDetails fields", () => {
		...
		const id = session.appendCustomMessageEntry<SkillPromptDetails>(
			...
			true,
			{
				...
				args: "bar",
				lineCount: 10,
				__pendingDisplayTag: "amaze-cmd-1-0",
			},
			"user",
		);
