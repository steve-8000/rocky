...
} from "@amaze/coding-agent/plan-mode/approved-plan";
...
describe("validatePlanContentForApproval", () => {
	const validPlan = [
		"# Goal-linked implementation plan",
		"",
		"## Objective",
		"Ship the goal and plan lifecycle changes.",
		"",
		"## Scope",
		"- Update goal runtime invariants.",
		"- Update plan mode approval.",
		"",
		"## Execution Plan",
		"1. Add tests.",
		...
		"",
		"## Verification",
		"- Run focused goal and plan tests.",
	].join("\n");

	it("accepts a plan with the required execution-contract sections", () => {
		expect(() => validatePlanContentForApproval(validPlan)).not.toThrow();
	});

	it("rejects an empty approval artifact", () => {
		expect(() => validatePlanContentForApproval(" \n\t ")).toThrow("Plan file is empty");
	});

	it("rejects a plan without a top-level title", () => {
		expect(() =>
			validatePlanContentForApproval(validPlan.replace("# Goal-linked implementation plan", "## Title")),
		).toThrow("Plan must include a top-level # title");
	});

	it("rejects a plan missing a verification section", () => {
		expect(() => validatePlanContentForApproval(validPlan.replace("## Verification", "## Checks"))).toThrow(
			"Plan must include a ## Verification section",
		);
	});

	it("rejects placeholder-only plans", () => {
		expect(() =>
			validatePlanContentForApproval(
				[
					"# Placeholder",
		...
		).toThrow("Plan still contains unresolved placeholder content");
	});
});
