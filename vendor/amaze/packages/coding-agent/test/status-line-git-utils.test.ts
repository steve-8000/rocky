...
import {
	...
	parseDefaultBranch,
	parseGitHubRepo,
} from "@amaze/coding-agent/modes/components/status-line/git-utils";

describe("parseGitHubRepo", () => {
	test("parses HTTPS URL", () => {
		expect(parseGitHubRepo("https://github.com/can1357/amaze.git")).toBe("can1357/amaze");
	});

	test("parses HTTPS URL without .git suffix", () => {
		expect(parseGitHubRepo("https://github.com/can1357/amaze")).toBe("can1357/amaze");
	});

	test("parses SSH scp-style URL", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/amaze.git")).toBe("loftiskg/amaze");
	});

	test("parses SSH scp-style URL without .git suffix", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/amaze")).toBe("loftiskg/amaze");
	});
