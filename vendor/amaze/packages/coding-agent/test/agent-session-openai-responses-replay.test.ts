...
import type { AssistantMessage, Message, ProviderPayload, ProviderSessionState, Usage } from "@amaze/ai/types";
import { createOpenAIResponsesHistoryPayload } from "@amaze/ai/utils";
import type { AgentSession } from "@amaze/coding-agent/session/agent-session";
import type { AuthStorage } from "@amaze/coding-agent/session/auth-storage";
import {
	type SessionEntry,
	SessionManager,
...
async function createSessionHarness(
	...
	const [{ createAgentSession }, { Settings }, { AuthStorage }] = await Promise.all([
		import("@amaze/coding-agent/sdk"),
		import("@amaze/coding-agent/config/settings"),
		import("@amaze/coding-agent/session/auth-storage"),
	]);
