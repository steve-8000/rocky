...
import { type TodoPhase, TodoReadTool, TodoWriteTool } from "@amaze/coding-agent/tools";
...
describe("TodoReadTool", () => {
	it("returns the current session todo phases without mutating them", async () => {
		const initial: TodoPhase[] = [
			{
				name: "Execution",
				tasks: [
					{ content: "Inspect current state", status: "completed" },
					{ content: "Continue remaining work", status: "in_progress" },
				],
			},
		];
		const tool = new TodoReadTool(createSession(initial));

		const result = await tool.execute("call-read", {});

		expect(result.isError).toBeUndefined();
		expect(result.details?.phases).toEqual(initial);
		const summary = result.content.find(part => part.type === "text");
		if (!summary || summary.type !== "text") throw new Error("Expected text summary from todo_read");
		expect(summary.text).toContain("Remaining items (1):");
		expect(summary.text).toContain("Continue remaining work [in_progress] (Execution)");
	});
});
