...
import { canUseInteractiveBashPty } from "@amaze/coding-agent/tools/bash-pty-selection";

const originalPlatform = process.platform;
const originalNoPty = Bun.env.AMAZE_NO_PTY;
...
function setNoPty(value: string | undefined): void {
	if (value === undefined) {
		delete Bun.env.AMAZE_NO_PTY;
		return;
	}
	Bun.env.AMAZE_NO_PTY = value;
}
