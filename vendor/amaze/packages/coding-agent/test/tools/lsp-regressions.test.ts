...
import type { RenderResultOptions } from "@amaze/agent-core";
import { LspTool } from "@amaze/coding-agent/lsp";
import * as lspClient from "@amaze/coding-agent/lsp/client";
import * as lspConfig from "@amaze/coding-agent/lsp/config";
import { getServersForFile, loadConfig } from "@amaze/coding-agent/lsp/config";
import { renderCall, renderResult } from "@amaze/coding-agent/lsp/render";
import type { CodeAction, Diagnostic, LspClient, ServerConfig, SymbolInformation } from "@amaze/coding-agent/lsp/types";
import {
	applyCodeAction,
	collectGlobMatches,
	...
	resolveSymbolColumn,
} from "@amaze/coding-agent/lsp/utils";
import { getThemeByName } from "@amaze/coding-agent/modes/theme/theme";
import type { ToolSession } from "@amaze/coding-agent/tools";
import { clampTimeout } from "@amaze/coding-agent/tools/tool-timeouts";
import * as piUtils from "@amaze/utils";
import { sanitizeText, TempDir } from "@amaze/utils";
...
describe("lsp regressions", () => {
	...
	it("limits glob collection to avoid large diagnostic stalls", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-glob-");
		try {
			await Promise.all([
	...
	it("treats existing bracket paths as literal diagnostic targets", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-bracket-path-");
		try {
	...
	it("resolves the requested symbol occurrence on a line", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-regression-");
		try {
	...
	it("throws when symbol does not exist on the target line", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-missing-symbol-");
		try {
			const filePath = `${tempDir.path()}/symbol.ts`;
	...
	it("throws when occurrence is out of bounds", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-occurrence-");
		try {
			const filePath = `${tempDir.path()}/symbol.ts`;
	...
	it("does not reuse stale file diagnostics after another URI publishes", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-stale-diags-");
		try {
	...
	it("detects Windows local .exe LSP shims in node_modules/.bin", async () => {
		...
		const tempDir = TempDir.createSync("@amaze-lsp-win32-bin-");
	...
	it("resolves bundled package LSP bins before PATH fallback", async () => {
		const projectDir = TempDir.createSync("@amaze-lsp-package-project-");
		const packageDir = TempDir.createSync("@amaze-lsp-package-dir-");
		const previousPackageDir = process.env.AMAZE_PACKAGE_DIR;
		const whichSpy = vi.spyOn(piUtils, "$which").mockReturnValue(null);

		try {
			await Promise.all([
				Bun.write(path.join(projectDir.path(), "pyproject.toml"), "[project]\nname = 'example'\n"),
				Bun.write(path.join(packageDir.path(), "package.json"), "{}"),
			]);
			const binDir = path.join(packageDir.path(), "node_modules", ".bin");
			await fs.promises.mkdir(binDir, { recursive: true });
			const bundledPyright = path.join(binDir, "pyright-langserver");
			await Bun.write(bundledPyright, "");
			process.env.AMAZE_PACKAGE_DIR = packageDir.path();

			const config = loadConfig(projectDir.path());

			expect(config.servers.pyright?.resolvedCommand).toBe(bundledPyright);
			...
			if (previousPackageDir === undefined) {
				...
				process.env.AMAZE_PACKAGE_DIR = previousPackageDir;
			}
			vi.restoreAllMocks();
			projectDir.removeSync();
			packageDir.removeSync();
		}
	});

	it("detects tlaplus files for LSP startup and language ids", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-tlaplus-");
	...
	it("rename_file applies LSP willRenameFiles edits and renames the file", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-rename-file-");
		try {
	...
	it("rename_file with apply:false previews edits without filesystem changes", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-rename-file-preview-");
		try {
	...
	it("rename_file enumerates every file inside a directory rename", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-rename-dir-");
		try {
	...
	it("request action sends raw LSP method with auto-built textDocument/position params", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-request-");
		try {
	...
	it("request action forwards explicit JSON payload verbatim", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-request-payload-");
		try {
			const server: ServerConfig = { command: "test-lsp", fileTypes: ["ts"], rootMarkers: [] };
			const client: LspClient = {
				name: "test-lsp",
	...
	it("capabilities action dumps server capabilities", async () => {
		const tempDir = TempDir.createSync("@amaze-lsp-caps-");
		try {
			const server: ServerConfig = { command: "test-lsp", fileTypes: ["ts"], rootMarkers: [] };
			const client: LspClient = {
				name: "test-lsp",
