...
} from "@amaze/coding-agent/tools/gh";
...
async function expectedWorktreePath(home: string, primaryRoot: string, localBranch: string): Promise<string> {
	const encoded = path
		...
		.replace(/^[/\\]/, "")
		.replace(/[/\\:]/g, "-");
	return fs.realpath(path.join(home, ".amaze", "wt", encoded, localBranch));
}
...
describe("github tool", () => {
	...
	it("checks out multiple pull requests in a single call when pr is an array", async () => {
		...
		try {
			...
			expect(runGit(fixture.repoRoot, ["config", "--get", "branch.pr-100.amazePrUrl"])).toBe(
				"https://github.com/owner/repo/pull/100",
			);
			expect(runGit(fixture.repoRoot, ["config", "--get", "branch.pr-200.amazePrUrl"])).toBe(
