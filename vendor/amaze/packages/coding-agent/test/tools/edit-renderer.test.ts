...
import { editToolRenderer } from "@amaze/coding-agent/edit/renderer";
import { HL_EDIT_SEP } from "@amaze/coding-agent/hashline/hash";
...
describe("editToolRenderer", () => {
	...
	it("shows hashline envelope input while preview diff is not computable yet", async () => {
		...
		const rendered = Bun.stripANSI(component.render(160).join("\n"));
		expect(rendered).toContain("crates/natives/src/shell.rs");
