 ...
 *
 * Each test isolates `AMAZE_GITHUB_CACHE_DB` to a temp file and clears
...
import { getOrFetchIssue, getOrFetchPr } from "@amaze/coding-agent/tools/gh";
import {
	clearAll,
	getCached,
	getOrFetchView,
...
} from "@amaze/coding-agent/tools/github-cache";
import * as git from "@amaze/coding-agent/utils/git";
...
beforeEach(async () => {
	...
	originalEnv = process.env.AMAZE_GITHUB_CACHE_DB;
	process.env.AMAZE_GITHUB_CACHE_DB = path.join(tempDir, "github-cache.db");
	resetCacheForTests();
});
...
afterEach(async () => {
	...
	if (originalEnv === undefined) {
		delete process.env.AMAZE_GITHUB_CACHE_DB;
	} else {
		process.env.AMAZE_GITHUB_CACHE_DB = originalEnv;
	}
	vi.restoreAllMocks();
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.AMAZE_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
