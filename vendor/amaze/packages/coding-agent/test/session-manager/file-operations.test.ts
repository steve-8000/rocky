...
import { getConfigRootDir, getSessionsDir, Snowflake, setAgentDir } from "@amaze/utils";
...
describe("SessionManager temp cwd session dirs", () => {
	...
	it("stores symlink-equivalent home cwd sessions under home-relative directories", () => {
		...
		const realProjectDir = fs.mkdtempSync(path.join(projectsRoot, "amaze-session-home-"));
		const nestedDir = path.join(realProjectDir, "nested");
		const aliasRoot = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-session-home-alias-"));
