...
import { executeBash } from "@amaze/coding-agent/exec/bash-executor";
import { DEFAULT_MAX_BYTES } from "@amaze/coding-agent/session/streaming-output";
import * as shellSnapshot from "@amaze/coding-agent/utils/shell-snapshot";
...
describe("executeBash", () => {
	...
	it("passes env vars", async () => {
		const result = await executeBash("echo $AMAZE_TEST_ENV", {
			cwd: tempDir,
			timeout: 5000,
			env: { AMAZE_TEST_ENV: "hello" },
		});
		expect(result.output.trim()).toBe("hello");
	});

	it("applies non-interactive environment defaults", async () => {
		const result = await executeBash('echo "$GIT_TERMINAL_PROMPT:$AMAZE_TEST_ENV"', {
			cwd: tempDir,
			timeout: 5000,
			env: { AMAZE_TEST_ENV: "hello" },
		});
	...
	it("resets persistent session state after abort", async () => {
		...
		await executeBash("export AMAZE_RESET_VAR=alive", { cwd: tempDir, timeout: 5000, sessionKey });
		const beforeAbort = await executeBash("echo $AMAZE_RESET_VAR", { cwd: tempDir, timeout: 5000, sessionKey });
		...
		const afterAbort = await executeBash("echo ${AMAZE_RESET_VAR:-unset}", {
			cwd: tempDir,
			timeout: 5000,
			sessionKey,
		});
	...
	it("sources snapshot env vars across session commands", async () => {
		...
		fs.writeFileSync(snapshotPath, "export AMAZE_SNAPSHOT_TEST=from_snapshot\n");
		...
		const result = await executeBash("echo $AMAZE_SNAPSHOT_TEST", { cwd: tempDir, timeout: 5000, sessionKey });
