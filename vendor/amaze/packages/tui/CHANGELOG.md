...
- Fixed `renderInlineMarkdown` crashing with `TypeError: undefined is not an object (evaluating 'e.replace')` when called with a non-string value during streaming — partial JSON parsing leaves option label fields temporarily unpopulated, causing the ask tool renderer to fail. ([#1176](https://github.com/can1357/amaze/issues/1176))

## [15.0.2] - 2026-05-15

### Added

- Restored the `Key` runtime helper on `@amaze/tui` to mirror upstream `@mariozechner/tui`'s surface. `Key.enter`, `Key.escape`, `Key.tab`, … return the canonical key-name strings; modifier methods (`Key.ctrl(k)`, `Key.shift(k)`, `Key.ctrlShift(k)`, etc.) build precisely-typed `KeyId` literals like `"ctrl+c"`. Pure runtime convenience for typed key-id construction — plugins built against the upstream package surface that import `Key` (e.g. `@plannotator/amaze-extension`, `@juicesharp/rpiv-ask-user-question`) load again now that the specifier shim remaps them onto this package.
...
- Fixed rapidly blinking cursor artifact during task execution by consolidating cursor control sequences into the synchronized output buffer ([#992](https://github.com/can1357/amaze/issues/992))
...
- Re-export `getDefaultTabWidth`, `getIndentation`, and `setDefaultTabWidth` from `@amaze/utils`; native text helpers still receive tab width via wrappers that read the JS default
...
- Fixed viewport repaint scrollback accounting during resize oscillation to avoid double-scrolling on height shrink and added exact-row scrollback assertions in overlay regression coverage ([#228](https://github.com/can1357/amaze/issues/228), [#234](https://github.com/can1357/amaze/issues/234))
## [13.5.3] - 2026-03-01
...
- Removed `AMAZE_TUI_RESIZE_CLEAR_STRATEGY`; resize behavior is no longer configurable between viewport/scrollback modes. The renderer now uses fixed semantics: width changes perform a hard reset (`3J` + full content rewrite), while height changes and diff fallbacks use viewport-scoped repainting.
...
- Added `AMAZE_TUI_RESIZE_CLEAR_STRATEGY` environment variable to control terminal behavior on resize: `viewport` (default) clears/redraws the viewport while preserving scrollback, or `scrollback` clears all history
...
- Restored terminal image protocol override and fallback detection for image rendering, including `AMAZE_FORCE_IMAGE_PROTOCOL` support and Kitty fallback for screen/tmux/ghostty-style TERM environments.
...
- Fixed history-entry navigation anchoring (Up opens at top, Down opens at bottom) and preserved editor scroll context when max-height changes to keep cursor movement visible in long prompts ([#99](https://github.com/can1357/amaze/issues/99)).
...
- Changed default value of `AMAZE_HARDWARE_CURSOR` environment variable from implicit true to explicit `"1"` for clarity
- Changed default value of `AMAZE_CLEAR_ON_SHRINK` environment variable from implicit false to explicit `"0"` for clarity
...
- Changed TUI rendering to add `clearOnShrink` option (controlled by `AMAZE_CLEAR_ON_SHRINK` env var) for reducing redraws on slower terminals
...
- Added `isNotificationSuppressed()` function to check `AMAZE_NOTIFICATIONS` environment variable
- Added `TERMINAL` constant providing detected terminal capabilities at runtime

### Changed

- Changed notification suppression environment variable from `AMAZE_NOTIFICATIONS` to `AMAZE_NOTIFICATIONS`
- Changed TUI write log environment variable from `AMAZE_TUI_WRITE_LOG` to `AMAZE_TUI_WRITE_LOG`
- Changed hardware cursor environment variable from `AMAZE_HARDWARE_CURSOR` to `AMAZE_HARDWARE_CURSOR`
- Updated environment variable access to use `getEnv()` utility function from `@amaze/utils` for consistent handling
...
- Moved `wrapTextWithAnsi` export to `@amaze/natives` package
...
- Removed `truncateToWidth`, `sliceWithWidth`, and `extractSegments` functions from public API (now re-exported directly from @amaze/natives)
...
Initial release under @amaze scope. See previous releases at [badlogic/amaze-mono](https://github.com/badlogic/amaze-mono).

## [0.31.1] - 2026-01-02

### Fixed

- `visibleWidth()` now strips OSC 8 hyperlink sequences, fixing text wrapping for clickable links ([#396](https://github.com/badlogic/amaze-mono/pull/396) by [@Cursivez](https://github.com/Cursivez))
...
- `visibleWidth()` reimplemented with grapheme-based width calculation, 10x faster on Bun and ~15% faster on Node ([#369](https://github.com/badlogic/amaze-mono/pull/369) by [@nathyong](https://github.com/nathyong))

### Fixed

- Markdown component now renders HTML tags as plain text instead of silently dropping them ([#359](https://github.com/badlogic/amaze-mono/issues/359))
- Crash in `visibleWidth()` and grapheme iteration when encountering undefined code points ([#372](https://github.com/badlogic/amaze-mono/pull/372) by [@HACKE-RC](https://github.com/HACKE-RC))
- ZWJ emoji sequences (rainbow flag, family, etc.) now render with correct width instead of being split into multiple characters ([#369](https://github.com/badlogic/amaze-mono/pull/369) by [@nathyong](https://github.com/nathyong))

## [0.29.0] - 2025-12-25

### Added

- **Auto-space before pasted file paths**: When pasting a file path (starting with `/`, `~`, or `.`) and the cursor is after a word character, a space is automatically prepended for better readability. Useful when dragging screenshots from macOS. ([#307](https://github.com/badlogic/amaze-mono/pull/307) by [@mitsuhiko](https://github.com/mitsuhiko))
- **Word navigation for Input component**: Added Ctrl+Left/Right and Alt+Left/Right support for word-by-word cursor movement. ([#306](https://github.com/badlogic/amaze-mono/pull/306) by [@kim0](https://github.com/kim0))
- **Full Unicode input**: Input component now accepts Unicode characters beyond ASCII. ([#306](https://github.com/badlogic/amaze-mono/pull/306) by [@kim0](https://github.com/kim0))

### Fixed

- **Readline-style Ctrl+W**: Now skips trailing whitespace before deleting the preceding word, matching standard readline behavior. ([#306](https://github.com/badlogic/amaze-mono/pull/306) by [@kim0](https://github.com/kim0))