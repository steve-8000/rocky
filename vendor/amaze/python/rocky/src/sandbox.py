...
PAT injection via `--config-env` (see `rocky.git_ops`); the `ProxyGitTransport`
in `rocky.proxy_client` forwards the same set of operations over HMAC RPC.
...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   ...
   single-owner. Owned by the active slot UID/GID (`amaze-N`) with mode
...
from rocky.natives_cache import CacheHit, NativesCache
...
class GitTransport(Protocol):
    ...
    - `rocky.proxy_client.ProxyGitTransport`: forwards over HMAC RPC.
    """
...
class SandboxManager:
    ...
    def _populate_natives_cache(self, workspace: Workspace, *, slot_uid: int | None = None) -> None:
        """Try to hardlink cached natives artifacts into the worktree.
