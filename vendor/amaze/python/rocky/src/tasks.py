...
from rocky.db import Database, IssueRow, IssueState, issue_key
...
from rocky.sandbox import GitTransport, SandboxManager
from rocky.worker import DirectiveInfo, TaskInputs, ThreadMessage, run_task
...
def _directive_from_payload(payload: Mapping[str, Any]) -> DirectiveInfo | None:
    ...
    raw = payload.get("_rocky_directive")
...
async def handle_pr_conversation(
    *,
    ...
    # _rocky_directive; _directive_from_payload rejects it but the key
    # being present tells us a mention happened. Reply cheaply without amaze.
    if directive is None and payload.get("_rocky_directive") is not None:
        comment = _comment_from_payload(payload)
        log.info(
