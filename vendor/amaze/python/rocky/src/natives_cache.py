...
# napi build reads: all workspace crates (natives transitively depends on
# pi-ast/amaze-iso/amaze-shell), the workspace Cargo manifest + lock, the rust
...
def _git_safe_directory_env(repo_dir: Path) -> dict[str, str]:
    ...
    ``rocky.sandbox._safe_directory_env`` but kept local to avoid a circular
...
class NativesCache:
    """Per-repo content-addressed cache of natives build outputs."""
