...
from rocky.db import INACTIVE_EVENT_STATES, get_database
...
from rocky.proxy_client import GitHubProxyClient
...
def triage(issue_ref: str, wait_timeout: float | None) -> None:
    ...
    if not cfg.allows(repo_full):
        click.echo(f"refusing: {repo_full} not in ROCKY_REPO_ALLOWLIST", err=True)
        sys.exit(2)
