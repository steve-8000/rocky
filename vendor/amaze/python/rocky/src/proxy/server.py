...
Rocky calls every endpoint with HMAC headers (see `rocky.proxy_hmac`).
Authenticated requests dispatch to a single `GitHubClient` instance holding
the PAT, or to `rocky.git_ops` for git transport. The PAT never leaves
this process.

Endpoint payloads are deliberately typed (no generic GitHub passthrough):
each one names exactly one operation rocky performs.
"""
...
from rocky.github_client import GitHubClient, GitHubError
from rocky.proxy_hmac import HEADER_SIGNATURE, HEADER_TIMESTAMP, verify
from rocky.sandbox import _safe_directory_env, _slot_subprocess_kwargs
from rocky.sandbox import workspace_key as compute_workspace_key
...
def _resolve_hmac_key(cfg: Settings) -> bytes:
    if cfg.gh_proxy_hmac_key is None:
        raise HTTPException(500, "gh-proxy: ROCKY_GH_PROXY_HMAC_KEY not configured")
...
def create_proxy_app(settings: Settings) -> FastAPI:
    ...
    app = FastAPI(title="rocky-gh-proxy", version="0.1.0", lifespan=lifespan)
    ...
    # ---- git transport ----
    #
    # The underlying `rocky.git_ops` primitives are blocking `subprocess.run`
