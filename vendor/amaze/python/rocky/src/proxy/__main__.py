"""`python -m rocky.proxy serve` — run the gh-proxy FastAPI app."""
...
from rocky.config import Settings, load_proxy_settings
from rocky.logging_config import configure_logging
...
def _settings_or_die() -> Settings:
    ...
    `ROCKY_GH_PROXY_HMAC_KEY` — the orchestrator's webhook secret,
...
def serve() -> None:
    ...
    if cfg.gh_proxy_hmac_key is None:
        click.echo("gh-proxy: ROCKY_GH_PROXY_HMAC_KEY is required in proxy mode", err=True)
        sys.exit(2)
