"""Unit tests for `rocky.natives_cache`.
...
from rocky.natives_cache import (
    CACHE_KEY_PATHS,
    NativesCache,
    _atomic_link,
    compute_key,
...
def _seed_repo(root: Path, *, with_all_inputs: bool = True) -> Path:
    ...
    if with_all_inputs:
        ...
        (natives / "package.json").write_text('{"name":"@amaze/natives"}\n')
...
def test_compute_key_changes_when_each_input_changes(tmp_path: Path) -> None:
    ...
    mutations: dict[str, tuple[str, str]] = {
        "crates": ("crates/natives/src.rs", "// new comment\n"),
