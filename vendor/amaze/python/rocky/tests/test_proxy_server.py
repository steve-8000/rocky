...
from rocky.proxy.server import create_proxy_app
from rocky.proxy_hmac import HEADER_SIGNATURE, HEADER_TIMESTAMP, sign
from rocky.sandbox import workspace_key
...
def _build_settings(tmp_path: Path) -> Settings:
    ...
    cfg = Settings.model_construct(
        ...
        sqlite_path=tmp_path / "rocky.sqlite",
...
def test_read_origin_url_uses_safe_directory_and_slot_identity(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from rocky.proxy import server as proxy_server
    ...
    monkeypatch.setattr("rocky.proxy.server.subprocess.run", fake_run)
    monkeypatch.setattr(
        "rocky.proxy.server._slot_subprocess_kwargs",
...
async def test_hmac_accept_post_comment_round_trip(proxy_settings: Settings) -> None:
    ...
    assert resp.json() == {"id": 42, "author": "rocky-bot", "body": "hello", "created_at": "2026-01-01T00:00:00Z"}
...
async def test_authenticated_login(proxy_settings: Settings) -> None:
    def gh(req: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json={"login": "rocky-bot"})
    ...
    assert resp.status_code == 200
    assert resp.json() == {"login": "rocky-bot"}
...
async def test_git_push_passes_slot_uid_to_git_push(
    ...
    monkeypatch.setattr("rocky.proxy.server.git_push", fake_git_push)
    app = _build_app(proxy_settings)
    body = (
        b'{"repo":"octo/widget","workspace_key":"octo__widget__1","branch":"'
        + branch.encode()
