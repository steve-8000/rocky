...
from rocky.tasks import _directive_from_payload
...
def test_directive_from_payload_missing_pragmas_is_empty_tuple() -> None:
    directive = _directive_from_payload({"_rocky_directive": {"body": "x", "author": "can1357"}})
    assert directive is not None
...
def test_directive_from_payload_returns_none_for_missing_directive() -> None:
    ...
    assert _directive_from_payload({"_rocky_directive": "not-a-mapping"}) is None
