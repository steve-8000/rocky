...
Runs only when ROCKY_INTEGRATION=1 and `amaze` is available on PATH (or via
ROCKY_AMAZE_COMMAND). Spins up:
...
INTEGRATION = os.environ.get("ROCKY_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROCKY_INTEGRATION=1 required to run the amaze-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from rocky.sandbox import LocalGitTransport, SandboxManager
    from rocky.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("ROCKY_GH_PROXY_URL", "http://gh-proxy.invalid:8081")
    monkeypatch.setenv("ROCKY_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    monkeypatch.setenv("GITHUB_TOKEN", "")
    monkeypatch.setenv("GITHUB_WEBHOOK_SECRET", "secret")
    monkeypatch.setenv("ROCKY_BOT_LOGIN", "rocky-bot")
    monkeypatch.setenv("ROCKY_REPO_ALLOWLIST", "octo/widget")
    monkeypatch.setenv("ROCKY_WORKSPACE_ROOT", str(tmp_path / "workspaces"))
    monkeypatch.setenv("ROCKY_SQLITE_PATH", str(tmp_path / "rocky.sqlite"))
    monkeypatch.setenv("ROCKY_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("ROCKY_TASK_TIMEOUT_SECONDS", "300")
