...
from rocky.sandbox import LocalGitTransport, SandboxManager, Workspace

pytestmark = pytest.mark.skipif(
    os.environ.get("ROCKY_PERMISSION_E2E") != "1",
    reason="set ROCKY_PERMISSION_E2E=1 to run slot-permission e2e tests",
)

_SLOT_ONE = 2001
_SLOT_TWO = 2002
_SHARED_OMP_GID = 2000
_AUTHOR_NAME = "rocky-bot"
_AUTHOR_EMAIL = "rocky-bot@example.invalid"
...
def slot_tmp_path() -> Iterator[Path]:
    root = Path(tempfile.mkdtemp(prefix="rocky-permission-e2e-", dir="/tmp"))
    root.chmod(0o755)
    try:
        yield root
    finally:
...
def _prepare_shared_natives_cache(slot_tmp_path: Path) -> NativesCache:
    """Provision `/data/cache/natives` shape (root:amaze, setgid 2770)."""
    cache_root = slot_tmp_path / "cache" / "natives"
...
def _stage_built_natives(bindings: host_tools.ToolBindings, *, body: str = "ELFx") -> None:
    ...
    """
    _write_as_slot(bindings, "packages/natives/native/amaze_natives.linux-arm64.node", body)
...
def test_natives_cache_shares_artifacts_across_slot_workspaces(
    ...
    _run_ok(bindings2, ["test", "-r", "packages/natives/native/amaze_natives.linux-arm64.node"])
    ...
    _run_ok(
        ...
        [
            ...
            "packages/natives/native/amaze_natives.linux-arm64.node",
        ],
    )
