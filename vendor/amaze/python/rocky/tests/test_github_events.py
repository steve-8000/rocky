...
from rocky.github_events import (
    extract_mention,
    is_maintainer,
    rate_limit_cap,
    route,
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @rocky-bot please look", "rocky-bot") == "hey please look"
    assert extract_mention("@rocky-bot do X", "rocky-bot") == "do X"


def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "rocky-bot") is None
    assert extract_mention(None, "rocky-bot") is None
    assert extract_mention("", "rocky-bot") is None


def test_extract_mention_is_case_insensitive() -> None:
    assert extract_mention("yo @ROCKY-BOT", "rocky-bot") == "yo"


def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @rocky-bot-helper must NOT match @rocky-bot.
    assert extract_mention("@rocky-bot-helper hi", "rocky-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@rocky-bot one, then @rocky-bot two", "rocky-bot") == "one, then two"
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@rocky-bot change the indentation in foo.py",
            },
    ...
    decision = route(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@rocky-bot use a generator here",
            },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@rocky-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
