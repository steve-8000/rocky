			...
			if let Ok(contents) = fs::read_to_string(&expanded) {
					...
					&& !expected.is_empty()
				{
					...
					if !actual_hex.eq_ignore_ascii_case(expected) {
						eprintln!(
							"[natives minimizer] settings_hash mismatch for {} (expected {}, got {}); \
							 ignoring file",
							expanded.display(),
							expected,
							actual_hex
				...
				match pipeline::parse_file(&contents, "user") {
					...
					Err(err) => {
						eprintln!("[natives minimizer] user filters: {err}");
					},
				}
			}
		}
...
impl SettingsFile {
	fn merge_into(self, cfg: &mut MinimizerConfig) {
		...
		{
			eprintln!(
				"[natives minimizer] unsupported schema_version {v} in settings file (expected \
