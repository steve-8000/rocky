...
async fn source_snapshot(shell: &mut BrushShell, snapshot_path: &str) -> Result<()> {
	...
	let source_info = SourceInfo::from("natives:snapshot");
...
async fn run_shell_command(
	...
	let source_info = SourceInfo::from("natives:command");
	let result = session
		.shell
...
async fn run_shell_command_streams(
	...
	let source_info = SourceInfo::from("amaze-shell:streams");
	let result = session
		.shell
		.run_string(options.command.clone(), &source_info, &params)
		.await;
...
impl builtins::Command for TimeoutCommand {
	...
	fn execute<SE: brush_core::ShellExtensions>(
		...
		async move {
			...
			let source_info = SourceInfo::from("natives:timeout");
...
mod tests {
	...
	///
	/// Lives in `natives` because the brush-core crate is excluded from the
	...
	async fn embedded_external_command_runs_in_its_own_session() {
		...
		// Build the same kind of session natives uses in production.
		...
		let shell_handle = tokio::spawn(async move {
			let source_info = SourceInfo::from("natives:test");
