...
const APP_NAME = process.env.PASEO_TEST_APP_NAME?.trim() || "Rocky";

// Rocky: default daemon home to ~/.rocky so a stock Paseo install on the same
// machine (~/.paseo, port 6767) is never touched. Explicit PASEO_HOME wins.
if (!process.env.PASEO_HOME) {
  process.env.PASEO_HOME = path.join(app.getPath("home"), ".rocky");
}
