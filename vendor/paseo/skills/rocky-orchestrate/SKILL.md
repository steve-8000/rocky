...
description: Rocky orchestrator mode — run a Leader/Teammate multi-agent team (AionUi Team Mode semantics) on the Rocky daemon using Paseo MCP tools or the rocky CLI. Use when the user says "orchestrate", "team mode", "leader", "teammates", or asks to split work across parallel agents.
user-invocable: true
---
...
This reproduces AionUi Team Mode on Rocky primitives: daemon agents are Teammates,
...
Prefer Paseo MCP tools when injected (`create_agent`, `wait_for_agent`,
...
3. **Decompose.** Split the goal into independent subtasks with explicit
   ...
   default for code changes; share the workspace folder (AionUi style) only for
...
5. **Monitor.** Loop on `wait_for_agent` + chat room reads.
   ...
   - Silent/stuck agent (no timeline progress for ~10 min) → `get_agent_activity`,
     ...
     mark the board `failed`, and reassign (AionUi auto-escalation).
