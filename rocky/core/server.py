...
"""
Unified OpenAI-compatible API server for rocky.
...
Usage:
    ...
    python -m rocky.server --model mlx-community/Llama-3.2-3B-Instruct-4bit

    # Batched mode (for multiple concurrent users)
    python -m rocky.server --model mlx-community/Llama-3.2-3B-Instruct-4bit --continuous-batching

    # With MCP tools
    python -m rocky.server --model mlx-community/Qwen3-4B-4bit --mcp-config mcp.json
...
async def lifespan(app: FastAPI):
    ...
    # ``rocky serve`` under a service manager would otherwise lose
    ...
    try:
        from rocky.core.telemetry import emit as _telemetry_emit
...
# the body to disk. See rocky/routes/audio.py for the rationale.
...
def load_model(
    ...
    *,
    ...
    if _profile_modality == "text-diffusion":
        ...
        # ``python -m rocky.server --model <alias>`` bypasses
...
Examples:
    ...
    # (rocky/cli.py) so this standalone entrypoint never becomes a
    ...
    # TurboQuant flags — accepted but only functional via rocky serve (cli.py)
    ...
    # was silently dropped — same bug class as #400. The unified rocky
