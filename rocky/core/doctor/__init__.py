...
``rocky doctor`` is a fast (≤ 5 s) self-diagnostic that answers one
...
benchmark``) moved to ``rocky bench --tier ...`` as of v0.7.22.

Entry point: ``rocky doctor [--verbose]``.
...
__all__ = [
    # New env-health surface (the public face of `rocky doctor`).
    "Check",
    "CheckStatus",
    "Report",
    "Section",
    "run_all",
    # Legacy runner — still re-exported because `rocky.bench.tiers.*`
