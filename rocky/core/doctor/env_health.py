...
"""Environment-health probes for ``rocky doctor``.
...
optional dev tools. The user runs ``rocky doctor`` to answer one question
...
Total wall-clock for ``rocky doctor`` ≤ 5 s on a warm cache, dominated by
...
def section_system() -> Section:
    ...
    ⚠ on:
      * non-Apple-Silicon (rocky targets M-series; works elsewhere but
        with Metal-fallback caveats).
      * < 20 GB free disk (model weights are big).
      * HF cache > 100 GB (suggest ``rocky rm`` cleanup).
    ...
    if not cache.exists():
        ...
        if cache_size_gb is None:
            ...
            s.add(
                ...
                "(consider `rocky rm` if unused models accumulated)",
                CheckStatus.WARN,
            ...
            s.add(
                ...
                "(consider `rocky rm` for unused models)",
                CheckStatus.WARN,
...
def _install_location() -> tuple[str, Path]:
    """Classify where ``rocky`` is installed: ``uv tool``, ``pipx``,
...
def section_python() -> Section:
    ...
    # user clearly if they somehow got rocky onto an older interpreter
    ...
    if sys.version_info >= (3, 10):  # noqa: UP036
        ...
        s.add(
            f"Python {py_ver} (rocky requires >= 3.10)",
            CheckStatus.FAIL,
            detail=f"executable={sys.executable}",
        )
...
_ARGCOMPLETE_HOOK_NEEDLE = "register-python-argcomplete rocky"
...
def section_shell_integration(
    *,
    ...
    cli_path = which_fn("rocky")
    if cli_path:
        s.add(
            f"rocky in $PATH ({cli_path})",
            CheckStatus.OK,
        ...
        s.add(
            "rocky NOT on $PATH",
            CheckStatus.FAIL,
            detail="shutil.which('rocky') returned None",
        )
    ...
    if present:
        ...
        s.add(
            ...
            '`eval "$(register-python-argcomplete rocky)"` to your shell rc',
            CheckStatus.WARN,
...
def section_optional_tools(
    *, which: Callable[[str], str | None] | None = None
    ...
    """Probe for development tools that improve the rocky experience but
...
# model stable across rocky versions.
_SECTION_BUILDERS = (
    section_system,
    section_python,
